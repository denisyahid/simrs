...
<body>
    ...
    <div style="text-align:center">
        <h3 style="font-size: 14px"><u>LAPORAN PENDAPATAN UNIT</u><br>Periode {{ $tglAwal }} s/d {{ $tglAkhir }}</h3>
    </div>
    ...
    <table class="custom-table fixed" style="width: 100%">
        ...
        <tbody>
            @php
                $grandtotal = 0;
            @endphp
            @foreach ($data->groupBy('namaruangan') as $item)
                <tr>
                    <td colspan="16" style="border: none; !important">
                        <h3 style="font-size: 12px"><u><i>{{$item[0]->namaruangan}}</i></u></h3>
                    </td>
                </tr>
            @foreach($item as $dt)
                <tr class="border-tb">
