...
<head>
    <title>Formulir Penggunaan Ventilator</title>
    @yield('page-style')
    <style>
        html,
        body {
...
<body>
    ...
    <table width="100%" cellspacing="0" cellpadding="0" style="text-align: center; padding-bottom: 20px;">
        <tr>
            <span style="font-weight: bold;">Formulir Penggunaan Ventilator</span>
        </tr>
    </table>
    ...
    <table width="100%" cellspacing="0" cellpadding="0" style="margin-left: 30px; margin-top: 5px;">
        ...
        <tr>
            ...
            <td>:</td>
            <td><span>{{isset($data['spesialisBertandaTangan']) ? $data['spesialisBertandaTangan'] : '-'}}</span>
            </td>
        </tr>
        <tr>
            <td><span>Jabatan</span></td>
            <td>:</td>
            <td><span>{{isset($data['jabatanBertandaTangan']) ? $data['jabatanBertandaTangan'] : '-'}}</span>
            </td>
        </tr>
    </table>
        ...
        <tr>
            ...
            <td>
                <span>
                    ...
                    echo $data['pasien']['tgllahir'] . ' / ' . hitungUmur($data['pasien']['tgllahir'], $data['registrasi']['tglregistrasi']);
                    ?>
                </span>
            </td>
        </tr>
    ...
    <table width="100%" cellspacing="0" cellpadding="0">
        <tr>
            <span color="#000000">
                Memang benar membutuhkan pemasangan Ventilator :
            </span>
        </tr>
    </table>

    <table width="100%" cellspacing="0" cellpadding="0" style="margin-left: 30px; margin-top: 5px;">
        @foreach ($data['detailPemasangan'] as  $item)
        <tr>
            <td style="width: 200px;"><span>Device:</span></td>
            <td style="width: 20px;">:</td>
            <td><span>{{ isset($item['device']) ? $item['device'] : '-' }}</span></td>
        </tr>
            <tr>
                <td style="width: 200px;"><span>Mulai Pemasangan:</span></td>
                <td style="width: 20px;">:</td>
                <td><span>{{ isset($item['mulaiPemasangan']) ? \Carbon\Carbon::parse($item['mulaiPemasangan'])->setTimezone('Asia/Jakarta')->format('d-m-Y H:i') : '-' }}</span></td>
            </tr>
            <tr>
                <td style="width: 200px;"><span>Akhir Pemasangan:</span></td>
                <td style="width: 20px;">:</td>
                <td><span>{{ isset($item['akhirPemasangan']) ? \Carbon\Carbon::parse($item['akhirPemasangan'])->setTimezone('Asia/Jakarta')->format('d-m-Y H:i') : '-' }}</span></td>
            </tr>
        @endforeach
        <tr>
            <td style="width: 200px;"><span>Alasan:</span></td>
            <td style="width: 20px;">:</td>
            <td>
                <span>{{ isset($data['alasanPemasangan']) ? $data['alasanPemasangan'] : '-' }}</span>
            </td>
        </tr>
    </table>
    ...
    <table class="no-page-break" width="100%" cellspacing="0" cellpadding="0"
        ...
        <tr>
            ...
            <td style="text-align: center" width="40%">
                <br>
                {{-- <span style="font-weight: bold;">Garut, {{ \Carbon\Carbon::parse($data['tanggalInput'])->format('d F Y') }}</span> <br> --}}
                <span style="font-weight: bold;">Garut, {{ \Carbon\Carbon::parse($data['detailPemasangan'][0]['mulaiPemasangan'])->setTimezone('Asia/Jakarta')->format('d F Y') }}</span> <br>
                <img src="data:image/png;base64, {!! $tte !!}"> <br>
                <span style="font-weight: bold;">
                    {{ isset($data['CBDokter'])
                    ? (is_array($data['CBDokter'])
                        ? $data['CBDokter']['label']
                        : $data['CBDokter'])
                    : '-' }}
                </span> <br>
