...
<body>
    ...
    <table width="100%" class="fnt" style="margin-left: 15px;border-collapse: collapse">
        <thead>
            ...
            <tr>
                <td style="width:18%;font-weight: bold;">Status</td>
                <td style="width:32%;">: {!! isset($data['status']) ? nl2br(e($data['status'])) : '-' !!}</td>
            </tr>
            <tr>
    ...
    <table class="no-page-break" width="100%" cellspacing="0" cellpadding="0"
        ...
        <tr>
            <td style="align-items: center">
                ...
                @if (!empty($data['BPJS']) && Str::startsWith($data['BPJS'], 'data:image'))
                    <img src="{{ $data['BPJS'] }}" style="width: 100px; height: 100px; display: block;">
                @else
                    <!-- Fallback jika data tidak valid -->
                    <span>TTD tidak tersedia</span>
                @endif
                <span>{{isset($data['NamaBPJS']) ? $data['NamaBPJS'] : '-' }}</span>
            </td>
        </tr>
    </table>
