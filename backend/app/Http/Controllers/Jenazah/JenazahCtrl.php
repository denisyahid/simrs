...
class JenazahCtrl extends Controller
{
    use Valet;
    ...
    public function jenazahDD(Request $r)
    {
        $set = explode(',', $this->settingFix('kdJenazah'));
    ...
    public function getOrderPJ(Request $request)
    {
        ...
        $dataOrder = DB::table('strukorder_t as so')
            ...
            ->where('ruTu.objectdepartemenfk', $this->settingFix('kdJenazah'))
        ...
        foreach ($dataOrder as $datas) {
            ...
            $result[] = [
                ...
                'objectkelompokpasienlastfk' => $datas->objectkelompokpasienlastfk,
    ...
    public function detailOrderPJ(Request $request)
    {
    ...
    public function savePelayananJenazah(Request $request)
    {
    ...
    public function DetailTindakanJenazah(Request $r)
    {
        ...
        $data = DB::table('pelayananpasien_t as pp')
            ...
            ->where('ru.objectdepartemenfk', $this->settingFix('kdJenazah'))
    ...
    public function simpanNonLayanan(Request $r)
    {
        ...
        try {
            ...
            $SP->objectkelompoktransaksifk = $this->kelompokTransaksi('PELAYANAN PEMULASARAAN JENAZAH');;
            $SP->keteranganlainnya = isset($r['keteranganlainnya']) ?  $r['keteranganlainnya'] : null;
            $SP->tglstruk = date('Y-m-d H:i:s');
            $SP->namapasien_klien = $r['namapasien'];
            $SP->totalharusdibayar = $r['totalharusdibayar'];
            $SP->objectpegawaimenerimafk = $r['pegawaiverifikatorfk'];
            $SP->objectpegawaipenanggungjawabfk = $r['objectpegawaifk'];
            $SP->objectkelompokpasienfk = $r['objectkelompokpasienfk'];
            $SP->objectrekananfk = $r['objectrekananfk'];
            $SP->save();

            $apd = AntrianPasienDiperiksa::where('noregistrasifk', $r['pd_norec'])
                ->where('kdprofile', $this->kdProfile)
                ->where('objectruanganfk', $r['idruangtujuan'])
                ->where('statusenabled', true)
                ->first();
            $getLastAntrian = AntrianPasienDiperiksa::where('objectruanganfk', $r['idruangtujuan'])
                ->where('tglregistrasi', '>=', date('Y-m-d', strtotime($r['tglregistrasi'])) . ' 00:00')
                ->where('tglregistrasi', '<=', date('Y-m-d', strtotime($r['tglregistrasi'])) . ' 23:59')
                ->where('statusenabled', true)
                ->max('noantrian');
            ...
            if (!$apd) {
                ...
                $dataAPD->objectkelasfk = 6;
                $dataAPD->noantrian = $getLastAntrian + 1;
                $dataAPD->noregistrasifk = $r['pd_norec'];
                $dataAPD->objectpegawaifk = $r['objectpegawaiorderfk'];
                $dataAPD->objectruanganfk = $r['idruangtujuan'];
                $dataAPD->statusantrian = 0;
                $dataAPD->statuspasien = 1;
                $dataAPD->status = "Belum Dipanggil";
                $dataAPD->objectstrukorderfk = $r['so_norec'];
                $dataAPD->tglregistrasi = $r['tglregistrasi']; // date('Y-m-d H:i:s');
                $dataAPD->tglmasuk = date('Y-m-d H:i:s');
                $dataAPD->tglkeluar = date('Y-m-d H:i:s');
                $dataAPD->noregistrasi =  $r['noregistrasi'];
                $dataAPD->save();
                $dataAPDnorec = $dataAPD->norec;
            } else {
                AntrianPasienDiperiksa::where('noregistrasifk', $r['pd_norec'])
                    ->where('kdprofile', $this->kdProfile)
                    ->where('objectruanganfk', $r['idruangtujuan'])
                    ->where('statusenabled', true)
                    ->update(['updated_at' => date('Y-m-d H:i:s')]);
            ...
            StrukOrder::where('norec', $r['so_norec'])
                ->where('kdprofile', $this->kdProfile)
                ->update(
                    [
                        'catatanklinis' => $r['catatanklinis'],
                        'statusorder' => 1,
                        'norec_apd' => $dataAPDnorec,
            ...
            $result = array(
                "status" => 400,
                "result"  => $e->getMessage(), // $e->getMessage() . ' '. $e->getLine()
            );
        }
        return $this->respond($result['result'], $result['status'], $transMessage);
    }

    public function laporanPemlusaranJenazah(Request $request)
    {
        ...
        $data = DB::table('pelayananpasien_t as pp')
            ...
            ->join('ruangan_m as rutu', 'rutu.id', 'so.objectruangantujuanfk')
            ->select(
                'prd.namaproduk',
                'ps.namapasien',
                'pg.namalengkap',
            ...
            ->where('cb.id', $request['carabayar'])
            ->where('so.objectkelompoktransaksifk', $this->kelompokTransaksi('PELAYANAN PEMULASARAAN JENAZAH'))
    ...
    public function cetakLaporan(Request $request)
    {
        ...
        $profile = MasterProfile::where('id', $this->kdProfile)->first();
        $periodeAwal = $this->getDateIndo($request['tglAwal']);
        $periodeAkhir = $this->getDateIndo($request['tglAkhir']);
        $departemen = Departemen::select('namadepartemen')->where('kdprofile', $this->kdProfile)->where('statusenabled', true)->where('id', 4)->first();
        $carabayar = CaraBayar::select('carabayar')->where('kdprofile', $this->kdProfile)->where('statusenabled', true)->where('id', $request['carabayar'])->first();
        $pageWidth = 400;
        if ($request['pdf'] == true) {
    ...
    public function getPasienForensikMedikolegal(Request $request)
    {
        ...
        $data = DB::table('antrianpasiendiperiksa_t as apd')
            ...
            ->select(
                ...
                'alm.alamatlengkap',
                DB::raw(
                    "CASE WHEN pj.norec IS NULL THEN 'Belum Diambil' ELSE 'Sudah Diambil' END AS status"
                ),
                'apd.tglmasuk',
    ...
    public function layananJenazahperTindakan(Request $request)
    {
        $kdProfile = $this->kdProfile;
        ...
        $dataReport = array(
            ...
            'user' => $user,
            'judul' => "BUKTI PELAYANAN JENAZAH",
            'header' => $data,
            'details' =>  $details,
        );
        // dd($dataReport);
        return view(
            'report.jenazah.bukti-layanan-jenazah',
...
"))->first();
    ...
    public function savePengambilanJenazah(Request $request)
    {
        $kdProfile = (int) $this->kdProfile;
        DB::beginTransaction();
        try {
            ...
            $newR->objectruanganfk = $r_R['objectruanganfk'];
            $newR->objectuserfk = $this->getUserId();
            $newR->save();
    ...
    public function detailPetugasPJ(Request $r)
    {
        $kdProfile = $this->kdProfile;
    ...
    public function savePetugasPJ(Request $r)
    {
        DB::beginTransaction();
        try {
            if ($r['norec'] == '') {
    ...
    public function listPasienMeninggal(Request $r)
    {
