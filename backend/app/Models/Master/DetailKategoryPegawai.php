...
class DetailKategoryPegawai extends Model
{
    use HasFactory;
    protected $table = "detailkategorypegawai_m";
    protected $fillable = [];
    ...
    public function scopeMine(Builder $builder, int $kdProfile = null)
    {
        ...
        ->select('id','detailkategorypegawai');
    }
    public function scopeSearch(Builder $builder, string $search = null)
    {
        ...
        return $builder->where(function (Builder $sql) use ($search) {
            $sql->where("detailkategorypegawai", "ilike", "%{$search}%")
