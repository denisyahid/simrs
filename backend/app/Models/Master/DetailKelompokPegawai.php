...
class DetailKelompokPegawai extends Model
{
    use HasFactory;
    protected $table = "detailkelompokpegawai_m";
    protected $fillable = [];
    ...
    public function scopeMine(Builder $builder, int $kdProfile = null)
    {
        ...
        ->select('id','detailkelompokpegawai');
    }
    public function scopeSearch(Builder $builder, string $search = null)
    {
        ...
        return $builder->where(function (Builder $sql) use ($search) {
            $sql->where("detailkelompokpegawai", "ilike", "%{$search}%")
