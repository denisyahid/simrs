...
class MapJenisPetugasToJenisPegawai extends Model
{
    use HasFactory;
    protected $table = "mapjenispetugasptojenispegawai_m";
    protected $fillable = [];
    ...
    public function scopeMine(Builder $builder, int $kdProfile = null)
    {
        ...
        ->select('id','objectjenispegawaifk', 'objectjenispetugasfk');
    }
    public function scopeSearch(Builder $builder, string $search = null)
    {
        ...
        return $builder->where(function (Builder $sql) use ($search) {
            $sql->where("objectjenispegawaifk", "ilike", "%{$search}%")
