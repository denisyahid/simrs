...
class PersenHargaJualProduk extends Model
{
     use HasFactory;

    protected $table = "persenhargajualproduk_m";
    protected $guarded = [];
    ...
    public function scopeSearch(Builder $builder, string $search = null)
    {
        ...
        return $builder->where(function (Builder $sql) use ($search) {
            $sql->where("persenuphargasatuan", "ilike", "%{$search}%");
        });
    }
}
