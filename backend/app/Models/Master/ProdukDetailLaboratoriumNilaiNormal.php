...
class ProdukDetailLaboratoriumNilaiNormal extends Model
{
    use HasFactory;
    protected $table = "produkdetaillaboratoriumnilainormal_m";
    protected $fillable = [];
    ...
    public function scopeSearch(Builder $builder, string $search = null)
    {
        ...
        return $builder->where(function (Builder $sql) use ($search) {
            $sql->where("refrange", "ilike", "%{$search}%")
                ->orWhere("refrange", "ilike", "%{$search}%");
        });
    }
