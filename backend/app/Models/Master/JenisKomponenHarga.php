...
class JenisKomponenHarga extends Model
{
    use HasFactory;
    protected $table = "jeniskomponenharga_m";
    protected $fillable = [];
    ...
    public function scopeMine(Builder $builder, int $kdProfile = null)
    {
        ...
        ->select('id','jeniskomponenharga','objectdepartemenfk');
    }
    public function scopeSearch(Builder $builder, string $search = null)
    {
        ...
        return $builder->where(function (Builder $sql) use ($search) {
            $sql->where("jeniskomponenharga", "ilike", "%{$search}%")
