...
class MapBkuToKelompokTransaksi extends Model
{
    use HasFactory;
    protected $table = "mapbkutokelompoktransaksi_m";
    protected $fillable = [];
    ...
    public function scopeMine(Builder $builder, int $kdProfile = null)
    {
        ...
        ->select('id','kelompoktransaksifk', 'idbku', 'bku');
    }
    public function scopeSearch(Builder $builder, string $search = null)
    {
        ...
        return $builder->where(function (Builder $sql) use ($search) {
            $sql->where("bku", "ilike", "%{$search}%")
                ->orWhere("kelompoktransaksifk", "ilike", "%{$search}%");
        });
    }
}
