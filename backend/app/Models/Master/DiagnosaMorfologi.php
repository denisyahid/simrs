...
class DiagnosaMorfologi extends Model
{
    use HasFactory;
    protected $table = "diagnosamorfologi_m";
    protected $fillable = [];
    ...
    public function scopeMine(Builder $builder, int $kdProfile = null)
    {
        ...
        ->select('id','diagnosamorfologi','kddiagnosamorfologi');
    }
    public function scopeSearch(Builder $builder, string $search = null)
    {
        ...
        return $builder->where(function (Builder $sql) use ($search) {
            $sql->where("diagnosamorfologi", "ilike", "%{$search}%");
            $sql->Orwhere("kddiagnosamorfologi", "ilike", "%{$search}%");
        });
    }
