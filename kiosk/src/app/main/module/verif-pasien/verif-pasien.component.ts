...
export class VerifPasienComponent implements OnInit {
  ...
  getPasienByNoCM() {
    ...
    if (this.formGroup.get('jenisPasien').value == 'lama') {
      ...
      this.loading = true
      this.httpservice.get('medifirst2000/reservasionline/get-history?noReservasi=' + this.formGroup.get('noCm').value
        +'&cekin=true').subscribe(e => {
        this.loading = false
        if (e.data.length > 0) {
          this.alertService.info('Pasien sudah memiliki reservasi hari ini ke' + e.data[0].namaruangan + 'silahkan melakukan Check-In', 'INFO', {
            toastClass: 'toast ngx-toastr',
            closeButton: true,
            positionClass: 'toast-bottom-center'
          });
          ...
          // this.httpservice.get('registrasi/pasien-hari-ini?nocm=' + this.formGroup.get('noCm').value).subscribe(er => {
          //   console.log(er)
          //   if (er != null) {
          ...
          //     });
          //   } else {
              this.httpservice.get('registrasi/cek-pasien-piutang?nocm=' + this.formGroup.get('noCm').value).subscribe(er => {
                console.log(er)
                if (er.closing != null) {
  ...
  pilihPoli() {
    ...
    this.router.navigate(['choose-poli'], { queryParams: { nocmfk: this.idPasien, tipepasien: this.url, kebangsaan: this.kebangsaan } })
  }
  tutupPasien() {
    this.item = {}
    this.isInfoPasienBaru = false
