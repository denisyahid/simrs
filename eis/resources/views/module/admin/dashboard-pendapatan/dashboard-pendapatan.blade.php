                                        ...
                                        <div class="panel-body">
                                            <div class="tab-content card-block">
                                                <div class="tab-pane active" id="home1" role="tabpanel">
                                                    <div id="chartPoliPasienPie"></div>
                                                </div>
                                                <div class="tab-pane" id="profile1" role="tabpanel">
                                                    <div id="chartPoliPasien"></div>
                                                </div>
                                            </div>
    ...
    <script>
        ...
        function setChartPendapatan() {
            ...
            for (let g = 0; g < array.length; g++) {
                ...
                if (element.namadepartemen == "Instalasi Rawat Jalan dan Rehabilitasi Medik"
                 || element.namadepartemen == 'Instalasi Rawat Jalan') {
                    totalRajal = totalRajal + parseFloat(element.total)
                    rawatJalan.push(element)
                }                
            }
            for (let i in rawatJalan) {
                samikeun = false
                for (let x in resultSumPoli) {
                    if (resultSumPoli[x].namaruangan == rawatJalan[i].namaruangan &&
                        resultSumPoli[x].kelompokpasien == rawatJalan[i].kelompokpasien) {
                            samikeun = true;
                            resultSumPoli[x].total = parseFloat(resultSumPoli[x].total) + parseFloat(rawatJalan[i].total)                        
                    }
                }
                if (samikeun == false) {
                    let result = {
                        namaruangan: rawatJalan[i].namaruangan,
                        total: rawatJalan[i].total,
                        namadepartemen: rawatJalan[i].namadepartemen,
                        kelompokpasien: rawatJalan[i].kelompokpasien,
                    }
                    resultSumPoli.push(result)
                }
            }
            let seriesPoliPasien = []
            let samakeun = false
            let resultSumPoliPasien = [];
            for (let i in rawatJalan) {
                samakeun = false
                for (let x in resultSumPoliPasien) {
                    if (resultSumPoliPasien[x].namaruangan == rawatJalan[i].namaruangan) {
                        samakeun = true;
                        resultSumPoliPasien[x].total = parseFloat(resultSumPoliPasien[x].total) + parseFloat(rawatJalan[i].total)                        
                    }
                }
                if (samakeun == false) {
                    var details = [];
                    var rinci = [];
                    for (var f = 0; f < resultSumPoliPasien.length; f++) {
                        if (rawatJalan[i].namaruangan == resultSumPoliPasien[f].namaruangan) {
                            details.push([resultSumPoliPasien[f].namaruangan, resultSumPoliPasien[f].total]);
                            rinci.push(resultSumPoliPasien[f]);
                        };
                    }
                    let result = {
                        namaruangan: rawatJalan[i].namaruangan,
                        total: rawatJalan[i].total,
                        id: rawatJalan[i].namaruangan,
                        name: rawatJalan[i].namaruangan,
                        data: details,
                        rincian: rinci
                    }
                    resultSumPoliPasien.push(result)
                }
            }
            // console.log(resultSumKelompokPasien)
            // asupkeun kana series data di CHART
            let dataPoliPasienPie = []
            let slices = true
            for (let z in resultSumPoliPasien) {
                let datana = [];
                datana.push({
                    y: parseFloat(resultSumPoliPasien[z].total),
                    color: this.colors[z],
                    drilldown: resultSumPoliPasien[z].namaruangan
                });

                seriesPoliPasien.push({
                    name: resultSumPoliPasien[z].namaruangan,
                    data: datana
                });
                dataPoliPasienPie.push({
                    name: resultSumPoliPasien[z].namaruangan,
                    y: parseFloat(resultSumPoliPasien[z].total),
                    sliced: slices,
                    selected: slices
            ...
            Highcharts.chart('chartPoliPasien', {
                chart: {
                    type: 'column',
                ...
                // },
                series: seriesPoliPasien,
                drilldown: {
                    series: resultSumPoliPasien
                },
                exporting: {
                    enabled: false
                },
                credits: {
                    // enabled: false
                    text: 'Total : Rp. ' + formatRupiah(totalRajal,''),
                    style: {
                        color:"black",
                        cursor: "pointer",
                        fontSize: "12px"
            ...
            Highcharts.chart('chartPoliPasienPie', {
                chart: {
                    plotBackgroundColor: null,
                    plotBorderWidth: null,
                    plotShadow: false,
