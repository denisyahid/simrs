...
sb.id,
    ...
    public function loginKeun(Request $r)
    {
        ...
        if (empty( $r->password) || empty($r->username)) {
            $response = array(
                'metaData' => array(
                    "code" => 400,
...
//                dd(session("role"));
    ...
    {
        $KelompokUser = collect(DB::select("
            SELECT nilaifield
            FROM settingdatafixed_m where kdprofile = 21 AND namafield = 'KelompokUserEis'
        "))->first();
        // dd($KelompokUser);
        // $listKelUser = explode(',',$KelompokUser->nilaifield);
        $user = \DB::table('loginuser_s')
            ->where('namauser', $data["username"])
            // ->where('passcode', $this->encryptSHA1($data['password']))
            ->first();
//            dd($user);
        if (!empty($user) && $this->checkHashEncypt($data['password'], $user->katasandi)) {

            // if (in_array($user->objectkelompokuserfk, $listKelUser)) {
                session(["role"=> 'admin']);
    ...
    {
        if (config('jwt.IS_PASSWORD_HASH')) {
            return Hash::make($pass);
        } else {
            return sha1($pass);
        }
