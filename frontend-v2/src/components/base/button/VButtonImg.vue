...
export type VButtonImgSize = 'medium' | 'big' | 'huge'
export type VButtonImgColor =
  | 'primary'
  | 'info'
  | 'success'
  | 'warning'
...
export type VButtonImgDark = '1' | '2' | '3' | '4' | '5' | '6' | '7'
...
export default defineComponent({
  props: {
    ...
    placeload: {
      ...
      validator: (value: string) => {
        if (value.match(CssUnitRe) === null) {
          console.warn(
            `VButtonImg: invalid "${value}" placeload. Should be a valid css unit value.`
          )
        }
    ...
    color: {
      type: String as PropType<VButtonImgColor>,
      default: undefined,
      validator: (value: VButtonImgColor) => {
        ...
        if (
          ...
          console.warn(
            `VButtonImg: invalid "${value}" color. Should be primary, info, success, warning, danger, dark, light, white or undefined`
          )
          return false
        }
    ...
    size: {
      type: String as PropType<VButtonImgSize>,
      default: undefined,
      validator: (value: VButtonImgSize) => {
        // The value must match one of these strings
        if ([undefined, 'medium', 'big', 'huge'].indexOf(value) === -1) {
          console.warn(
            `VButtonImg: invalid "${value}" size. Should be big, huge, medium or undefined`
          )
          return false
        }
    ...
    dark: {
      type: String as PropType<VButtonImgDark>,
      default: undefined,
      validator: (value: VButtonImgDark) => {
        // The value must match one of these strings
        if ([undefined, '1', '2', '3', '4', '5', '6', '7'].indexOf(value) === -1) {
          console.warn(
            `VButtonImg: invalid "${value}" dark. Should be 1, 2, 3, 4, 5, 6 or undefined`
          )
          return false
        }
  ...
  setup(props, { slots, attrs }) {
    ...
    const getChildrens = () => {
      ...
      if (isIconify.value) {
        ...
        const icon = h('img', { 'aria-hidden': true, src: props.icon })
        iconWrapper = h('span', { class: 'icon' }, icon)
      }
      ...
      if (isCaretIconify.value) {
        ...
        const caret = h('img', { 'aria-hidden': true, src: props.iconCaret })
        caretWrapper = h('span', { class: 'caret' }, caret)
      }
...
.is-dark {
  .v-button {
    &:not(.is-primary):not(.is-success):not(.is-info):not(.is-purple):not(.is-warning):not(.is-danger):not(.is-light):not(.is-white) {
    ...
    &.is-orange {
      ...
      &.is-raised:hover {
        box-shadow: var(--orange-box-shadow) !important;
      }
