                ...
                <div class="column is-12 pt-3">
                    <div class="columns is-multiline">
                        <div class="column is-3 columns is-multiline">
                            ...
                            <div class="column is-6">
                                <VDatePicker v-model="item.tglakhir" color="green" trim-weeks>
                                    <template #default="{ inputValue, inputEvents }">
                                        <VField label="&nbsp;">
                                            <VControl icon="feather:calendar">
                                                <VInput type="text" placeholder="Select a date" :value="inputValue"
                            ...
                            <DataTable v-else :rows="10" :value="dataSource" :rowsPerPageOptions="[5, 10, 15,50,100,1000]"
                                ...
                                <Column field="tanggal" header="Tanggal">
                                    <template #body="slotProps">
                                        {{ H.formatDateIndo(slotProps.data.tanggal) }}
                                    </template>
                                </Column>
                                <Column field="jam" header="Jam" />
                                <Column field="jam_nobukti" header="Jam No Bukti" style="min-width:40px" width="10%"/>
                                <Column field="tglreg" header="Tgl.Reg">
                                    <template #body="slotProps">
                                        {{ H.formatDateIndo(slotProps.data.tanggal) }}
                                    </template>
                                </Column>
                                <Column field="noreg" header="NoReg" />
                                <Column field="nobukti" header="NoBukti" />
                                <Column field="nrm" header="NRM" />
                                <Column field="namapasien" header="Nama Pasien" />
                                <Column field="alamat" header="Alamat" />
                                <Column field="nokartu" header="NoKartu" />
                                <Column field="nosep" header="NoSEP" />
                                <Column field="jenispasien" header="Jenis Pasien" />
                                <Column field="nama_customer" header="Nama Customer" />
                                <Column field="tipeperawatan" header="Tipe Perawatan" />
                                <Column field="layanan" header="Layanan" style="text-align:right">
                                    <template #body="slotProps">
                                        {{ H.formatRupiah(slotProps.data.layanan, '') }}
                                    </template>
                                </Column>
                                <Column field="ibsa" header="IBSA" style="text-align:right">
                                    <template #body="slotProps">
                                        {{ H.formatRupiah(slotProps.data.ibsa, '') }}
                                    </template>
                                </Column>
                                <Column field="obat" header="Obat" style="text-align:right">
                                    <template #body="slotProps">
                                        {{ H.formatRupiah(slotProps.data.obat, '') }}
                                    </template>
                                </Column>
                                ...
                                <Column field="nilaibayar" header="Nilai Bayar" style="text-align:right">
                                    <template #body="slotProps">
                                ...
                                <Column field="nama_asli" header="Nama Asli" />
                                <Column field="description" header="Description" />
                                <Column field="pasienlost" header="Pasien Lost" />
                                <Column field="ket" header="Keterangan" />
...
const fetchPagu = async () => {
    ...
    await useApi().get(`remunerasi/get-pagu-remunerasi?tglpelayanan=${H.formatDate(item.value.tglPulang, 'YYYY-MM-DD')}${kpId}${dokter}${search}`).then((response: any) => {
...
const savePagu = async () => {
    ...
    loadSave.value = true
    let tglAwal = H.formatDate(item.value.tglPulang, 'YYYY-MM-DD 00:00:00')
    let tglAkhir = H.formatDate(item.value.tglPulang, 'YYYY-MM-DD 23:59:59')
    let a = 0
    let b = 0
    let c = 0
    let e = 0
...
const fetchKelompokPasien = async () => {
    useApi().get('jasapelayanan/get-combo-idx').then((response) => {
...
async function loadDataApi(activeTab: any) {
    if (activeTab == 0) {
        await useApi().get(`jasapelayanan/get-pagu-noreg?
        tglawal=${H.formatDate(item.value.tglawal, 'YYYY-MM-DD')}&
        tglakhir=${H.formatDate(item.value.tglakhir, 'YYYY-MM-DD')}`).then((response: any) => {
            // response.data1.forEach((element: any, i: any) => {
            //     element.no = i + 1
            //     item.value.tglLayanan = H.formatDateIndoSimple(element.tglpelayanan)
            //     item.value.THargaSatuan = parseFloat(element.hargasatuan) + parseFloat(item.value.THargaSatuan)
