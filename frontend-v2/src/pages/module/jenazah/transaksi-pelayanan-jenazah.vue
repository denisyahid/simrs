<template>
                  ...
                  <div class="column is-7">
                    <div class="columns is-multiline is-pulled-right mr-4">
                      <div class="column pr-0 m-0">
                        <VButton class="ml-2 is-pulled-right" icon="pi pi-plus" raised bold
                          @click="inputTindakan(item)" :loading="isLoadingBill" color="info">
                          Input Tindakan
                        </VButton>
                      </div>
  ...
  const listButton: any = ref([
    ...
    {
      label: 'Cetak Bukti layanan jenazah',
      icon: 'fas fa-print',
      command: () => {
        ...
        H.printBlade('laboratorium/layanan-lab-pertindakan?noregistrasi=' + item.registrasi.noregistrasi + '&ket=jenazah'
          + '&norec=' + norec);
      }
    },
    // {
    //   label: 'Cetak Surat Serah Terima Jenazah',
    //   icon: 'fas fa-print',
    //   command: () => {
    ...
    // },
    // {
    //   label: 'Cetak Surat Permohonan Tindakan Pada Jenazah',
    //   icon: 'fas fa-print',
    //   command: () => {
    //     hasilLabLIS(item.norec);
    //   }
  ...
  const fetchLayanan = async () => {
    ...
    dataSource.value = []
    await useApi().get(
      `/jenazah/rincian-pelayanan?norec_pd=${NOREC_PD}`).then(async (response: any) => {
  ...
  const loadPetugas = async (e: any) => {
    ...
    await useApi().get(
      `/jenazah/petugas-pj?norec=${e.norec}`).then((response: any) => {
  ...
  const savePetugas = () => {
    ...
    useApi().post(
      `/jenazah/save-petugas-pj`, json).then((response: any) => {
