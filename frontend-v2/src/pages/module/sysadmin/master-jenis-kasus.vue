<template>
    ...
    <VCard>
        <div class="columns column c-title">
            <h3 class="title is-5 mb-2 ml-2" style="z-index:1">Jenis Kasus</h3>
        </div>

        <div class="columns is-multiline ml-4">
            <div class="column is-4">
                <img src="/images/avatars/label/validasi.png" slt="image Jenis Kasus" class="s-keluar">
                <VCard>
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-12">
                            <VField label="Jenis Kasus">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.jeniskasus" placeholder="Jenis Kasus"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
            ...
            <div class="column is-8 mt-3">
                ...
                <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
                    <DataTable :value="dataSourcefiltered" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
                        ...
                        <Column field="jeniskasus" header="Jenis Kasus" :sortable="true"></Column>
...
useHead({
    title: 'Jenis Kasus - ' + import.meta.env.VITE_PROJECT,
})
...
const dataSourcefiltered = computed(() => {
    ...
    return dataSource.value.filter((items: any) => {
        return (
            items.jeniskasus.match(new RegExp(filters.value, 'i'))
        )
    })
})
...
const fetchData = async () => {
    ...
    await useApi().get(`/sysadmin/master-jenis-kasus?statusenabled=${isAktif.value}`).then((response: any) => {
...
const save = async (item: any) => {
    if (!item.jeniskasus) {
        useToaster().error('Jenis Kasus harus di isi')
        return
    }
    ...
    await useApi().post(
        `/sysadmin/master-jenis-kasus/save`, objSave).then((response: any) => {
...
const deleterow = async (e: any) => {
    isLoading.value = true
    await useApi().post(
        `/sysadmin/master-jenis-kasus/delete`, { 'id': e.id }).then((response: any) => {
            isLoading.value = false
            clear()
            fetchData()
        }, (error) => {
...
const edit = (e: any) => {
    item.value.id = e.id
    item.value.jeniskasus = e.jeniskasus
