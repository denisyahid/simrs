<template>
    ...
    <VCard>
        <div class="columns column c-title">
            <h3 class="title is-5 mb-2 mr-1">Jenis Racikan</h3>
        </div>
            ...
            <div class="column is-8" style="margin-top: 2.2rem;">
                ...
                <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
                    <DataTable :value="dataSourcefiltered" :paginator="true" :rows="5" :rowsPerPageOptions="[5, 10, 25]"
                        ...
                        <Column field="jenisracikan" header="Jenis Racikan" :sortable="true"></Column>
                        ...
                        <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
                            <div class="tile-grid-item">
                                <div class="tile-grid-item-inner"  @click="edit(item)">
                                    <VAvatar size="medium" picture="/images/avatars/icon/ic-jenisracikan.png"
                                        color="primary" squared bordered />

                                    <div class="meta">
                                        <span class="dark-inverted mb-2">{{ item.jenisracikan }}</span>
                ...
                <VCard>
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-12">
                            <VField label="Jenis Racikan">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.jenisracikan" placeholder="Jenis Racikan"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
        ...
        <VModal :open="modalDetail" title="Detail Jenis Racikan" actions="right"
...
useHead({
  title: 'Jenis Racikan - ' + import.meta.env.VITE_PROJECT,
})
...
const dataSourcefiltered = computed(() => {
    ...
    return dataSource.value.filter((items: any) => {
        return (
            items.jenisracikan.match(new RegExp(filters.value, 'i'))
        )
    })
})
...
const fetchData = async ()=>{
    ...
    await useApi().get(`/sysadmin/master-jenis-racikan?statusenabled=${item.value.aktif}`).then((response)=>{
...
const save = async ()=>{
    if (!item.value.jenisracikan) {
        useToaster().error('Jenis Racikan harus di isi')
        return
    }
    var objSave =
    {
        'jenisracikan': {
            ...
            'jenisracikan': item.value.jenisracikan,
            'statusenabled': item.value.statusenabled,
        }
    }
    isLoadingBtn.value = true
    await useApi().post(
        `/sysadmin/master-jenis-racikan/save`, objSave).then((response: any) => {
...
const deleteItem = async (e: any)=>{
    isLoading.value = true
    await useApi().post(
        `/sysadmin/master-jenis-racikan/delete`, { 'id': e.id }).then((response: any) => {
            isLoading.value = false
            fetchData()
        }, (error) => {
        })
...
const edit = (e: any)=>{
    item.value.id = e.id
    item.value.jenisracikan = e.jenisracikan
