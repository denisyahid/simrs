<template>
  <VCard radius="small">
    <div class="columns column">
      <h3 class="title is-5 mb-2 mr-1">Signa</h3>
    </div>
    ...
    <div class="columns is-multiline">
      <div class="column is-8">
        ...
        <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
          <DataTable :value="dataSourcefiltered" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
            ...
            <Column field="signa" header="Signa" :sortable="true"></Column>
            <Column field="jumlahpakai" header="Jumlah Pakai"></Column>
            ...
            <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
              <div class="tile-grid-item">
                <div class="tile-grid-item-inner">
                  ...
                  <div class="meta">
                    <span class="dark-inverted"> Signa : {{ item.signa }}</span>
                    <span style="margin-top: 0.4rem; margin-bottom: 0.4rem">Jumlah Pakai : {{ item.jumlahpakai }} </span>
      ...
      <div class="column is-4">
        <img src="/images/avatars/label/signa.png" alt="" srcset=""
          style="max-width:50%;margin-left: 13rem; margin-top: -5rem;">
            ...
            <div class="column is-12">
              <VField label="Signa">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.signa" placeholder="Signa" class="is-rounded" />
                </VControl>
              </VField>
            </div>
            <div class="column is-12">
              <VField label="Jumlah Pakai">
                <VControl icon="feather:bookmark">
                  <VInput type="number" v-model="item.jumlahpakai" placeholder="Jumlah Pakai" class="is-rounded" />
                </VControl>
              </VField>
            </div>
...
useHead({
  title: 'Signa - ' + import.meta.env.VITE_PROJECT,
})
...
async function fetchData() {
  ...
  if (item.signa) Signa = '&signa' + item.signa
  ...
  const response = await useApi().get(
    '/sysadmin/master-signa?offset=' + offset +
    '&limit=' + limit +
    '&rows=' + rows +
    Signa + JumlahPakai + StatusEnabled
  )
  isLoading = false
...
async function save() {
  debugger
  if (!item.value.signa) {
    useToaster().error('Signa harus di isi')
    return
  }
  if (!item.value.jumlahpakai) {
    useToaster().error('Jumlah Pakai harus di isi')
    return
  }
  var objSave =
  {
    'signa': {
      ...
      'signa': item.value.signa,
      'jumlahpakai': item.value.jumlahpakai,
      'statusenabled': item.value.statusenabled ? item.value.statusenabled : null,
    }
  }
  isLoadingTT.value = true
  await useApi().post(
    `/sysadmin/save-signa`, objSave).then((response: any) => {
...
async function deleterow(e: any) {
  await useApi().post(
    `/sysadmin/delete-signa`, { 'id': e.id }).then((response: any) => {
      fetchData ()
    }, (error) => {
    })
}
