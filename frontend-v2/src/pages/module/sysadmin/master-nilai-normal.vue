<template>
    ...
    <VCard>
        <div class="columns column c-title">
            <h3 class="title is-5 mb-2 mr-1">Nilai Normal</h3>
        </div>
        <div class="columns is-multiline">
            <div class="column is-8" style="margin-top: 2.2rem;">
                        ...
                        <VField v-slot="{ id }" class="is-icon-select">
                            <VControl class="mr-0">
                                <Multiselect v-model="selectView" :attrs="{ id }" placeholder="Select View" label="rhesus"
                                    :options="d_View" :searchable="true" track-by="rhesus" mode="single"
                ...
                <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
                    <DataTable :value="dataSourcefiltered" :paginator="true" :rows="5" :rowsPerPageOptions="[5, 10, 25]"
                        ...
                        <Column field="kelompokumur" header="Kelompok Umur" :sortable="true"></Column>
                        <Column field="nilaitext" header="Nilai Normal" :sortable="true"></Column>
                        <Column field="nilaimin" header="Nilai Min" :sortable="true"></Column>
                        <Column field="nilaimax" header="Nilai Max" :sortable="true"></Column>
                        ...
                        <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
                            <div class="tile-grid-item">
                                <div class="tile-grid-item-inner">
                                    <VAvatar size="medium" picture="/images/avatars/svg/lab.svg"
                                        color="primary" squared bordered />

                                    <div class="meta">
                                        <span class="dark-inverted mb-2">{{ item.rhesus }}</span>
            ...
            <div class="column is-4">
                <img src="/images/avatars/label/hand.png" class="rhesus" alt="">
                <VCard>
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-12">
                            <VField label="Metode Pemeriksaan">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.metode" placeholder="Rhesus"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-6">
                            <VField label="Nilai Min">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.nilaimin" placeholder="Nilai Min"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-6">
                            <VField label="Nilai Max">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.nilaimax" placeholder="Nilai Max"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-12">
                            <VField label="Nilai Text">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.nilaitext" placeholder="Nilai Text"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-12">
                            <VField label="Jenis Kelamin">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.objectjeniskelaminfk" placeholder="Jenis Kelamin"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-12">
                            <VField label="Kelompok Umur">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.kelompokumurfk" placeholder="Jenis Kelamin"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
        ...
        <VModal :open="modalDetail" title="Detail Rhesus" actions="right"
                ...
                <form class="modal-form">
                    <div class="columns is-multiline">
                        <div class="column is-12">
                            <VField label="Rhesus">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.rhesus" placeholder="Rhesus"
...
useHead({
  title: 'Nilai Normal - ' + import.meta.env.VITE_PROJECT,
})
...
const dataSourcefiltered = computed(() => {
    ...
    return dataSource.value.filter((items: any) => {
        return (
            items.nilaitext.match(new RegExp(filters.value, 'i'))
        )
    })
})
...
const fetchData = async ()=>{
    ...
    await useApi().get(`/laboratorium/get-nilai-normal?statusenabled=${item.value.aktif}`).then((response)=>{
...
const save = async ()=>{
    if (!item.value.rhesus) {
        useToaster().error('Rhesus harus di isi')
        return
    }
    var objSave =
    {
        'rhesus': {
            ...
            'rhesus': item.value.rhesus,
            'statusenabled': item.value.statusenabled,
        }
    }
    isLoadingTT.value = true
    await useApi().post(
        `/sysadmin/master-rhesus/save`, objSave).then((response: any) => {
...
const deleteItem = async (e: any)=>{
    ...
    await useApi().post(
        `/sysadmin/master-rhesus/delete`, { 'id': e.id }).then((response: any) => {
