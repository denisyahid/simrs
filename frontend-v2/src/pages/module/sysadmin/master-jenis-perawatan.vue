<template>
    ...
    <VCard>
        <div class="columns column c-title">
            <h3 class="title is-5 mb-2 mr-1">Jenis Perawatan</h3>
        </div>
            ...
            <div class="column is-8">
                ...
                <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
                    <DataTable :value="dataSourcefiltered" :paginator="true" :rows="5" :rowsPerPageOptions="[5, 10, 25]"
                        ...
                        <Column field="jenisperawatan" header="Jenis Perawatan" :sortable="true"></Column>
                        ...
                        <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
                            <div class="tile-grid-item">
                                <div class="tile-grid-item-inner" @click="edit(item)">
                                    <VAvatar size="medium" picture="/images/avatars/icon/ic-jenisperawatan.png"
                                        color="primary" squared bordered />

                                    <div class="meta">
                                        <span class="dark-inverted mb-2">{{ item.jenisperawatan }}</span>
                                    ...
                                    <VDropdown icon="feather:more-vertical" spaced right>
                                        <template #content>
                                            ...
                                            <a v-if="item.statusenabled == true" role="menuitem"
                ...
                <VCard>
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-12">
                            <VField label="Jenis Perawatan">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.jenisperawatan" placeholder="Jenis Perawatan"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
        ...
        <VModal :open="modalDetail" title="Detail Jenis Perawatan" actions="right" @close="modalDetail = false, clear()">
            <template #content>
...
useHead({
    title: 'Transmedic - Jenis Perawatan',
})
...
const dataSourcefiltered = computed(() => {
    ...
    return dataSource.value.filter((items: any) => {
        return (
            items.jenisperawatan.match(new RegExp(filters.value, 'i'))
        )
    })
})
...
const fetchData = async () => {
    ...
    await useApi().get(`/sysadmin/master-jenis-perawatan?statusenabled=${item.value.aktif}`).then((response) => {
...
const save = async () => {
    if (!item.value.jenisperawatan) {
        useToaster().error('Jenis Perawatan harus di isi')
        return
    }
    var objSave =
    {
        'jenisperawatan': {
            ...
            'jenisperawatan': item.value.jenisperawatan,
            'statusenabled': item.value.statusenabled,
        }
    }
    isLoadingBtn.value = true
    await useApi().post(
        `/sysadmin/master-jenis-perawatan/save`, objSave).then((response: any) => {
...
const deleteItem = async (e: any) => {
    isLoading.value = true
    await useApi().post(
        `/sysadmin/master-jenis-perawatan/delete`, { 'id': e.id }).then((response: any) => {
            isLoading.value = false
            fetchData()
        }, (error) => {
        })
...
const edit = (e: any) => {
    item.value.id = e.id
    item.value.jenisperawatan = e.jenisperawatan
