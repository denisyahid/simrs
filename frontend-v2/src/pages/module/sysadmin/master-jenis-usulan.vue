<template>
    ...
    <VCard>
        <div class="columns column c-title">
            <h3 class="title is-5 mb-2 mr-1">Jenis Usulan</h3>
        </div>
            ...
            <div class="column is-8">
                ...
                <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
                    <DataTable :value="dataSourcefiltered" :paginator="true" :rows="5" :rowsPerPageOptions="[5, 10, 25]"
                        ...
                        <Column field="jenisusulan" header="Jenis Usulan" :sortable="true"></Column>
                        ...
                        <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
                            <div class="tile-grid-item">
                                <div class="tile-grid-item-inner" @click="edit(item)">
                                    <VAvatar size="medium" picture="/images/avatars/icon/ic-jenis-generik.png"
                                        color="primary" squared bordered />

                                    <div class="meta">
                                        <span class="dark-inverted mb-2">{{ item.jenisusulan }}</span>
                ...
                <VCard>
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-12">
                            <VField label="Jenis Usulan">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.jenisusulan" placeholder="Jenis Usulan"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
        ...
        <VModal :open="modalDetail" title="Detail Jenis Usulan" actions="right" @close="modalDetail = false, clear()">
            <template #content>
...
const dataSourcefiltered = computed(() => {
    ...
    return dataSource.value.filter((items: any) => {
        return (
            items.jenisusulan.match(new RegExp(filters.value, 'i'))
        )
    })
})
...
const fetchData = async () => {
    ...
    await useApi().get(`/sysadmin/master-jenis-usulan?statusenabled=${item.value.aktif}`).then((response) => {
...
const save = async () => {
    if (!item.value.jenisusulan) {
        useToaster().error('Jenis Usulan harus di isi')
        return
    }
    var objSave =
    {
        'id': item.value.id ? item.value.id : '',
        'jenisusulan': item.value.jenisusulan,
        'statusenabled': item.value.statusenabled,
    }
    isLoadingBtn.value = true
    await useApi().post(
        `/sysadmin/master-jenis-usulan/save`, objSave).then((response: any) => {
...
const deleteItem = async (e: any) => {
    isLoading.value = true
    await useApi().post(
        `/sysadmin/master-jenis-usulan/delete`, { 'id': e.id }).then((response: any) => {
            isLoading.value = false
            fetchData()
        }, (error) => {
        })
...
const edit = (e: any) => {
    item.value.id = e.id
    item.value.jenisusulan = e.jenisusulan
