<template>
  <div class="columns column">
    <h3 class="title is-5 mb-2 mr-1">Jenis Petugas Pelaksana</h3>
  </div>
    ...
    <div class="column is-8">
      ...
      <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
        <DataTable :value="dataSourcefiltered" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
          ...
          <Column field="jenispetugaspe" header="Jenis Petugas Pelaksana" :sortable="true"></Column>
          ...
          <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
            <div class="tile-grid-item">
              <div class="tile-grid-item-inner">
                <VAvatar size="medium" picture="/images/avatars/svg/pelaksana.svg" color="primary" squared bordered />
                <div class="meta">
      ...
      <VCard>
        <div class="columns is-multiline">
          ...
          <div class="column is-12">
            <VField label="Jenis Petugas Pelaksana">
              <VControl icon="feather:user">
                <VInput type="text" v-model="item.jenispetugaspe" placeholder="Jenis Petugas Pelaksana" class="is-rounded" />
              </VControl>
            </VField>
          </div>
          <div class="column is-8">
          ...
          <div class="column is-6">
            <VButton @click="clear()" :loading="isLoadingTT" type="button" icon="feather:skip-back" class="is-fullwidth mr-3"
              color="warning" raised rounded>
              Batal
            </VButton>
          </div>
        </div>
...
useHead({
  title: 'Jenis Petugas Pelaksana - ' + import.meta.env.VITE_PROJECT,
})
const item: any = ref({
  aktif: true
})
...
const d_kelompokjenispetugaspe = ref([])
let isLoading: any = ref(false)
let isLoadingTT: any = ref(false)
let ID_JenisPetugasPelaksana = useRoute().query.id as string
let ID_JenisPetugasPelaksana_SET = ref()
...
async function fetchData() {
  ...
  let id = ''
  let StatusEnabled = ''
  if (item.value.jenispetugaspe) JenisPetugasPe = '&jenispetugaspe=' + item.value.jenispetugaspe
  ...
  const response = await useApi().get(
    '/sysadmin/master-jenis-petugas-pelaksana?offset=' + offset +
    '&limit=' + limit +
    '&rows=' + rows +
    id + JenisPetugasPe + ReportDisplay + StatusEnabled
  )
  isLoading = false
...
function edit(e: any) {
  item.value.id = e.id
  item.value.jenispetugaspe = e.jenispetugaspe
...
async function save() {
  debugger
  if (!item.value.jenispetugaspe) {
    useToaster().error('Nama Jenis Petugas Pelaksana harus di isi')
    return
  }
  var objSave =
  {
    'jenispetugaspe': {
      ...
      'jenispetugaspe': item.value.jenispetugaspe,
  ...
  await useApi().post(
    `/sysadmin/save-jenis-petugas-pelaksana`, objSave).then((response: any) => {
...
async function deleterow(e: any) {
  await useApi().post(
    `/sysadmin/delete-jenis-petugas-pelaksana`, { 'id': e.id }).then((response: any) => {
      fetchData()
    }, (error) => {
    })
}
