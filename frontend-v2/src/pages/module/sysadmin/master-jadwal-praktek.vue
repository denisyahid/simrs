<template>
    ...
    <VCard>
        <div class="columns column c-title">
            <h3 class="title is-5 mb-2 ml-2" style="z-index:1">Jadwal Praktek</h3>
        </div>

        <div class="columns is-multiline">
            <div class="column is-4">
                <img src="/images/avatars/label/scheduler.png" slt="image status keluar" class="mix">
                <VCard>
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-12">
                            <VField label="Waktu Praktek">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.waktupraktek" placeholder="Waktu Praktek"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-6">
                            <VField label="Jam Mulai">
                                <VControl icon="feather:clock">
                                    <VInput type="text" v-model="item.jammulai" placeholder="Jam Mulai"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-6">
                            <VField label="Jam Selesai">
                                <VControl icon="feather:clock">
                                    <VInput type="text" v-model="item.jamselesai" placeholder="Jam Selesai"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
            ...
            <div class="column is-8 mt-3">
                ...
                <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
                    <DataTable :value="dataSourcefiltered" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
                        ...
                        <Column field="waktupraktek" header="Waktu Praktek" :sortable="true"></Column>
                        <Column field="kdpraktek" header="Kode" :sortable="true"></Column>
                        ...
                        <Column :exportable="false" header="Action" style="text-align: center;">
                            <template #body="slotProps">
                                ...
                                <VIconButton v-if="item.aktif == true" type="button" icon="fas fa-trash" color="danger"
                                    circle outlined raised v-tooltip.top="'Hapus'" @click="dialogConfirm(slotProps.data)">
                                </VIconButton>
                                <VIconButton v-else disabled type="button" icon="fas fa-trash" color="danger" circle
                        ...
                        <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
                            <div class="tile-grid-item">
                                <div class="tile-grid-item-inner">
                                    <VAvatar size="medium" picture="/images/avatars/icon/ic-timeScheduler.png"
                                        color="primary" squared bordered />

                                    <div class="meta">
                                        <span class="dark-inverted mb-2">{{ item.waktupraktek }}</span>
                                        <p class="dark-inverted">{{ item.jammulai }} - {{ item.jamselesai }}</p>
        ...
        <VModal :open="modalDetail" title="Detail Jadwal Praktek" actions="right" @close="modalDetail = false, clear()">
            <template #content>
                <form class="modal-form">
                    <div class="columns is-multiline">
                        <div class="column is-12">
                            <VField label="Waktu Praktek">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" readonly style="cursor:pointer" v-model="item.waktupraktek"
                                        placeholder="Kelompok Produk" class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-8">
                            <VField label="Kode Praktek">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" readonly style="cursor:pointer" v-model="item.kdpraktek"
                            ...
                            <VField label="Status Aktivasi">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" readonly style="cursor:pointer" v-model="item.status"
                                        placeholder="Status Aktivasi" class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-6">
                            <VField label="Jam Mulai">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" readonly style="cursor:pointer" v-model="item.jammulai"
                                        placeholder="Jam Mulai" class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-6">
                            <VField label="Jam Selesai">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" readonly style="cursor:pointer" v-model="item.jamselesai"
                                        placeholder="Jam Selesai" class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                    </div>
...
useHead({
    title: 'Transmedic - Jadwal Praktek',
})
const item: any = ref({
    aktif: true
})
...
const dataSourcefiltered = computed(() => {
    ...
    return dataSource.value.filter((items: any) => {
        return (
            items.waktupraktek.match(new RegExp(filters.value, 'i'))
        )
    })
})
...
const fetchData = async () => {
    ...
    await useApi().get(`/sysadmin/master-jadwal-praktek?statusenabled=${item.value.aktif}`).then((response: any) => {
...
const save = async () => {
    if (!item.value.waktupraktek) {
        useToaster().error('Waktu Praktek Harus di Isi')
        return
    }
    var objSave =
    {
        'jadwalpraktek': {
            ...
            'waktupraktek': item.value.waktupraktek,
            'statusenabled': item.value.statusenabled,
            'jammulai': item.value.jammulai,
            'jamselesai': item.value.jamselesai,
            'kdpraktek': item.value.kdpraktek,
        }
    }
    isLoadingTT.value = true
    await useApi().post(
        `/sysadmin/master-jadwal-praktek/save`, objSave).then((response: any) => {
...
const deleteItem = async (e: any) => {
    isLoading.value = true
    await useApi().post(
        `/sysadmin/master-jadwal-praktek/delete`, { 'id': e.id }).then((response: any) => {
            isLoading.value = false
            clear()
            fetchData()
        }, (error) => {
...
const edit = (e: any) => {
    ...
    item.value.jamselesai = e.jamselesai
...
const detail = (e: any) => {
    ...
    item.value.kdpraktek = e.kdpraktek
    modalDetail.value = true
}

const clear = () => {
    item.value.id = ''
    item.value.waktupraktek = ''
    item.value.jammulai = ''
    item.value.jamselesai = ''
    item.value.kdpraktek = ''
}
