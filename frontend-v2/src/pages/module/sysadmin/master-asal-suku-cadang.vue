<template>
    ...
    <VCard>
        <div class="columns column c-title">
            <h3 class="title is-5 mb-2 ml-2" style="z-index:1">Asal Suku Cadang</h3>
        </div>
        ...
        <div class="columns is-multiline">
            <div class="column is-8" style="margin-top: 2.9rem;">
                ...
                <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
                    <DataTable :value="dataSourcefiltered" :paginator="true" :rows="5"
                        ...
                        <Column field="asalsukucadang" header="Asal Suku Cadang" :sortable="true"></Column>
                        ...
                        <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
                            <div class="tile-grid-item">
                                <div class="tile-grid-item-inner">
                                    ...
                                    <div class="meta">
                                        <span class="dark-inverted mb-2">{{ item.asalsukucadang }}</span>
                                    </div>
            ...
            <div class="column is-4">
                <img src="/images/avatars/label/asal-suku-cadang.png" class="sukucad">
                <VCard>
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-12">
                            <VField label="Asal Suku Cadang">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.asalsukucadang" placeholder="Asal Suku Cadang"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
        ...
        <VModal :open="modalDetail" title="Detail Asal Suku Cadang" actions="right" @close="modalDetail = false,clear()">
            <template #content>
...
useHead({
  title: 'Asal Suku Cadang - ' + import.meta.env.VITE_PROJECT,
})
...
const dataSourcefiltered = computed(() => {
    ...
    return dataSource.value.filter((items: any) => {
        return (
            items.asalsukucadang.match(new RegExp(filters.value, 'i'))
        )
    })
})
...
const fetchData = async ()=>{
    ...
    await useApi().get(`/sysadmin/master-asal-suku-cadang?statusenabled=${item.value.aktif}`).then((response)=>{
...
const save = async ()=>{
    if (!item.value.asalsukucadang) {
        useToaster().error('Asal Suku Cadang Harus di Isi')
        return
    }
    var objSave =
    {
        'asalsukucadang': {
            ...
            'asalsukucadang': item.value.asalsukucadang,
    ...
    await useApi().post(
        `/sysadmin/master-asal-suku-cadang/save`, objSave).then((response: any) => {
...
const deleteItem = async (e: any) => {
    isLoading.value = true
    await useApi().post(
        `/sysadmin/master-asal-suku-cadang/delete`, { 'id': e.id }).then((response: any) => {
            isLoading.value = false
            fetchData()
        }, (error) => {
        })
...
const edit = (e: any)=>{
    item.value.id = e.id
    item.value.asalsukucadang = e.asalsukucadang
