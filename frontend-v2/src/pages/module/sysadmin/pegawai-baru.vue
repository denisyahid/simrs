                                            ...
                                            <VCard>
                                                ...
                                                <div class="columns is-multiline">
                                                    ...
                                                    <div class="column is-1">
                                                        <span><br></span>
                                                        <VButton icon="feather:refresh-cw" type="submit" color="primary" raised @click="syncHRIS()" class="is-rounded"
                                                            :loading="isHRIS">
                                                            HRIS
                                                        </VButton>
                                                    </div>
...
async function onInit() {
    item.loading = false
    if(ID_PEGAWAI){
        await useApi().get(`sysadmin/pegawai-by-id?id=${ID_PEGAWAI}`).then((response:any)=>{
            // item.loading = true
           item.id = response.pegawai.id 
           item.namalengkap = response.pegawai.namalengkap
           item.namapanggilan = response.pegawai.namapanggilan
           item.nik = response.pegawai.nik_intern
           item.nip = response.pegawai.nip
           item.tempatlahir = response.pegawai.tempatlahir
           item.tgllahir = response.pegawai.tgllahir
           item.objectpendidikanterakhirfk = response.pegawai.objectpendidikanterakhirfk
           item.objectstatusperkawinanpegawaifk = response.pegawai.objectstatusperkawinanpegawaifk
           item.gelardepan = response.pegawai.gelardepan
           item.gelarbelakang = response.pegawai.gelarbelakang
           item.objectjeniskelaminfk = response.pegawai.objectjeniskelaminfk
           item.objectagamafk = response.pegawai.objectagamafk
           item.npwp = response.pegawai.npwp
           item.alamat = response.pegawai.alamat
           item.kodepos = response.pegawai.kodepos
           item.notlp = response.pegawai.notlp
           item.email = response.pegawai.email
           item.bankrekeningnama = response.pegawai.bankrekeningnama
           item.bankrekeningnomor = response.pegawai.bankrekeningnomor
           item.bankrekeningatasnama = response.pegawai.bankrekeningatasnama
           item.objectunitkerjafk = response.pegawai.objectunitkerjapegawaifk
           item.objectstatuspegawaifk = response.pegawai.objectstatuspegawaifk
           item.tglmasuk = response.pegawai.tglmasuk
           item.tglkeluar = response.pegawai.tglkeluar
           item.pensiun = response.pegawai.pensiun
           item.tglpensiun = response.pegawai.tglpensiun
           item.kedudukanfk = response.pegawai.kedudukanfk
           item.objectgolonganpegawaifk = response.pegawai.objectgolonganpegawaifk
           item.objectjabatanfungsionalfk = response.pegawai.objectjabatanfungsionalfk
           item.objectkelompokjabatanfk = response.pegawai.objectkelompokjabatanfk
           item.objecteselonfk = response.pegawai.objecteselonfk
           item.objectjenispegawaifk = response.pegawai.objectjenispegawaifk
           item.objectshiftkerja = response.pegawai.objectshiftkerja
           item.fingerprintid = response.pegawai.idfinger
           item.nilaijabatan = response.pegawai.nilaijabatan
           item.grade = response.pegawai.grade
        })
    }
    loadDrop()
}
...
const syncHRIS = async()=>{
    if(item.nik == undefined){
        H.alert('error','NIK wajib diisi!')
        return
    }
    var objSearch ={
        data: null,
        method: 'GET',
        url:'bridgingSIMRS/findPegawaiByNoKTP?noKTP='+item.nik
    }
    isHRIS.value = true
    await useApi()
        .post('hris/sync-hristools', objSearch)
        .then(
            (response)=>{
                isHRIS.value = false
                item.namalengkap = response.data.pegawai.namaLengkap
                item.nip = response.data.pegawai.nikPegawai
                item.tempatlahir = response.data.pegawai.tempatLahir
                item.tgllahir = new Date(response.data.pegawai.tglLahir * 1000)
                item.npwp = response.data.pegawai.npwp
                item.alamat = response.data.pegawai.alamatSekarang
                item.kodepos = response.data.pegawai.kodePosSekarang
                item.notlp = response.data.pegawai.noTelp1.trim()
                item.notelp = response.data.pegawai.noTelp1.trim()
                item.email = response.data.pegawai.email
                item.bankrekeningnama = response.data.pegawai.namaBank
                item.bankrekeningnomor = response.data.pegawai.noRekening.trim()
                item.bankrekeningatasnama = response.data.pegawai.namaRekening
            },
            (error) => {
                isSimpan.value = false
            }
