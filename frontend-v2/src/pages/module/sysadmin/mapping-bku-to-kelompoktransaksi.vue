                ...
                <div class="form-outer">
                    <div :class="[isStuck && 'is-stuck']" class="form-header stuck-header">
                        <div class="form-header-inner">
                            <div class="left">
                                <h3>Mapping BKU to Kelompok Transaksi</h3>
                            </div>
                            <div class="right">
                                <div class="buttons">
                            ...
                            <div class="columns is-multiline">
                                <div class="column is-8">
                                    <VField class="cis-rounded-select_Z  is-autocomplete-select" v-slot="{ id }">
                                        <VLabel>Buku Kas Umum</VLabel>
                                        <VControl icon="feather:search">
                                            <Multiselect mode="single" v-model="item.bku" :options="d_Bku"
                                                placeholder="Pilih data" :searchable="true" :attrs="{ id }"
                                                @select="handleChangeProduk(item.bku)" />
                                        </VControl>
                                    </VField>
                                </div>
                                ...
                                <Divider align="center">
                                    <span class="p-tag">Kelompok Transaksi</span>
                                </Divider>
                                            ...
                                            <div class="form-section-inner has-padding-bottom h-700-o ">
                                                ...
                                                <div class="columns is-multiline mt-2">
                                                    <div class="column is-4" v-for="items in listChecked"
                                                        :key="items.kelompoktransaksi">
                                                        <VTag color="purple"  style="width: 100%; justify-content: start;" :label="items.kelompoktransaksi" />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                <VField>
                                                    <VControl raw subcontrol>
                                                        <VCheckbox v-model="checkboxProduk[items.id]" :value="items.id"
                                                            :label="items.kelompoktransaksi" color="info" square
...
const dataSourcefiltered = computed(() => {
    if (!filters.value) {
        return d_KelompokTransaksi.value
    }
    return d_KelompokTransaksi.value.filter((items: any) => {
        return (
...
async function listDropdown() {
    const response = await useApi().get(
        `/sysadmin/dd-bku`)
    d_Bku.value = response.bku.map((e: any) => { return { label: e.bku, value: e.id } })
    d_KelompokTransaksi.value = response.kelompoktransaksi
    d_ListDefault.value = response.kelompoktransaksi
    item.kelompoktransaksi = response.kelompoktransaksi[0].id
    ...
    if (ID_RUANGAN) {
        item.value.bku = ID_RUANGAN
        handleChangeProduk(ID_RUANGAN)
    }
}

async function produkByID(id: any) {
    const { data: detail } = await useApi().get(`/sysadmin/master-map-bku?id=${id}`)
    item.value.bku = detail[0].idbku
    item.value.arrProduk = detail[0].kelompoktransaksifk
}



async function saveMapRuanganToProduk() {
    if (!item.value.bku) { H.alert('warning', 'BKU harus di pilih'); return }
    if (jumlahCeklis.value == 0) { H.alert('warning', 'Kelompok Transaksi harus di pilih'); return }
    let arrProduk = []
    ...
    await useApi().post(
        `/sysadmin/save-map-bku`, json).then((response: any) => {
            ...
            for (var i = 0; i < d_ListDefault.value.length; i++) {
                ...
                if (element2.id == element) {
                    for (var z = 0; z < listChecked.value.length; z++) {
                        ...
                        if (element3.kelompoktransaksi == element2.kelompoktransaksi) {
                            listChecked.value.splice(z, 1)
                        }
                    }
                    listChecked.value.push({ kelompoktransaksi: element2.kelompoktransaksi })
                }
            }
            jumlah = jumlah + 1
...
function handleChangeProduk(e: any) {
    ...
    jumlahCeklis.value = 0
    useApi().get(
        `/sysadmin/master-map-bku?idbku=${e}`).then((response: any) => {
            isLoading.value = false
            jumlahCeklis.value = response.data.length
            for (let x = 0; x < response.data.length; x++) {
                const element = response.data[x];
                checkboxProduk.value[element.kelompoktransaksifk] = true
            }
            clickProduk()
        }).catch((e: any) => {
            isLoading.value = false
