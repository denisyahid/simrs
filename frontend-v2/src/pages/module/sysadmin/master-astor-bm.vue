  ...
  <VCard>
    <TabView v-model:activeIndex="activeValue">
      ...
      <TabPanel header="Master Generik">
        <MappingGenerik v-if="activeValue == 1"></MappingGenerik>
      </TabPanel>
      <TabPanel header="Master Divisi">
        <MasterDivisi v-if="activeValue == 2"></MasterDivisi>
      </TabPanel>
      <TabPanel header="Master Operasi Profilaksis">
        <MasterOperasi v-if="activeValue == 3"></MasterOperasi>
      </TabPanel>
      <TabPanel header="Master Data Astor BM Profilaksis">
        <MasterProfilaksis v-if="activeValue == 4"></MasterProfilaksis>
      </TabPanel>
      <TabPanel header="Master Tindakan Empiris">
        <MasterTindakan v-if="activeValue == 5"></MasterTindakan>
      </TabPanel>
      <TabPanel header="Master Data Astor BM Empiris">
        <MasterEmpiris v-if="activeValue == 6"></MasterEmpiris>
      </TabPanel>
    </TabView>
...
import MasterOperasi from './master-operasi-profilaksis.vue'
import MasterTindakan from './master-tindakan-empiris.vue'
import MasterProfilaksis from './master-profilaksis.vue'
import MasterEmpiris from './master-empiris.vue'
import MasterDivisi from './master-ppra-divisi.vue'
...
function listDropdown() {
  ...
  useApi().get(`/sysadmin/master-produk-dropdown`).then((response: any) => {
    ...
    const defaultOption = DetailProduk.value.find(item => item.value === 2546)
    {
      if(defaultOption)
      {
          item.detailjenisproduk = defaultOption.value 
      }
    }
  })
}
