    ...
    <VCard>
        ...
        <div class="columns is-multiline" style="margin-top:1rem">
            <div class="column is-4 ">
                <img src="/images/avatars/label/jenis-kondisi-pasien.png" alt="Jenis Kondisi Pasien" class="mix-2" />
                <VCard>
                    ...
                    <DataTable :value="dataSourcefiltered" class="p-datatable-sm" :loading="isLoadingDelete"
                        ...
                        <Column :exportable="false" header="Action" style="text-align: center;">
                            <template #body="slotProps">
                                ...
                                <VIconButton type="button" v-else icon="fas fa-trash" disabled class="mr-3" color="danger"
        ...
        <VModal :open="modalDetail" title="Detail Jenis Kondisi Pasien" actions="right"
...
useHead({
    title: 'Transmedic - Jenis Kondisi Pasien',
})
...
const dataSourcefiltered = computed(() => {
    ...
    return dataSource.value.filter((items: any) => {
        return (
            items.jeniskondisipasien.match(new RegExp(filters.value, 'i'))
        )
    })
})
...
const fetchData = async () => {
    ...
    await useApi().get(`/sysadmin/master-jenis-kondisi-pasien?${statusEnabled}`).then((response: any) => {
...
const save = async () => {
    if (!item.value.jeniskondisipasien) {
    ...
    var objSave =
    {
        'jeniskondisipasien': {
            ...
            'jeniskondisipasien': item.value.jeniskondisipasien,
    ...
    await useApi().post(`/sysadmin/master-jenis-kondisi-pasien/save`, objSave).then((response: any) => {
...
const deleteItem = async (e: any) => {
    ...
    await useApi().post(`/sysadmin/master-jenis-kondisi-pasien/delete`, { 'id': e.id }).then((response: any) => {
        clear()
        fetchData()
