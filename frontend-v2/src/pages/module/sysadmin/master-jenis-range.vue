  ...
  <VCard>
    <div class="columns is-multiline" style="margin-top:1rem">
      <div class="column is-4 ">
        <img src="/images/avatars/label/validasi.png" class="mix-2" alt="Jenis Range" />
        <VCard>
            ...
            <div class="column is-12">
              <VField label="Jenis Range">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.jenisrange" placeholder="Jenis Range" class="is-rounded" />
                </VControl>
              </VField>
            </div>
      ...
      <div class="column is-8">
        ...
        <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
          <DataTable :value="dataSourcefiltered" class="p-datatable-sm" :loading="isLoading" :paginator="true" :rows="5"
            ...
            <Column field="jenisrange" header="Jenis Range" :sortable="true"></Column>
            ...
            <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
              <div class="tile-grid-item">
                <div class="tile-grid-item-inner" @click="edit(item)">
                  <VAvatar size="medium" picture="/images/avatars/icon/ic-jenis-generik.png" color="primary" squared
                    bordered />
                  <div class="meta">
                    <span class="dark-inverted">{{ item.jenisrange }}</span>
                  </div>
    ...
    <VModal :open="modalDetail" title="Detail Jenis Range" actions="right" @close="modalDetail = false, clear()">
      <template #content>
...
useHead({ title: 'Transmedic - Jenis Generik' })
...
const dataSourcefiltered = computed(() => {
  ...
  return dataSource.value.filter((items: any) => {
    return (
      items.jenisrange.match(new RegExp(filters.value, 'i'))
    )
  })
})
...
const fetchData = async () => {
  ...
  await useApi().get(`/sysadmin/master-jenis-range?statusenabled=${item.value.aktif}`).then((response) => {
...
const save = async () => {
  if (!item.value.jenisrange) {
    useToaster().error('Jenis Range harus di isi')
    return
  }
  var objSave =
  {
    'jenisrange': {
      ...
      'jenisrange': item.value.jenisrange,
  ...
  await useApi().post(
    `/sysadmin/master-jenis-range/save`, objSave).then((response: any) => {
...
const deleteItem = async (e: any) => {
  isLoading.value = true
  await useApi().post(
    `/sysadmin/master-jenis-range/delete`, { 'id': e.id }).then((response: any) => {
      clear()
      fetchData()
      isLoading.value = false
    }, (error) => {
