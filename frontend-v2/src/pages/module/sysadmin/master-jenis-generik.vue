  ...
  <VCard>
    ...
    <div class="columns is-multiline" style="margin-top:1rem">
      <div class="column is-4 ">
        <img src="/images/avatars/label/jenis-generik.png" class="mix-2" alt="Jenis Generik" />
        <VCard>
              ...
              <div class="tile-grid-item">
                <div class="tile-grid-item-inner" @click="edit(item)">
                  <VAvatar size="medium" picture="/images/avatars/icon/ic-jenis-generik.png" color="primary" squared
                    bordered />
                  <div class="meta">
                    <span class="dark-inverted">{{ item.jenisgenerik }}</span>
                  </div>
    ...
    <VModal :open="modalDetail" title="Detail Jenis Generik" actions="right" @close="modalDetail = false, clear()">
      <template #content>
        <form class="modal-form">
          <div class="columns is-multiline">
            <div class="column is-12">
              <VField label="Jenis Generik">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.jenisgenerik" placeholder="Jenis Generik" class="is-rounded" readonly
...
const dataSourcefiltered = computed(() => {
  ...
  return dataSource.value.filter((items: any) => {
    return (
      items.jenisgenerik.match(new RegExp(filters.value, 'i'))
    )
  })
})
...
const fetchData = async () => {
  ...
  await useApi().get(`/sysadmin/master-jenis-generik?statusenabled=${item.value.aktif}`).then((response) => {
...
const save = async () => {
  if (!item.value.jenisgenerik) {
    useToaster().error('Jenis Generik harus di isi')
    return
  }
  var objSave =
  {
    'jenisgenerik': {
      ...
      'jenisgenerik': item.value.jenisgenerik,
  ...
  await useApi().post(
    `/sysadmin/master-jenis-generik/save`, objSave).then((response: any) => {
...
const deleteItem = async (e: any) => {
  isLoading.value = true
  await useApi().post(
    `/sysadmin/master-jenis-generik/delete`, { 'id': e.id }).then((response: any) => {
      clear()
      fetchData()
      isLoading.value = false
    }, (error) => {
