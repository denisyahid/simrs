<template>
    ...
    <VCard>
        <div class="columns column c-title">
            <h3 class="title is-5 mb-2 ml-2" style="z-index:1">Unit Kerja Pegawai</h3>
        </div>
                ...
                <VCard>
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-12">
                            <VField label="Unit Kerja Pegawai">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.name" class="is-rounded"
            ...
            <div class="column is-8 mt-3">
                ...
                <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
                    <DataTable :value="dataSourcefiltered" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
                        ...
                        <Column field="name" header="Unit Kerja Pegawai" :sortable="true"></Column>
...
useHead({
    title: 'Unit Kerja Pegawai - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
    ...
    await useApi().get(`/sysadmin/master-unit-kerja-pegawai?statusenabled=${isAktif.value}`).then((response: any) => {
...
const save = async (item: any) => {
    if (!item.name) {
        useToaster().error('Unit Kerja Pegawai harus di isi')
        return
    }
    ...
    await useApi().post(
        `/sysadmin/master-unit-kerja-pegawai/save`, objSave).then((response: any) => {
...
const deleterow = async (e: any) => {
    isLoading.value = true
    await useApi().post(
        `/sysadmin/master-unit-kerja-pegawai/delete`, { 'id': e.id }).then((response: any) => {
            isLoading.value = false
            clear()
            fetchData()
        }, (error) => {
