<template>
    ...
    <VCard>
        <div class="columns column c-title">
            <h3 class="title is-5 mb-2 mr-1">Sediaan</h3>
        </div>
        <div class="columns is-multiline">
            <div class="column is-8">
                ...
                <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
                    <DataTable :value="dataSourcefiltered" :paginator="true" :rows="5" :rowsPerPageOptions="[5, 10, 25]"
                        ...
                        <Column field="name" header="Sediaan" :sortable="true"></Column>
                        ...
                        <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
                            <div class="tile-grid-item">
                                <div class="tile-grid-item-inner"  @click="edit(item)">
                                    <VAvatar size="medium" picture="/images/avatars/icon/ic-name.png"
                                        color="primary" squared bordered />

                                    <div class="meta">
                                        <span class="dark-inverted mb-2">{{ item.name }}</span>
            ...
            <div class="column is-4">
                <img src="/images/avatars/label/hand.png" class="Sediaan">
                <VCard>
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-12">
                            <VField label="Sediaan">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.name" placeholder="Sediaan"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
        ...
        <VModal :open="modalDetail" title="Detail Sediaan" actions="right"
...
useHead({
  title: 'Sediaan - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async ()=>{
    ...
    await useApi().get(`/sysadmin/master-sediaan?statusenabled=${item.value.aktif}`).then((response)=>{
...
const save = async ()=>{
    if (!item.value.name) {
        useToaster().error('Sediaan harus di isi')
        return
    }
    var objSave =
    {
    ...
    await useApi().post(
        `/sysadmin/master-sediaan/save`, objSave).then((response: any) => {
...
const deleteItem = async (e: any)=>{
    isLoading.value = true
    await useApi().post(
        `/sysadmin/master-sediaan/delete`, { 'id': e.id }).then((response: any) => {
            isLoading.value = false
            fetchData()
        }, (error) => {
        })
