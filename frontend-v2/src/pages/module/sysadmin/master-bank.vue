<template>
    ...
    <VCard>
        <div class="columns column c-title">
            <h3 class="title is-5 mb-2 ml-2" style="z-index:1">Bank Account</h3>
        </div>
                ...
                <VCard>
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-12">
                            <VField label="Nama Bank">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.bankaccountnama" placeholder="Nama Bank"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-12">
                            <VField label="Nomor Akun">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.bankaccountnomor" placeholder="Nomor Akun"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-12">
                            <VField label="Keterangan">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.keteranganlainnya" placeholder="Keterangan"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-12" v-if="item.id" style="display:flex; padding: 0px;">
                            <VField class="column is-8 is-rounded-select is-autocomplete-select">
                                ...
                                <VControl icon="feather:search">
                                    <Multiselect mode="single" v-model="item.kdrekananfk"
            ...
            <div class="column is-8 mt-3">
                ...
                <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
                    <DataTable :value="dataSourcefiltered" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
                        ...
                        <Column field="bankaccountnama" header="Nama Bank" :sortable="true"></Column>
                        <Column field="bankaccountnomor" header="Nomor Akun" :sortable="true"></Column>
                        <Column field="namarekanan" header="Rekanan" :sortable="true"></Column>
                        <Column field="keteranganlainnya" header="Keterangan" :sortable="true"></Column>
...
useHead({
    title: 'Bank Account - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
    ...
    await useApi().get(`/sysadmin/master-akun-bank?statusenabled=${isAktif.value}`).then((response: any) => {
...
const save = async (item: any) => {
    if (!item.bankaccountnama) {
        useToaster().error('Nama Bank harus di isi')
        return
    }
    ...
    let objSave =
    {
        'bankaccountnama': {
            ...
            'bankaccountnama': item.value.bankaccountnama,
            'bankaccountnomor': item.value.bankaccountnomor,
            'keteranganlainnya': item.value.keteranganlainnya ? item.value.keteranganlainnya : '',
            'statusenabled': item.value.statusenabled,
            'kdrekananfk': item.value.kdrekananfk ? item.value.kdrekananfk : '',
        }
    }

    isLoadingButton.value = true
    await useApi().post(
        `/sysadmin/save-bank-akun`, objSave).then((response: any) => {
...
const deleterow = async (e: any) => {
    isLoading.value = true
    await useApi().post(
        `/sysadmin/delete-bank-akun`, { 'id': e.id }).then((response: any) => {
            isLoading.value = false
            clear()
            fetchData()
        }, (error) => {
...
const loadDropdown = () => {
    useApi().get(`/sysadmin/load-rekanan`).then((response: any) => {
...
const edit = (e: any) => {
    item.value.id = e.id
    item.value.bankaccountnama = e.bankaccountnama
    item.value.bankaccountnomor = e.bankaccountnomor
    item.value.keteranganlainnya = e.keteranganlainnya
    item.value.statusenabled = e.statusenabled
    item.value.kdrekananfk = e.kdrekananfk
}
...
const detail = (e: any) => {
    ...
    item.value.keteranganlainnya = e.keteranganlainnya
    item.value.namarekanan = e.namarekanan
