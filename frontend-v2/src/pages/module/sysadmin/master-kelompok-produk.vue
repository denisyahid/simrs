<template>
  <VCard radius="small">
    <div class="columns column">
      <h3 class="title is-5 mb-2 mr-1">Kelompok Produk</h3>
    </div>
        ...
        <VCard>
          <div class="columns is-multiline">
            ...
            <div class="column is-12" v-if="item.id" style="display:flex;padding: 0px;">
              ...
              <VField class="column is-4">
                ...
                <VControl class="is-pulled-right">
                  <VSwitchBlock style="padding:0px;margin-top:5px" v-model="item.statusenabled" label="Aktif"
                    color="danger" @change="changeSwitch()" />
                </VControl>
              </VField>
            ...
            <div class="column is-12" style="padding-top:0px">
              <VField class="is-rounded-select is-autocomplete-select">
                <VLabel> Departemen Produk</VLabel>
                <VControl icon="feather:search">
            ...
            <div class="column is-6">
              <VField label="Jumlah Persediaan">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.ishavingstok" placeholder="Persediaan" class="is-rounded" />
                </VControl>
              </VField>
            </div>

            <div class="column is-6">
              <VField label="Jumlah Harga">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.ishavingprice" placeholder="Harga" class="is-rounded" />
                </VControl>
              </VField>
            </div>
      ...
      <div class="column is-8">
        ...
        <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
          <DataTable :value="dataSourcefiltered" :paginator="true" :rows="5" :rowsPerPageOptions="[5, 10, 25]"
            ...
            <Column field="status" header="Status" :sortable="true" style="text-align:center"></Column>
    ...
    <VModal :open="modalDetail" title="Detail Kelompok Produk" actions="right" @close="modalDetail = false,clear()">
      <template #content>
        <form class="modal-form">
          <div class="columns is-multiline">
            ...
            <div class="column is-8">
              <VField label="Jenis Transaksi">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.jenistransaksi" placeholder="Jenis Transaksi" class="is-rounded" />
                </VControl>
              </VField>
            </div>
            <div class="column is-4">
              <VField label="Kode External">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.kodeexternal" placeholder="Kode Kelompok Produk" class="is-rounded" />
                </VControl>
              </VField>
            </div>
            <div class="column is-4">
              <VField label="Jumlah Barang Tersedia">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.ishavingstok" placeholder="Barang yang tersedia" class="is-rounded" />
                </VControl>
              </VField>
            </div>
            <div class="column is-4">
              <VField label="Harga Tersedia">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.ishavingprice" placeholder="Harga yang tersedia" class="is-rounded" />
                </VControl>
              </VField>
            </div>
            <div class="column is-4">
              <VField label="Status Aktivasi">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.status" class="is-rounded" />
                </VControl>
              </VField>
            </div>
          </div>
...
useHead({
  title: 'Transmedic - Kelompok Produk',
})
const item: any = ref({
  aktif: true
})
let d_Departemen = ref([])
let d_JenisTransaksi = ref([])
...
const dataSourcefiltered = computed(() => {
  ...
  return dataSource.value.filter((items: any) => {
    return (
      items.kelompokproduk.match(new RegExp(filters.value, 'i'))
    )
  })
})
...
async function fetchData() {
  ...
  if (item.namadepartemen) namaDepartemen = '&namadepartemen=' + item.namadepartemen
  if (item.jenistransaksi) jenisTransaksi = '&jenistransaksi=' + item.jenistransaksi

  if (item.id) kdkelompokproduk = '&kdkelompokproduk=' + item.kdkelompokproduk

  const response = await useApi().get(
    '/sysadmin/master-kelompok-produk?offset=' + offset +
    '&limit=' + limit +
    '&rows=' + rows +
    kdkelompokproduk + kelompokproduk + statusEnabled + namaDepartemen + jenisTransaksi
  )
  isLoading = false
...
async function loadDropdown() {
  const response = await useApi().get(`/sysadmin/master-kelompok-produk-dropdown`)
  d_JenisTransaksi.value = response.jenistransaksi.map((e: any) => {
    return { label: e.jenistransaksi, value: e.id }
  })
...
function edit(e: any) {
  ...
  item.value.ishavingprice = e.ishavingprice
  item.value.ishavingstok = e.ishavingstok
...
function detail(e: any) {
  ...
  item.value.jenistransaksi = e.jenistransaksi
...
async function save() {
  ...
  var objSave =
  {
    'kelompokproduk': {
      ...
      'kelompokproduk': item.value.kelompokproduk,
      ...
      'ishavingstok': item.value.ishavingstok ? item.value.ishavingstok : null,
      'ishavingprice': item.value.ishavingprice ? item.value.ishavingprice : null,
    }
  }
  isLoadingTT.value = true
  await useApi().post(
    `/sysadmin/save-master-kelompok-produk`, objSave).then((response: any) => {
...
async function deleterow(e: any) {
  await useApi().post(
    `/sysadmin/delete-kelompok-produk`, { 'id': e.id }).then((response: any) => {
      fetchData()
    }, (error) => {
    })
}
...
function clear() {
  ...
  item.value.kelompokproduk = ''
  item.value.ishavingstok = ''
  item.value.ishavingprice = ''
