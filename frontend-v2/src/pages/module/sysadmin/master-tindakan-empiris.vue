<template>
  <VCard radius="small">
    <div class="columns column">
      <h3 class="title is-5 mb-2 mr-1"> Master Tindakan Empiris</h3>
    </div>
      ...
      <div class="column is-8">
        ...
        <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
          <DataTable :value="dataSourcefiltered" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
            ...
            <Column field="idpt" header="Kode" :style="{ width: '90px' }" :hidden="true"></Column>
    ...
    <VModal :open="modalInput" title="Form Master Tindakan" actions="right" @close="modalInput = false">
      <template #content>
        <form class="modal-form">
          <div class="columns is-multiline">
            <div class="column is-12">
              <VField label="Nama Tindakan">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.namatindakan" placeholder="Masukkan Nama Tindakan"
                    class="is-rounded" />
                </VControl>
              </VField>
            </div>
...
async function fetchData() {
  ...
  const response = await useApi().get(
    '/sysadmin/master-tindakan-empiris?offset=' + offset +
    '&limit=' + limit +
    '&rows=' + rows +
...
function loadDropdown() {
  d_divisi.value = []
  useApi().get(
    `/sysadmin/master-detail-divisi?statusantibiotik=2`).then((response: any) => {
...
function loadData() {
  dataSource.value = []
  useApi().get(
    `/sysadmin/master-tindakan-empiris?statusenabled=${item.value.aktif}`).then((response: any) => {
      dataSource.value = response.data
    })
}
...
async function save() {
  ...
  if (!item.value.namatindakan) {
    useToaster().error('Nama Tindakan Harus Diisi')
    return
  }
  ...
  var objSave =
  {
    'datasave': {
      ...
      'namatindakan': item.value.namatindakan,
      'divisi': item.value.objectdivisifk,
    }
  }
  isLoadingTT.value = true
  await useApi().post(
    `/sysadmin/save-master-tindakan-empiris`, objSave).then((response: any) => {
...
async function deleterow(e: any) {
  await useApi().post(
    `/sysadmin/delete-master-tindakan-empiris`, { 'id': e.idpt }).then((response: any) => {
      loadData()
    }, (error) => {
    })
}
