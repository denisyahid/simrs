<template>
  ...
  <VCard>
    <div class="columns column c-title">
      <h3 class="title is-5 mb-2 mr-1">Hubungan Keluarga</h3>
    </div>

    <div class="columns is-multiline" style="margin-top:1rem">
      <div class="column is-4 ">
        <img src="/images/avatars/label/keluarga.png" class="mix-2" />
        <VCard>
          <div class="columns is-multiline">
            ...
            <div class="column is-12">
              <VField label="Hubungan">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.hubungankeluarga" placeholder="Hubungan Keluarga"
                    class="is-rounded" />
                </VControl>
              </VField>
            </div>
            <div class="column is-12" v-if="item.id">
              <VField label="Aktivasi" style="display:flex;align-items:center">
                <VControl class="is-pulled-right" style="margin-left:-6.5rem">
                  <VSwitchBlock v-model="item.statusenabled" label="Aktif" color="danger" @change="changeSwitch(item.statusenabled)" />
                </VControl>
              </VField>
            </div>
      ...
      <div class="column is-8">
        ...
        <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
          <DataTable :value="dataSourcefiltered" class="p-datatable-sm" :loading="isLoading" :paginator="true"
            ...
            <Column field="hubungankeluarga" header="Hubungan Keluarga" :sortable="true"></Column>
            ...
            <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
              <div class="tile-grid-item">
                <div class="tile-grid-item-inner">
                  <VAvatar size="medium" picture="/images/avatars/icon/ic-family.png" color="primary" squared bordered />
                  <div class="meta">
                    <span class="dark-inverted">{{ item.hubungankeluarga }}</span>
                  </div>
    ...
    <VModal :open="modalDetail" title="Detail Hubungan Keluarga" actions="right"
      @close="modalDetail = false, clear()">
      <template #content>
        <form class="modal-form">
          <div class="columns is-multiline">
            <div class="column is-12">
              <VField label="Hubungan Keluarga">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.hubungankeluarga" placeholder="Jenis Kondisi Pasien"
...
useHead({
  title: 'Hubungan Keluarga - ' + import.meta.env.VITE_PROJECT,
})
...
const dataSourcefiltered = computed(() => {
  ...
  return dataSource.value.filter((items: any) => {
    return (
      items.hubungankeluarga.match(new RegExp(filters.value, 'i'))
    )
  })
})
...
const fetchData = async ()=>{
  ...
  await useApi().get(`/sysadmin/master-hubungan-keluarga?statusenabled=${item.value.aktif}`).then((response:any)=>{
...
const save = async()=>{
  if (!item.value.hubungankeluarga) {
    useToaster().error('Jenis Kondisi Pasien harus di isi')
    return
  }
  var objSave =
  {
  ...
  await useApi().post(
    `/sysadmin/master-hubungan-keluarga/save`, objSave).then((response: any) => {
...
const deleteItem = async (e: any)=>{
  isLoading.value = true
  await useApi().post(
    `/sysadmin/master-hubungan-keluarga/delete`, { 'id': e.id }).then((response: any) => {
      clear()
      fetchData()
      isLoading.value = false
    }, (error) => {
...
const edit = (e: any)=>{
  item.value.id = e.id
  item.value.kdhubungankeluarga = e.kdhubungankeluarga
  item.value.hubungankeluarga = e.hubungankeluarga
