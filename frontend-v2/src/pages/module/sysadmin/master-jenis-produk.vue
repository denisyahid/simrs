<template>
  <VCard radius="small">
    <div class="columns column">
      <h3 class="title is-5 mb-2 mr-1">Data Jenis Produk</h3>
    </div>
              ...
              <div class="tile-grid-item">
                <div class="tile-grid-item-inner" @click="edit(item)">
                  <VAvatar size="medium" picture="/images/avatars/svg/jenisproduk.svg" color="primary" squared
                    bordered />
                  <div class="meta">
                    <span class="dark-inverted">Jenis Produk : {{ item.jenisproduk }}</span>
        ...
        <div class="columns is-multiline">
          ...
          <div class="column is-12">
            <VField class="is-autocomplete-select">
              ...
              <VControl icon="feather:search">
                <Multiselect mode="single" v-model="item.kelompokproduk" :options="d_KelompokProduk"
    ...
    <VModal :open="modalInput" title="Tambah Jenis Produk" actions="right" @close="modalInput = false">
      <template #content>
...
async function fetchData() {
  ...
  if (item.value.kelompokproduk) KelompokProduk = '&kelompokproduk=' + item.value.kelompokproduk
  if (item.value.namadepartemen) Departemen = '&namadepartemen=' + item.namadepartemen
  dataSource.value = []
  const response = await useApi().get(
    '/sysadmin/master-jenis-produk?offset=' + offset +
    '&limit=' + limit +
    '&rows=' + rows +
    JenisProduk + KelompokProduk + Departemen
  )
  isLoading.value = false
...
function loadDropdown() {
  ...
  useApi().get(
    `/sysadmin/master-jenis-produk-dropdown`).then((response: any) => {
...
function edit(e: any) {
  item.value.id = e.id
  item.value.jenisproduk = e.jenisproduk
...
async function save() {
  if (!item.value.jenisproduk) {
    useToaster().error('Jenis Produk harus di isi')
    return
  }
  var objSave =
  {
    'jenisproduk': {
      ...
      'jenisproduk': item.value.jenisproduk,
  ...
  await useApi().post(
    `/sysadmin/save-master-jenis-produk`, objSave).then((response: any) => {
...
async function deleterow(e: any) {
  await useApi().post(
    `/sysadmin/delete-jenis-produk`, { 'id': e.id }).then((response: any) => {
      fetchData()
    }, (error) => {
    })
}
