<template>
  ...
  <div class="columns column">
    <h3 class="title is-5 mb-2 mr-1">Pekerjaan</h3>
  </div>
    ...
    <div class="column is-8">
      ...
      <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
        <DataTable :value="dataSourcefiltered" class="p-datatable-sm" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
          ...
          <Column field="pekerjaan" header="Pekerjaan" :sortable="true"></Column>
          ...
          <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
            <div class="tile-grid-item">
              <div class="tile-grid-item-inner">
                <VAvatar size="medium" picture="/images/avatars/svg/jabatan.svg" color="primary"/>
                <div class="meta">
                  <span class="dark-inverted">{{ item.pekerjaan }}</span>
                  <VTag :color="item.status_c" :label="item.status" style="margin-top: 0.5rem;" />
                </div>
            ...
            <div class="column is-12">
              <VField label="Pekerjaan">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.pekerjaan" placeholder="Pekerjaan" class="is-rounded" />
                </VControl>
              </VField>
            </div>
...
useHead({
  title: 'Pekerjaan - ' + import.meta.env.VITE_PROJECT,
})
...
const dataSourcefiltered = computed(() => {
  ...
  return dataSource.value.filter((items: any) => {
    return (
      items.pekerjaan.match(new RegExp(filters.value, 'i'))
    )
  })
})
...
async function fetchData() {
  ...
  if (item.pekerjaan) Pekerjaan = '&pekerjaan' + item.pekerjaan
  if (item.id) kdPekerjaan = '&kdpekerjaan=' + item.kdpekerjaan
  item.value.aktif ? StatusEnabled = '&statusenabled=' + item.value.aktif : StatusEnabled = '&statusenabled=false'

  const response = await useApi().get(
    '/sysadmin/master-pekerjaan?offset=' + offset +
    '&limit=' + limit +
    '&rows=' + rows +
    kdPekerjaan + Pekerjaan + StatusEnabled
  )
  isLoading = false
...
function edit(e: any) {
  item.value.id = e.id
  item.value.pekerjaan = e.pekerjaan
...
async function save() {
  if (!item.value.pekerjaan) {
    useToaster().error('Pekerjaan harus di isi')
    return
  }
  var objSave =
  {
    'pekerjaan': {
      ...
      'pekerjaan': item.value.pekerjaan,
      'statusenabled': item.value.statusenabled ? item.value.statusenabled : null,
    }
  }
  isLoadingTT.value = true
  await useApi().post(
    `/sysadmin/save-pekerjaan`, objSave).then((response: any) => {
...
async function deleterow(e: any) {
  await useApi().post(
    `/sysadmin/delete-pekerjaan`, { 'id': e.id }).then((response: any) => {
      fetchData ()
    }, (error) => {
    })
}
