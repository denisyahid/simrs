                            ...
                            <div class="columns is-multiline">
                                <div class="column is-8">
                                    <VField class="cis-rounded-select_Z  is-autocomplete-select" v-slot="{ id }">
                                        <VLabel>Nama Depo</VLabel>
                                        <VControl icon="feather:search">
                                            <Multiselect mode="single" v-model="item.namadepo" :options="d_Depo"
                                                placeholder="Pilih data" :searchable="true" :attrs="{ id }"
                                                @select="handleChangeRuangan(item.namadepo)" />
                                        </VControl>
                                    </VField>
                                </div>
...
async function listDropdown() {
    const response = await useApi().get(`/sysadmin/master-map-depo-to-ruangan-dropdown`)
    d_Depo.value = response.depo.map((e: any) => { return { label: e.namaruangan, value: e.id} })
...
async function saveMapLoginUserToRuangan() {
    let depofk
    d_Depo.value.forEach(element => {
        if(element.value == item.value.namadepo){
            depofk = element.departemendepo
        }
    });
    if (!item.value.namadepo) { H.alert('warning', 'Nama User harus di pilih'); return }
    ...
    let json = {
        'objectdepofk': item.value.namadepo,
        'detail': arrRuangan

    }
    isLoading.value = true
    await useApi().post(
        `/sysadmin/save-map-depo-to-ruangan`, json).then((response: any) => {
...
function handleChangeRuangan(e: any) {
    ...
    jumlahCeklis.value = 0
    useApi().get(`/sysadmin/master-map-depo-to-ruangan?objectdepofk=${e}`).then((response: any) => {
            isLoading.value = false
