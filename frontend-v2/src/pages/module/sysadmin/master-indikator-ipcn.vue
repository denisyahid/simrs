<template>
  <VCard>
    <div class="columns column">
      <h3 class="title is-5 mb-2 mr-1">Indikator IPCN</h3>
    </div>
      ...
      <div class="column is-8">
        ...
        <div class="tile-grid tile-grid-v1">
          <transitionGroup name="list" tag="div" class="columns is-multiline" v-if="isLoading">
            <div v-for="key in 2" :key="key" class="column is-6">
          ...
          <TransitionGroup name="list" tag="div" class="columns is-multiline" v-else>
            <div v-for="(item, key) in dataSource" :key="key" class="column is-12">
              <div v-if="rowGroupMetadata[item.kelompok] && rowGroupMetadata[item.kelompok].index === key">
                <span style="font-weight: bold; font-size: 16px; font-family: var(--font-alt);">
                  {{ item.kelompok }}
                </span>
                <Badge :value="rowGroupMetadata[item.kelompok].size" v-if="rowGroupMetadata[item.kelompok].size > 0"
                  class="ml-2 mt-2-mi" />
              </div>
              <div class="tile-grid-item">
                <div class="tile-grid-item-inner">
                  ...
                  <div class="meta">
                    <span class="dark-inverted">{{ item.indikatoripcn }}</span>
        ...
        <VCard style="margin-top : 3rem;">
          <div class="columns is-multiline">
            ...
            <div class="column is-12">
              <VField label="Kelompok" class="is-rounded-select  is-autocomplete-select" v-slot="{ id }">
                <VControl class="prime-auto-select">
                  <AutoComplete v-model="item.kelompokfk" :suggestions="d_Kelompok" :optionLabel="'label'"
                    @complete="fetchKelompok($event)" :dropdown="true" :minLength="3" :appendTo="'body'"
                    :loadingIcon="'pi pi-spinner'" :field="'label'" placeholder="Kelompok..." />
                </VControl>
              </VField>
            </div>
            <div class="column is-12">
              <VField label="Indikator">
                <VControl>
                  <VTextarea rows="3" type="text" v-model="item.indikator" placeholder="Kode Kelompok IPCN..." />
                </VControl>
              </VField>
            </div>
            <div class="column is-12">
              <VField label="Departement" class="is-rounded-select  is-autocomplete-select" v-slot="{ id }">
                <VControl class="prime-auto-select">
...
useHead({
  title: 'Indikator IPCN - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
  ...
  const response = await useApi().get(
    `/sysadmin/indikator-ipcn?limit=${limit}&offset=${offset}&search=${search}&status=${status}`
  )
...
const edit = async (data: any) => {
  ...
  item.indikator = data.indikatoripcn
  item.kelompokfk = { value: data.kelompokipcnfk, label: data.kelompokipcn }
  item.departemenfk = { value: data.departemenfk, label: data.namadepartemen }
...
const save = async () => {
  ...
  if (!item.kelompokfk) {
    H.alert('warning', 'Kelompok Harus Diisi !');
    return;
  }
  ...
  let json = {
    ...
    'kelompokfk': item.kelompokfk ? item.kelompokfk.value : '',
    'kelompok': item.Kelompok,
    ...(item.id && { 'statusenabled': item.statusenabled })
  }
  await useApi().post(`/sysadmin/save-indikator-ipcn`, json).then((response: any) => {
    fetchData()
    clear()
  })
...
const deleterow = async (data: any) => {
  ...
  useApi().post(`/sysadmin/delete-indkator-ipcn`, json).then((response: any) => {
    fetchData()
  })
}
...
const fetchKelompok = async (filter: any) => {

  await useApi().get(`emr/dropdown/kelompokipcn_m?select=id,kelompokipcn&param_search=kelompokipcn&query=${filter.query}&limit=10`
  ).then((response) => {
    d_Kelompok.value = response
  })
}
...
const updateRowGroupMetaData = () => {
  ...
  if (dataSource.value) {
    for (let i = 0; i < dataSource.value.length; i++) {
      let rowData = dataSource.value[i];
      let kelompok = rowData.kelompok;

      if (i == 0) {
        rowGroupMetadata.value[kelompok] = { index: 0, size: 1 };
      }
      else {
        let previousRowData = dataSource.value[i - 1];
        let previousRowGroup = previousRowData.kelompok;
        if (kelompok === previousRowGroup)
          rowGroupMetadata.value[kelompok].size++;
        else
          rowGroupMetadata.value[kelompok] = { index: i, size: 1 };
      }
    }
  }
  console.log(JSON.stringify(rowGroupMetadata.value));
