<template>
  <VCard radius="small">
    <div class="columns column">
      <h3 class="title is-5 mb-2 mr-1"> Master Operasi Profilaksis</h3>
    </div>
      ...
      <div class="column is-8">
        ...
        <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
          <DataTable :value="dataSourcefiltered" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
            ...
            <Column field="idpjo" header="Kode" :style="{ width: '90px' }" :hidden="true"></Column>
            <Column field="idpd" header="Kode" :style="{ width: '90px' }" :hidden="true"></Column>
        ...
        <div class="columns is-multiline">
          ...
          <div class="column is-12" style="margin-top: 12px;">
            <VField label="Nama Divisi">
              <VControl icon="feather:search">
                <input v-model="filters2" class="input custom-text-filter" placeholder="Cari Data..." />
              </VControl>
            </VField>
          </div>
    ...
    <VModal :open="modalInput" title="Form Master Operasi" actions="right" @close="modalInput = false">
      <template #content>
          ...
          <div class="columns is-multiline">
            <div class="column is-12">
              <VField label="Nama Operasi">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.namaoperasi" placeholder="Masukkan Nama Operasi"
                    class="is-rounded" />
                </VControl>
              </VField>
            </div>
            <div class="column is-12">
              <VField class="is-rounded-select is-autocomplete-select">
                ...
                <VControl icon="feather:search">
                  <Multiselect mode="single" v-model="item.objectdivisifk" :options="d_divisi"
...
const dataSourcefiltered = computed(() => {
  return dataSource.value.filter((items: any) => {
    const matchOperasi = !filters.value || items.namaoperasi.match(new RegExp(filters.value, 'i'))
    const matchDivisi = !filters2.value || items.divisi.match(new RegExp(filters2.value, 'i'))
    return matchOperasi && matchDivisi
  })
})
...
async function fetchData() {
  ...
  const response = await useApi().get(
    '/sysadmin/master-operasi-profilaksis?offset=' + offset +
    '&limit=' + limit +
    '&rows=' + rows +
...
function loadDropdown() {
  d_divisi.value = []
  useApi().get(
    `/sysadmin/master-detail-divisi?statusantibiotik=1`).then((response: any) => {
      d_divisi.value = response.namadivisippra.map((e: any) => { return { label: e.divisi, value: e.id } })
    })
}
...
function loadData() {
  dataSource.value = []
  useApi().get(
    `/sysadmin/master-operasi-profilaksis?statusenabled=${item.value.aktif}`).then((response: any) => {
      dataSource.value = response.data
    })
}
...
function edit(e: any) {
  item.value.id = e.idpjo
  item.value.namaoperasi = e.namaoperasi
  item.value.objectdivisifk = e.idpd
  modalInput.value = true
}

async function save() {
  
  if (!item.value.namaoperasi) {
    useToaster().error('Nama Operasi Harus Diisi')
    return
  }

  if (!item.value.objectdivisifk) {
    useToaster().error('Divisi Harus Dipilih')
    return
  }
  ...
  var objSave =
  {
    'datasave': {
      ...
      'namaoperasi': item.value.namaoperasi,
      'divisi': item.value.objectdivisifk,
    }
  }
  isLoadingTT.value = true
  await useApi().post(
    `/sysadmin/save-master-operasi-profilaksis`, objSave).then((response: any) => {
...
async function deleterow(e: any) {
  await useApi().post(
    `/sysadmin/delete-master-operasi-profilaksis`, { 'id': e.idpjo }).then((response: any) => {
      loadData()
    }, (error) => {
    })
}
