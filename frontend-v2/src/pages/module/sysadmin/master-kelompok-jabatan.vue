<template>
  ...
  <div class="columns column">
    <h3 class="title is-5 mb-2 mr-1">Kelompok Jabatan</h3>
  </div>
    ...
    <div class="column is-8">
      ...
      <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
        <DataTable :value="dataSourcefiltered" class="p-datatable-sm" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
          ...
          <Column field="namakelompokjabatan" header="Kelompok Jabatan" :sortable="true"></Column>
          ...
          <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
            <div class="tile-grid-item">
              <div class="tile-grid-item-inner">
                <VAvatar size="medium" picture="/images/avatars/svg/team.png" color="primary" squared bordered />
                <div class="meta">
                  <span class="dark-inverted">{{ item.namakelompokjabatan }}</span>
        ...
        <VCard style="margin-top : -3rem;">
          <div class="columns is-multiline">
            ...
            <div class="column is-12">
              <VField label="Kelompok Jabatan">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.namakelompokjabatan" placeholder="Kelompok Jabatan" class="is-rounded" />
                </VControl>
              </VField>
            </div>
            ...
            <div class="column is-5">
              <VField label="Usia Pensiun">
                <VControl icon="feather:bookmark">
                  <VInput type="number" v-model="item.usiapensiun" placeholder="Usia" class="is-rounded" />
                </VControl>
              </VField>
            </div>
...
useHead({
  title: 'Kelompok Jabatan - ' + import.meta.env.VITE_PROJECT,
})
const item: any = ref({
  aktif: true
})
...
const dataSourcefiltered = computed(() => {
  ...
  return dataSource.value.filter((items: any) => {
    return (
      items.namakelompokjabatan.match(new RegExp(filters.value, 'i'))
    )
  })
})
...
async function fetchData() {
  ...
  if (item.namakelompokjabatan) KelompokJabatan = '&kelompokjabatan' + item.namakelompokjabatan
  ...
  const response = await useApi().get(
    '/sysadmin/master-kelompok-jabatan?offset=' + offset +
    '&limit=' + limit +
    '&rows=' + rows +
    StatusEnabled + KelompokJabatan + ReportDisplay
  )
  isLoading = false
...
function edit(e: any) {
  item.value.id = e.id
  item.value.namakelompokjabatan = e.namakelompokjabatan
...
async function save() {
  if (!item.value.namakelompokjabatan) {
  ...
  var objSave =
  {
    'kelompokjabatan': {
      ...
      'namakelompokjabatan': item.value.namakelompokjabatan,
  ...
  await useApi().post(
    `/sysadmin/save-master-kelompok-jabatan`, objSave).then((response: any) => {
...
async function deleterow(e: any) {
  await useApi().post(
    `/sysadmin/delete-master-kelompok-jabatan`, { 'id': e.id }).then((response: any) => {
      fetchData ()
    }, (error) => {
    })
}
...
function clear() {
  item.value.id = ''
  item.value.namakelompokjabatan = ''
