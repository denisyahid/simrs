<template>
  <VCard>
    <div>
      <h3 class="title is-5 mb-2 mr-1">Formulir RL 4B</h3>
      <span>DATA KEADAAN MORBIDITAS PASIEN RAWAT JALAN RUMAH SAKIT</span>
    </div>
    <div class="columns">
...
useHead({
  title: 'RL4b Penyakit Rawat Jalan' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
  ...
  await useApi().get(`laporan/get-laporan-rl4b?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}`).then((response: any) => {
...
const exportExcel = () => {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, 'RL4A Rawat Jalan', true);

  XLSXStyle.writeFile(workbook, 'RL4ARawatJalan.xlsx');
}
