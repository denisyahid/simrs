<template>
  <VCard>
    <div>
      <h3 class="title is-5 mb-2 mr-1">Laporan RL 5.1 PENGUNJUNG</h3>
    </div>
    <div class="columns">
            ...
            <div class="column is-6" v-for="(data, i) in dataSource">
              <div class="tile-grid-item">
                <div class="tile-grid-item-inner">
                  ...
                  <div class="meta">
                    <span class="dark-inverted mb-2">{{ data.jumlah }}</span>
                    <span class="dark-inverted">{{ data.statuspasien }}</span>
...
useHead({
  title: 'RL5.1 Laporan Pengunjung' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
  ...
  await useApi().get(`/laporan/get-laporan-rl5a?departemenfk=${dept}&ruanganfk=${ruangan}&tglAwal=${tglAwal}&tglAkhir=${tglAkhir}`).then((response: any) => {
...
const exportExcel = () => {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Laporan RL 5.1 - Pengunjung', true);

  XLSXStyle.writeFile(workbook, 'Laporan RL 5.1 - Pengunjung.xlsx');
}
const clearFilter = () => {
