<template>
  ...
  const listButton: any = ref([
    ...
    {
      label: 'Cetak Surat Keterangan Meninggal ',
      icon: 'fas fa-print',
      command: () => {
  ...
  const fetchLayanan = async () => {
    ...
    dataSource.value = []
    await useApi().get(
      `/ambulan/rincian-ambulan?norec_pd=${NOREC_PD}`).then(async (response: any) => {
  ...
  const loadPetugas = async (e: any) => {
    ...
    await useApi().get(
      `/ambulan/petugas-ambulan?norec=${e.norec}`).then((response: any) => {
  ...
  const savePetugas = () => {
    ...
    useApi().post(
      `/ambulan/save-petugas-ambulan`, json).then((response: any) => {
