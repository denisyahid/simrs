<template>
    <div>
                ...
                <div class="column is-12">
                    ...
                    <div class="column is-12" style="margin-top: 2rem;">
                        ...
                        <VTabs class="slider-lab" slider selected="Pasien" :tabs="[
                            ...
                            { label: 'Daftar Pasien Ambulan', value: 'Penunjang' },
                                                ...
                                                <div class="list-view-item-inner">
                                                    ...
                                                    <div class="meta-left">
                                                        ...
                                                        <span>
                                                            ...
                                                            <span>Tgl Order : {{ items.tglorder }}</span>
                                                            <i aria-hidden="true" class="fas fa-circle icon-separator"></i>
                                                            <i aria-hidden="true" class="iconify"
                                                    ...
                                                    <div class="meta-right">
                                                        ...
                                                        <VIconButton v-tooltip.bottom.left="'Surat Jalan'"
                                                            label="Bottom Left" color="info" circle outlined
                                                            icon="fas fa-paper-plane" @click="showSuratJalan(items)"
                                                            :loading="isLoadingBtnSurat" style="margin-right: 15px;" />
                                                        ...
                                                        <VIconButton v-tooltip.bottom.left="'Verifikasi'"
                                                            ...
                                                            @click="orderVerify(items)" :loading="item.isLoading"
        ...
        <VModal :open="modalSuratJalan" title="Surat Jalan" :noclose="true" size="medium" actions="right"
            @close="modalSuratJalan = false, clearSJ()">
            <template #content>
                <div class="columns is-multiline">
                    <div class="column is-12">
                        <VField label="Nomer Surat Jalan">
                            <VControl>
                                <VInput type="text" placeholder="Nomer Surat Jalan" class="is-rounded"
                                    v-model="item.nosuratjalan" />
                            </VControl>
                        </VField>
                    </div>
                    <div class="column is-5">
                        <VField label="Nomer Kendaraan">
                            <VControl>
                                <VInput type="text" placeholder="Nomer Kendaraan" class="is-rounded"
                                    v-model="item.nokendaraan" />
                            </VControl>
                        </VField>
                    </div>
                        ...
                        <!-- <VField class="is-autocomplete-select">
                            <VControl icon="feather:search">
                                <Dropdown v-model="item.jeniskeselamatanfk" :options="d_JenisKeselamatan"
                                    optionLabel="label" class="is-rounded" style="width: 100%;" :filter="true"
                                    appendTo="body" />
                            </VControl>
                        </VField> -->
                    </div>
                    <div class="column is-6">
                        <VField label="Pengemudi" class="is-autocomplete-select">
                            <VControl icon="fa:user" class="prime-auto-cus ">
                                <AutoComplete v-model="item.pengemudifk" :suggestions="d_Pegawai" :optionLabel="'label'"
                                    @complete="fetchPegawai($event)" :dropdown="true" :minLength="3" :appendTo="'body'"
                                    :loadingIcon="'pi pi-spinner'" :field="'label'" placeholder="Pilih Pengemudi"
                                    class="mt-2 is-rounded" />
                            </VControl>
                        </VField>
                    </div>
                    <div class="column is-6">
                        <VField label="Perawat" class="is-autocomplete-select">
                            <VControl icon="fa:user" class="prime-auto-cus ">
                                <AutoComplete v-model="item.perawatfk" :suggestions="d_Pegawai" :optionLabel="'label'"
                                    @complete="fetchPegawai($event)" :dropdown="true" :minLength="3" :appendTo="'body'"
                                    :loadingIcon="'pi pi-spinner'" :field="'label'" placeholder="Pilih Perawat"
                                    class="mt-2 is-rounded" />
                            </VControl>
                        </VField>
                    </div>
                ...
                <div class="columns is-multiline">
                    <div class="column is-3">
                        <VField label="Jalan">
                            <VControl>
                                <VInput type="number" placeholder="Kilometer" class="is-rounded" v-model="item.jalan" />
                            </VControl>
                        </VField>
                    </div>
                    <div class="column is-3">
                        <VField label="Datang">
                            <VControl>
                                <VInput type="number" placeholder="Kilometer" class="is-rounded" v-model="item.datang" />
                            </VControl>
                        </VField>
                    </div>
                    <div class="column is-3">
                        <VField label="Jarak Tempuh">
                            <VControl>
                                <VInput type="number" placeholder="Kilometer" class="is-rounded"
                                    v-model="item.jaraktempuh" />
                            </VControl>
                        </VField>
                    </div>
                    <div class="column is-3">
                        <VField label="Bensin">
                            <VControl>
                                <VInput type="number" placeholder="Liter" class="is-rounded" v-model="item.bensin" />
                            </VControl>
                        </VField>
                    </div>
                </div>

                <div class="column is-7 p-0 mb-2">
                    <VField class="is-autocomplete-select" label="Keperluan">
                        <VControl icon="feather:search">
                            <Dropdown v-model="item.keperluanfk" placeholder="Pilih Keperluan" :options="d_Keperluan"
                                optionLabel="label" class="is-rounded" style="width: 100%;" :filter="true"
                                appendTo="body" />
                        </VControl>
                    </VField>
                ...
                <div class="column is-12 pl-0 pr-0">
                    <VField label="Tujuan">
                        <VControl>
                            <VTextarea v-model="item.tujuan" rows="3">
                            </VTextarea>
                        </VControl>
                    </VField>
                </div>
            ...
            <template #action>
                <VButton icon="feather:save" @click="saveSuratJalan()" :loading="isLoading" color="primary" raised>
                    Lanjut</VButton>
            </template>
        </VModal>
    </div>
...
useHead({
    title: 'Dashboard Ambulance - ' + import.meta.env.VITE_PROJECT,
})
...
let modalSuratJalan: any = ref(false)
...
let isLoadingBtnSurat: any = ref(false)
...
const fetchDataOrder = async (q: any) => {
    try {
        ...
        const response = await useApi().get(`/ambulan/ambulan-verif?${search}${tglAwal}${tglAkhir}${qnamapasien}${statusOrder}${qnocm}${qnoregistrasi}&limit=${limit}&offset=${offset}`);
...
const fetchPenunjang = async () => {
    try {
        ...
        const response = await useApi().get(`/ambulan/penunjang-ambulan?ruanganid=${ruanganid}${tglAwal}${tglAkhir}${qnama}${qnocm}${qnoregistrasi}${search}&limit=${limit}&offset=${offset}`);
...
const getListPelayanan = async (data: any) => {
    const response = await useApi().get(`/ambulan/get-layanan-ambulance?idkelas=${data.idkelas}&idjenispelayanan=${data.idJenisPelayanan}&idruangan=${data.idruangan}`)
...
const orderVerify = async (e: any) => {
    ...
    const getHargaLayanan = await useApi().get(`/ambulan/detail-verif-ambulan?strukorderfk=${e.so_norec}&objectkelasfk=${e.objectkelasfk}`)
    const response = await useApi().get(`/ambulan/ambulan-verif?statusorder=${statusOrder.value}&noorder=${e.noorder}`)
...
const save = async () => {
    ...
    await useApi().post('/ambulan/save-verif-ambulan', { 'data': datas, 'parameter': parameter }).then((response: any) => {
        isLoading.value = false
...
const getHarga = async (e: any) => {
    const response = await useApi().get(`/ambulan/get-layanan-ambulance?idkelas=${item.value.kelas}&idjenispelayanan=${item.value.idJenisPelayanan}&idruangan=${item.value.idRuanganTujuan}`)
...
const fetchdDropdown = async () => {
    const response = await useApi().get(`/ambulan/dd-ambulan`)
    d_Keperluan.value = response.keperluan.map((e: any) => { return { label: e.keperluan, value: e.id } })
...
const transaksiPelayanan = (e: any) => {
    router.push({
        name: 'module-ambulan-transaksi-pelayanan-ambulan',
        query: {
            nocmfk: e.nocmfk,
...
const showSuratJalan = async (e: any) => {

    item.value.norec_pd = e.pd_norec
    item.value.strukorderfk = e.so_norec

    isLoadingBtnSurat.value = true;
    await useApi().get(`ambulan/get-surat-jalan?norec_pd=${item.value.norec_pd}&strukorderfk=${e.so_norec}`).then((response) => {
        if (response != null) {
            item.value.norec = response.norec
            item.value.nosuratjalan = response.nosurat
            item.value.nokendaraan = response.nosint
            item.value.pengemudifk = {label : response.pengemudi , value : response.pegawaifk }
            item.value.perawatfk = { label: response.perawat, value : response.dokterfk }
            item.value.jalan = response.jarakjalan
            item.value.datang = response.jarakdatang
            item.value.jaraktempuh = response.jaraktempuh
            item.value.bensin = response.bensin
            item.value.keperluanfk = { label: response.keperluan, value: response.keperluanfk }
            item.value.tujuan = response.tujuan
        }

        modalSuratJalan.value = true
    })
    isLoadingBtnSurat.value = false;
}

const cetakBuktiOrder = () => {
    // console.log(so_norec)

    H.printBlade('ambulan/cetak-bukti-ambulan?noregistrasi=' + item.value.noregistrasi
        + '&so_norec=' + so_norec);
}
...
const fetchPasien = async () => {
    ...
    const response = await useApi().get(
        '/ambulan/list-pasien-regist?ruanganid=' + ruanganid
        + '&dari=' + dari
        + '&sampai=' + sampai
...
const saveTransaksi = async (e: any) => {
    let json = {
        ...
        antrianpasiendiperiksa: {
            ...
            'objectpegawaifk': item.pegawaiID
        }
    }

    isLoading.value = true
    await useApi()
        .post(`/ambulan/save-transaksi-ambulan`, json)
        .then((response: any) => {
            isLoading.value = false
            clear()
...
const saveSuratJalan = async () => {
    ...
    if (!item.value.pengemudifk) {
        H.alert('error', 'Pengemudi Wajib Diisi')
        return
    }
    if (!item.value.perawatfk) {
        H.alert('error', 'Perawat Wajib Diisi')
        return
    }
    isLoading.value = true
    let objSave = {
        ...
        strukorderfk: item.value.strukorderfk,
        nosuratjalan: item.value.nosuratjalan,
        nokendaraan: item.value.nokendaraan,
        // jeniskeselamatanfk : item.value.jeniskeselamatanfk.value,
        pengemudifk: item.value.pengemudifk.value,
        perawatfk: item.value.perawatfk.value,
        jalan: item.value.jalan,
        datang: item.value.datang,
        jaraktempuh: item.value.jaraktempuh,
        bensin: item.value.bensin,
        keperluanfk: item.value.keperluanfk.value,
        tujuan: item.value.tujuan
    }
    await useApi().post('ambulan/save-surat-jalan', objSave).then((response) => {
        clearSJ()
        modalSuratJalan.value = false
        isLoading.value = false
    }).catch((e: any) => {
        modalSuratJalan.value = false
        isLoading.value = false
...
const clearSJ = () => {
    delete item.value.strukorderfk
    delete item.value.nosuratjalan
    delete item.value.nokendaraan
    delete item.value.pengemudifk
    delete item.value.perawatfk
    delete item.value.jalan
    delete item.value.datang
    delete item.value.jaraktempuh
    delete item.value.bensin
    delete item.value.keperluanfk
...
const changeSelected = (key:any) => {
  ...
  if (key == "PasienDaftar") {
    currentPasienDaftar.value.limit =9
    currentPasienDaftar.value.page = computed(() => {
      try {
        return Number.parseInt(route.query.page as string) || 1
      } catch { }
