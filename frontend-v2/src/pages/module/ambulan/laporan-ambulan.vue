            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Laporan Ambulan</label>
                        <span>List Pasien Order Ambulan</span>
                    </div>
                </div>

                <VPlaceload height="20rem" width="100%" class="mx-2 mt-4" v-if="loadData" />
                <DataTable v-else :rows="5" :value="dataSourcefiltered" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                        ...
                        <div class="columns is-multiline">
                            ...
                            <div class="column is-10">
                                <div class="columns is-multiline" style="justify-content: right;">
                                    ...
                                    <div class="column is-3" style="margin-top: 22px;">
                    ...
                    <Column field="noregistrasi" header="No Pendaftaran" />
                    <Column field="nocm" header="NORM" />
                    <Column field="namapasien" header="Nama Pasien" />
                    <Column field="tglorder" header="Tanggal Daftar" />
                    <Column field="tglpelayanan" header="Tanggal Pelayanan"/> 
                    <Column field="zona" header="Layanan"/>
                    <Column field="namaperawat" header="Perawat"/>
                    <Column field="pengemudi" header="Pengemudi"/>
                    <Column field="biaya" header="Biaya" style="text-align:right">
                        <template #body="slotProps">
                         {{ H.formatRupiah(H.roundToDecimal(parseFloat(slotProps.data.biaya), 2), '') }}
                        </template>
                    </Column>
...
useHead({
    title: 'Daftar Kematian Pasien > 48  Jam - ' + import.meta.env.VITE_PROJECT,
})
...
const op = ref();
const title = ref('Daftar Kematian Pasien > 48 Jam');
...
const fetchData = async () => {
    ...
    await useApi().get(`ambulan/laporan-order-ambulan?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}${ruangan}`).then((response) => {
...
const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ['Daftar Order Ambulan'],
        [],
        ['NO', 'SEP', 'NORM','NAMA PASIEN','NAMA PASIEN','TGL MENINGGAL','RUANGAN'],
...
const cetak = () => {
    ...
    H.printBlade(`ambulan/cetak-laporan?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}&pdf=true`);
}
