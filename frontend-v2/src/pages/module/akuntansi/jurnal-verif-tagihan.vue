...
const title = 'Verifikasi Tagihan Detail Jurnal'
useHead({
...
const fetchData = async () => {
    ...
    const response = await useApi().get(
        '/akuntansi/get-detailverifikasi?tglAwal=' + dari
        + '&tglAkhir=' + sampai
        + '&search=' + search
        + '&deptId=' + inst
        + '&ruangId=' + ruang
    ...
    H.cacheHelper().set('c_junralpelayanan_verif', c_set);
    // set page to 1
    route.query.page = 1
}
...
const loadMap = async (dataItem) => {
    ...
    item.tittlegridmap = ' : ' + dataItem.kelompokpasien + ' ' + dataItem.namarekanan
    item.objectkelompokpasienfkPilih = dataItem.objectkelompokpasienlastfk;
    item.objectrekananfkPilih = dataItem.objectrekananfk;
    await fetchRekanan({ query: dataItem.namarekanan })
    d_Rekanan.value.forEach(element => {
        if (dataItem.objectrekananfk == element.id) {
...
const loadDataMapJurnal = async () => {
    const response = await useApi().get("/akuntansi/get-detail-map-coa-by-rekanan-kelompokpasien?" +
        "objectkelompokpasienfk=" + item.objectkelompokpasienfkPilih +
        // "&objectrekananfk=" + item.objectrekananfkPilih
        "&objectjenistrxfk=2"  )
...
const addMap = () => {
    ...
    if (!item.objectkelompokpasienfkPilih) {
        H.alert('error', 'Pilih Data Pasien')
        return
    }
...
const updateJurnal = async (e: any) => {
    ...
    await useApi().post("/akuntansi/post-detail-jurnal-verifikasi", objSave)
        .then((r) => {
            e.isLoading2 = false
            fetchData()
        })
...
let c = H.cacheHelper().get('c_junralpelayanan_verif');
if (c != undefined) {
