                                ...
                                <DataTable v-model:filters="filtersTrans" :value="dataSource" paginator :rows="50"
                                    ...
                                    <template #header>
                                        <div class="columns is-multiline">
                                            <div class="column is-5">
                                                ...
                                                <VButton type="button" icon="pi pi-times-circle" class="mr-3" color="danger" :loading="isClosing2"
                                                    outlined circle raised @click="batalClosingJurnal()">
                                                    Batal Closing
                                                </VButton>


                                                <VButton type="button" icon="pi pi-save" class="mr-3" color="success" circle :loading="isClosing"
                                                    raised @click="closingJurnal()">
                                                    Closing Jurnal
                                                </VButton>
                                    ...
                                    <ColumnGroup type="header">
                                        <Row>
                                            ...
                                            <Column :colspan="2" header="Saldo Akhir"></Column>
                                        </Row>
                                        <Row>
...
const fetchData = async () => {
    ...
    const dat = await useApi().get(
        '/akuntansi/get-data-trial-balance?tglAwal=' + tglAwal1
        + '&tglAkhir=' + tglAkhir1
    ...
    item.saldoAwalDebet = saldoAwalDebet;//parseFloat(saldoAwalDebet).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,")
    item.saldoAwalKredit = saldoAwalKredit;//parseFloat(saldoAwalKredit).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,")

    item.mutasiDebet = mutasiDebet;//parseFloat(mutasiDebet).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,")
    item.mutasiKredit = mutasiKredit;//parseFloat(mutasiKredit).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,")

    item.saldoAkhirDebet = saldoAkhirDebet;//parseFloat(saldoAkhirDebet).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,")
    item.saldoAkhirKredit =saldoAkhirKredit;// parseFloat(saldoAkhirKredit).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,")
