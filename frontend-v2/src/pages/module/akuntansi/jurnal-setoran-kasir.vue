                            ...
                            <VField>
                                <VControl icon="feather:search">
                                    <input v-model="item.filter" v-on:keyup.enter="fetchData()" type="text"
                                        class="input is-rounded" placeholder="Cari Nama Pasien, No RM" />
                                </VControl>
                            </VField>
                        </div>
...
const title = 'Transaksi Penerimaan Kasir Detail Jurnal'
useHead({
...
const fetchData = async () => {
    ...
    const response = await useApi().get(
        '/akuntansi/get-detail-terima-kasir?tglAwal=' + dari
        + '&tglAkhir=' + sampai
        + '&search=' + search
        + '&deptId=' + inst
        + '&ruangId=' + ruang
...
const loadMap = async (dataItem) => {
    ...
    item.tittlegridmap = ' : ' + dataItem.carabayar
    item.objectprodukfkPilih = dataItem.objectcarabayarfk;
...
const loadDataMapJurnal = async () => {
    const response = await useApi().get("/akuntansi/get-detail-map-coa-by-carabayarid?carabayarid=" + item.objectprodukfkPilih)
...
const editMap = (e: any) => {
    ...
    if (e.objectrekananfk) {
        d_Rekanan.value.forEach(element => {
            if (element.id == e.objectrekananfk) {
...
const updateJurnal = async (e: any) => {
    ...
    await useApi().post("/akuntansi/post-detail-jurnal-kwitansi", objSave)
        .then((r) => {
            e.isLoading2 = false
            fetchData()
        })
...
const saveSetting = async () => {
    ...
    let objSave = {
        ...
        "objectcarabayarfk": item.caraBayar == undefined ? null : item.caraBayar.id,
        "objectprodukfk": null
    };
    isLoading.value = true
