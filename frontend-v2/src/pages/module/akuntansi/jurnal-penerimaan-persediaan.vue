    ...
    <section>
        ...
        <Dialog v-model:visible="modalJurnal" modal
            ...
            <div class="columns is-multiline">
                ...
                <div class="column is-4">
                    <VField label="Detail Jenis" class="is-rounded-select is-autocomplete-select
                              mt-0 pt-0" v-slot="{ id }">
                        <VControl icon="fas fa-pie-chart" fullwidth class="prime-auto-select">
                            <Dropdown v-model="item.detailJenisProduk" :options="d_DetailJenis"
                                :optionLabel="'detailjenisproduk'" class="is-rounded" placeholder="Detail Jenis"
...
const title = 'Penerimaan Persediaan Detail Jurnal'
useHead({
...
const fetchDropdown = async () => {
    await useApi().get('/akuntansi/get-data-combo-map-coa').then((r) => {
        ...
        d_JenisProduk.value = r.jenisproduk
...
const fetchData = async () => {
    ...
    const response = await useApi().get(
        '/akuntansi/get-detail-penerimaanbarang?tglAwal=' + dari
        + '&tglAkhir=' + sampai
        + '&nostruk=' + search
        + '&deptId=' + inst
        + '&ruangId=' + ruang
    ...
    H.cacheHelper().set('c_junralpelayanan_sup', c_set);
    // set page to 1
    route.query.page = 1
}
...
const loadMap = async (dataItem) => {
    ...
    item.objectprodukfkygdipilih = dataItem.prid;
...
const loadDataMapJurnal = async () => {
    const response = await useApi().get("/akuntansi/get-detail-map-coa-by-produkid?" +
        "produkid=" + item.objectprodukfkygdipilih +
        "&objectjenistrxfk=4,6,7"
    )
...
const editMap = (e: any) => {
    ...
    if (e.objectdetailjenisprodukfk) {
        d_DetailJenis.value.forEach(element => {
            if (element.id == e.objectdetailjenisprodukfk) {
                item.detailJenisProduk = element
                return
            }
        });
    }
    if (e.objectjenisprodukfk) {
        d_JenisProduk.value.forEach(element => {
            if (element.id == e.objectjenisprodukfk) {
...
const updateJurnal = async (e: any) => {
    ...
    await useApi().post("/akuntansi/post-detail-jurnal-penerimaan-suplier", objSave)
        .then((r) => {
            e.isLoading2 = false
            fetchData()
        })
...
const saveSetting = async () => {
    ...
    let objSave = {
        ...
        "objectjenisprodukfk": item.detailJenisProduk == undefined ? null : item.detailJenisProduk.objectjenisprodukfk,
        
        "objectdetailjenisprodukfk": item.detailJenisProduk == undefined ? null : item.detailJenisProduk.id,
...
let c = H.cacheHelper().get('c_junralpelayanan_sup');
if (c != undefined) {
