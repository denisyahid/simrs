                    ...
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-4">
                            <VCard class="card-round-1">
                                <p class="title-c">RIWAYAT REGISTRASI</p>
                                <DataTable v-model:filters="filters" :value="dataSourceReg" paginator :rows="10"
                                    dataKey="id" filterDisplay="row" :rowsPerPageOptions="[5, 10, 25, 100]"
                                    :globalFilterFields="['namapasien', 'norm', 'noregistrasi']"
                                    ...
                                    <Column :exportable="false" header="Detail" style="width:50px">
                                        <template #body="slotProps">
                                            <VIconButton type="button" icon="pi pi-arrow-right" class="mr-3" color="danger"
                                                circle outlined raised v-tooltip-prime="'Lihat Transaksi'"
                                                @click="loadPelayanan(slotProps.data)" :loading="slotProps.data.isLoading">
                                            </VIconButton>
                                        </template>
                                    </Column>
                                    <Column v-for="col in columnReg" :field="col.field" :header="col.title"
                                        :style="'width:' + col.width"></Column>
                                    <ColumnGroup type="footer">
                                        <Row>
                                            <Column :footer="'Terdapat ' + dataSourceReg.length + ' data.'"
                                                :colspan="columnReg.length + 1" />
                                        </Row>
                                    </ColumnGroup>
                                </DataTable>
                            </VCard>
                                ...
                                <DataTable v-model:filters="filtersTrans" :value="dataSourceTrans" paginator :rows="10"
                                    ...
                                    <Column v-for="col in columnTrans" :field="col.field" :header="col.title"
                                        ...
                                        <template #body="slotProps">
                                            ...
                                            <span v-else> <VTag class="mr-1 mb-1" :color="'success'" :label="slotProps.data[col.field]" /></span>
                                        </template>
                                    </Column>
...
const title = 'Pelayanan Pasien Detail Jurnal'
useHead({
...
const columnTrans: any = [
    ...
    {
        "field": "deskripsiproduktransaksi",
        "title": "Kelompok",
        "width": "50px"
    },
    {
...
const fetchData = async () => {
    ...
    const response = await useApi().get(
        '/akuntansi/get-daftar-registrasi-pasien?tglAwal=' + dari
        + '&tglAkhir=' + sampai
        + '&search=' + search
        + '&deptId=' + inst
        + '&ruangId=' + ruang
    ...
    dataSourceReg.value = response

    let c_set = {
        0: dari,
        1: sampai,
    }
    H.cacheHelper().set('c_junralpelayanan', c_set);
    // set page to 1
    route.query.page = 1
}
...
const loadDataMapJurnal = async () => {
    const response = await useApi().get("/akuntansi/get-detail-map-coa-by-produkid?produkid=" + item.objectprodukfkygdipilih
    +  "&objectjenistrxfk=1" )
...
const updateJurnal = async (e: any) => {
    e.isLoading2 = true
    for (var i = 0; i < dataSourceTrans.value.length; i++) {
        if (e.prid == dataSourceTrans.value[i].prid) {
            var objSave = {
                "registrasi": item.objectNoregistrasi,
                "namapasien": item.objectNamaPasien,
                "pelayanan": dataSourceTrans.value[i]
            };

            await useApi().post("/akuntansi/post-detail-jurnal", objSave)
            .then((r)=>{
              e.isLoading2 = false
                loadPelayanan(selectedRegis.value)
            })
            .catch((xx => {
                e.isLoading2 = false
            }))
...
const saveSetting = async () => {
    ...
    let objSave = {
        ...
        "objectprodukfk": item.objectprodukfkygdipilih == undefined ? null : item.objectprodukfkygdipilih,
    };
    isLoading.value = true
...
let c = H.cacheHelper().get('c_junralpelayanan');
if (c != undefined) {
