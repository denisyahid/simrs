                    ...
                    <DataTable :value="dataSource" :rows="20" v-model:expandedRows="expandedRows"
                      ...
                      <ColumnGroup type="header">
                        ...
                        <Row>
                          ...
                          <Column header="Jumlah Penerimaan/Pengadaan" :colspan="3" />
                          <Column header="Jumlah Pengeluaran" :colspan="3" />
                      ...
                      <Column field="totalhargaterima" style="min-width: 150px">
                        <template #body="slotProps">
                          {{ H.formatRupiah(Math.ceil(slotProps.data.totalhargaterima), 'Rp') }}
                        </template>
                      </Column>
                      ...
                      <Column field="totalhargakeluar" style="min-width: 150px">
                        <template #body="slotProps">
                          {{ H.formatRupiah(Math.ceil(slotProps.data.totalhargakeluar), 'Rp') }}
                        </template>
                      </Column>
                      ...
                      <Column field="totalrped" style="min-width: 150px">
                        <template #body="slotProps">
                          {{ H.formatRupiah(Math.ceil(slotProps.data.totalrped), 'Rp') }}
                        </template>
                      </Column>
                      ...
                      <Column field="totalrpsaldoakhir" style="min-width: 150px">
                        <template #body="slotProps">
                          {{ H.formatRupiah(Math.ceil(slotProps.data.totalrpsaldoakhir), 'Rp') }}
                        </template>
                      </Column>
                      ...
                      <ColumnGroup type="footer">
                        <Row>
                          ...
                          <Column :footer="H.formatRp(totalPENERIMAAN, 'Rp. ')" :colspan="3"
                            footerStyle="text-align:right" />
                          <Column :footer="H.formatRp(totalPENGELUARAN, 'Rp. ')" :colspan="3"
                            footerStyle="text-align:right" />
...
const fetchData = async () => {
  ...
  useApi().get(
        "/farmasi/get-data-laporan-persediaan-new?" +
        "&blnLalu=" + bulanlalu +
        "&tglawal=" + tglAwal +
        "&tglakhir=" + tglAkhir +
...
const hitungJML = (e: any) => {
  ...
  for (let x = 0; x < e.length; x++) {
    const element = e[x];
    totalSALDOAWAL.value += parseFloat(element.totalrpsaldoawal) || 0
    totalPENERIMAAN.value += parseFloat(element.totalhargaterima) || 0
    totalPENGELUARAN.value += parseFloat(element.totalhargakeluar) || 0
    totalEXP.value += parseFloat(element.totalrped) || 0
    totalSTOK.value += parseFloat(element.saldoakhir) * parseFloat(element.hargasatuan) || 0
  }
}
