<template>
  ...
  useHead({
    title: 'Laporan Psikotropika - ' + import.meta.env.VITE_PROJECT,
  })
  async function cariRiwayat() {
    ...
    useApi()
      .get(
        `/pelayanan/laporan-persediaan-psikotropika?tglAwal=${`${selectyear.value.code}-${selectmonth.value.code}-01`}&tglAkhir=${
          date.akhir
        }&produk=${produk}`
      )
      .then((response: any) => {
  ...
  const fetchProduk = async (filter: any) => {
    ...
    await useApi()
      .get(`pelayanan/get-laporan-persediaan-psikotropika?produk=${produk}`)
      .then((response) => {
