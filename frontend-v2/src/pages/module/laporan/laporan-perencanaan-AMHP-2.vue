...
<template>
    <div class="columns is-multiline">
      <VCard style="padding-bottom: 0px">
        <div class="column c-title pt-2 mb-5">
          <label class="title-page">Laporan Perencanaan PerSatelit (AMHP)</label>
        </div>
  ...
  useHead({
    title: 'Laporan Perencanaan AMHP (Satelit) - ' + import.meta.env.VITE_PROJECT,
  })
  ...
  const fetchData = async () => {
    ...
    await useApi().get(`pelayanan/get-laporan-perencanaan-AMHP-2?${tglAwal}${tglAkhir}${ruangan}${dokter}${nama}`).then((response: any) => {
