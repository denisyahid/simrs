            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Formulir RL 3.8</label>
                        <label for="">REKAPITULASI KEGIATAN PELAYANAN LABORATORIUM</label>
                    </div>
                </div>

                <VPlaceload height="20rem" width="100%" class="mx-2 mt-4" v-if="loadData" />
                <DataTable v-else :rows="200" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <Column field="namaproduk" header="JENIS KEGIATAN" />
                    <Column field="jmlproduk" header="JUMLAH" />
                </DataTable>
            </VCard>
        </div>
    </section>
...
const fetchData = async () => {
    ...
    await useApi().get(`laporan/get-laporan-kegiatan-Laboratorium?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}`).then((response) => {
...
const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ['FORMULIR RL 3.8 - KEGIATAN LABORATORIUM'],
        [],
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Laporan RL 3.8', true);
    XLSXStyle.writeFile(workbook, 'FORMULIR RL 3.8 - KEGIATAN LABORATORIUM.xlsx');
}
