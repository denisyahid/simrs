            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Formulir RL 3.18</label>
                        <label for="">Rekapitulasi Kegiatan Pelayanan Farmasi Rumah Sakit - Resep</label>
                    </div>
                </div>
                ...
                <DataTable v-else :rows="5" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <Column field="jumlahrajal" header="Rawat Jalan" />
                    <Column field="jumlahigd" header="IGD" />
                    <Column field="jumlahranap" header="Rawat Inap" />
                </DataTable>
            </VCard>
        </div>
    </section>
...
const fetchData = async () => {
    ...
    await useApi().get(`laporan/get-pelayanan-farmasi-resep?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}`).then((response) => {
        response.data.forEach((element:any,i:any) => {
            element.no = i+1
            no = element.no || 0;
            total += element.rawat_jalan_count || 0;   
            total2 += element.igd_count || 0;
            total3 += element.rawat_inap_count || 0;
        });
...
const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ['Formulir RL 3.18 Rekapitulasi Kegiatan Pelayanan Farmasi Rumah Sakit - Resep'],
        [],
        ['No', 'Golongan Obat', 'Rawat Jalan', 'IGD', 'Rawat Inap'],
    ...
    const columnWidths = [10, 40,20,20,20];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Formulir RL 3.18', true);
    XLSXStyle.writeFile(workbook, 'RL 3.18 Rekapitulasi Kegiatan Pelayanan Farmasi Rumah Sakit - Resep.xlsx');
}
