            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Formulir RL 3.5</label>
                        <label for="">REKAPITULASI KUNJUNGAN</label>
                    </div>
                </div>
                ...
                <DataTable v-else :rows="50" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <Column field="jenis_spesialisasi" style="min-width: 100px;" header="Jenis Kegiatan" />
                    <Column field="DalamKotaL" style="min-width: 80px;text-align: center;" header="Dalam Kota L" />
                    <Column field="DalamKotaP" style="min-width: 50px;text-align: center;" header="Dalam Kota P" />
                    <Column field="LuarKotaL" style="min-width: 80px;text-align: center;" header="Luar Kota L" />
                    <Column field="LuarKotaP" style="min-width: 50px;text-align: center;" header="Luar Kota P" />
                    <Column field="Total" style="min-width: 50px;text-align: center;" header="TOTAL" />
                </DataTable>
            </VCard>
        </div>
    </section>
...
useHead({
    title: 'Laporan Formulir RL 3.4 - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
    ...
    await useApi().get(`/laporan/get-laporan-rekapitulasi-kunjungan?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}`).then((response) => {
...
const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ...
        [],
        ['NO', 'JENIS KEGIATAN', 'DALAM KOTA L', 'DALAM KOTA P', 'LUAR KOTA L', 'LUAR KOTA P', 'TOTAL'],
    ...
    const columnWidths = [8, 40, 10, 40, 40, 40, 40];
    ...
    XLSXStyle.writeFile(workbook, 'RL 3.5. Rekapitulasi Kunjungan .xlsx');
}
