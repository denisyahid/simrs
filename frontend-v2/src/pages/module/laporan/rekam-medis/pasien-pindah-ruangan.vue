            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label for="">LAPORAN PASIEN PINDAH RUANGAN</label>
                    </div>
                </div>
                ...
                <DataTable v-else :rows="5" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <Column field="no" header="No" frozen/>
                    <Column field="tglmasuk" header="Tgl Masuk" />
                    ...
                    <Column field="ruangantujuan" header="Ruangan Tujuan" />
                    <Column field="kamarasal" header="Kamar Asal" />
                    <Column field="kamartujuan" header="Kamar Tujuan" />
                    <Column field="umur" header="Umur" />
                    <Column field="namalengkap" header="Dokter" />
                </DataTable>
            </VCard>
        </div>
    </section>
...
useHead({
    title: 'Laporan Pasien Pindah Ruangan - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
    ...
    await useApi().get(`laporan/pasien-pindah-ruangan?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}`).then((response) => {
...
const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ['Laporan Pasien Pindah Ruangan'],
        [],
        ['No', 'Tanggal Masuk', 'No Registrasi', 'No RM', 'Nama Pasien', 'Jenis Kelamin','Ruangan Asal','Ruangan Tujuan','Kamar Asal','Kamar Tujuan','Umur','Dokter'],
    ...
    const columnWidths = [10, 20, 20, 20, 20, 20, 20, 20,20,20,15,20];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Laporan Pasien Pindah Ruangan', true);
    XLSXStyle.writeFile(workbook, 'sheet 1.xlsx');
}
