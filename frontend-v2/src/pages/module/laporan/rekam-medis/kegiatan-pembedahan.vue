            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Formulir RL 3.12</label>
                        <label for="">REKAPITULASI KEGIATAN PELAYANAN PEMBEDAHAN</label>
                    </div>
                </div>

                <VPlaceload height="20rem" width="100%" class="mx-2 mt-4" v-if="loadData" />
                <DataTable v-else :rows="20" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <Column field="jumlah" header="TOTAL" />
                    <Column field="khusus" header="KHUSUS"/>
                    <Column field="besar" header="BESAR" />
                    <Column field="sadang" header="SEDANG" />
                    <Column field="kecil" header="KECIL" />
                </DataTable>
            </VCard>
        </div>
    </section>
...
const fetchData = async () => {
    ...
    await useApi().get(`laporan/get-laporan-kegiatan-pembedahan?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}`).then((response) => {
...
const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ['FORMULIR RL 3.12 - KEGIATAN PEMBEDAHAN'],
        [],
        ['NO', 'SPESIALISASI', 'TOTAL', 'KHUSUS', 'BESAR', 'SEDANG','KECIL'],
    ...
    const columnWidths = [10, 15,15,15,15,15,15];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'FORMULIR RL 3.6', true);
    XLSXStyle.writeFile(workbook, 'FORMULIR RL 3.12 - KEGIATAN PEMBEDAHAN.xlsx');
}
