            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Formulir RL 3.16</label>
                        <label for="">KEGIATAN PELAYANAN KELUARGA BERENCANA</label>
                    </div>
                </div>

                <VPlaceload height="20rem" width="100%" class="mx-2 mt-4" v-if="loadData" />
                <DataTable v-else :rows="10" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <ColumnGroup type="header">
                        <Row>
                            ...
                            <Column header="Jenis Pelayanan Keluarga Berencana" style="min-width: 80px; text-align: center;" :rowspan="2" />
                            <Column header="Pelayanan KB" style="text-align: center;" :colspan="4" />
                            <Column header="Komplikasi KB" style="text-align: center" :rowspan="2" />
                            <Column header="Kegagalan KB" style="text-align: center" :rowspan="2"  />
                            <Column header="Efek Samping" style="text-align: center" :rowspan="2"  />
                            <Column header="Luka-luka" style="text-align: center" :rowspan="2"  />
                            <Column header="Drop Out" style="text-align: center" :rowspan="2"  />
                        </Row>
                        <Row>
                            <Column header="Pasca Persalinan" style="text-align: center;" />
                            <Column header="Pasca Keguguran" style="text-align: center;" />
                            <Column header="Interval" style="text-align: center;" />
                    ...
                    <Column field="jmltindakan" style="min-width: 50px;" />
                    <Column field="jmltindakan" />
                    <Column field="jmltindakan" />
                    <Column field="jmltindakan" />
                    <Column field="jmltindakan" />
                    <Column field="jmltindakan" />
...
useHead({
    title: 'Laporan Formulir RL 3.16 - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
    ...
    let tglAwal = H.formatDate(item.value.filterDate.start, 'YYYY-MM-DD 00:00')
    let tglAkhir = H.formatDate(item.value.filterDate.end, 'YYYY-MM-DD 23:59')

    loadSearch.value = true
    await useApi().get(`/laporan/get-laporan-keluarga-berencana?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}`).then((response) => {
...
const exportExcel = () => {
    ...
    XLSXStyle.writeFile(workbook, 'RL 3.11. Gigi dan Mulut .xlsx');
}
