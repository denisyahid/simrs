            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Formulir RL 3.17</label>
                        <label for="">Rekapitulasi Kegiatan Pelayanan Farmasi RS - Pengadaan Obat</label>
                    </div>
                </div>
                ...
                <DataTable v-else :rows="5" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <Column field="jenis" header="Golongan Obat" />
                    <Column field="jumlahitemobat" header="Jumlah Item Obat" />
                    <Column field="jumlahtersedia" header="Jumlah Item Obat Yang Tersedia Di Rumah Sakit" />
                </DataTable>
            </VCard>
        </div>
    </section>
...
const fetchData = async () => {
    ...
    await useApi().get(`laporan/get-kegiatan-laporan-farmasi?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}`).then((response) => {
        let total = 0;
        let total2 = 0;
        let no = 0;
        ...
        response.data.forEach((element: any, i: any) => {
            element.no = i + 1;
            no = element.no || 0;
            total += element.jumlah || 0;   
            total2 += element.jumlahtersedia || 0;
        });
...
const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ['Formulir RL 3.17 Rekapitulasi Kegiatan Pelayanan Farmasi RS - Pengadaan Obat'],
        [],
        ['No', 'Golongan Obat', 'Jumlah Item Obat', 'Jumlah Item Obat Yang Tersedia Di Rumah Sakit'],
    ...
    const columnWidths = [10, 40,20,40];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'FORMULIR RL 3.17', true);
    XLSXStyle.writeFile(workbook, 'Formulir RL 3.17 Rekapitulasi Kegiatan Pelayanan Farmasi RS - Pengadaan Obat.xlsx');
}
