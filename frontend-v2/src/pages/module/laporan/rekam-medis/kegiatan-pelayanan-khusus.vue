            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        ...
                        <label for="">PELAYANAN KHUSUS</label>
                    </div>
                </div>

                <VPlaceload height="20rem" width="100%" class="mx-2 mt-4" v-if="loadData" />
                <DataTable v-else :rows="25" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <Column field="kelompoklaporan" header="JENIS KEGIATAN" />
                    <Column field="jumlah"  style="min-width: 50px; text-align: center;" header="JUMLAH"/>
                </DataTable>
            </VCard>
        </div>
    </section>
...
useHead({
    title: 'PELAYANAN Khusus - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
    ...
    await useApi().get(`laporan/get-laporan-kegiatan-pelayanan-khusus?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}`).then((response) => {
...
const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ['FORMULIR RL 3.14 - PELAYANAN KHUSUS'],
        [],
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'RL 3.14', true);
    XLSXStyle.writeFile(workbook, 'FORMULIR RL 3.14 - PELAYANAN KHUSUS.xlsx');
}
fetchData()
