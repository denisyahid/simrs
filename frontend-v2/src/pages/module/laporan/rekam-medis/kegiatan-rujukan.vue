            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Formulir RL 3.10</label>
                        <label for="">REKAPITULASI KEGIATAN PELAYANAN RUJUKAN</label>
                    </div>
                </div>
                ...
                <DataTable v-else :rows="100" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <ColumnGroup type="header">
                        ...
                        <Row>
                            ...
                            <Column header="TOTAL RUJUKAN MASUK" style="text-align: center;" />
                            ...
                            <Column header="TOTAL RUJUKAN MASUK DIKEMBALIKAN" style="text-align: center;" />
                            <Column header="PASIEN RUJUKAN" style="text-align: center;" />
                            <Column header="PASIEN DATANG SENDIRI" style="text-align: center;" />
                            <Column header="TOTAL DIRUJUK KELUAR" style="text-align: center;" />
...
const fetchData = async () => {
    ...
    await useApi().get(`/laporan/get-laporan-kegiatan-asal-rujukan?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}`).then((response) => {
        response.data.forEach((element: any, i: any) => {
            element.no = i + 1
            element.totalmasuk = element.puskesmas + element.hospital + element.klinik
            element.totalmasukkembali = element.DKembalikanKPuskes + element.DKembalikanKRs + element.DKembalikanKKlinik
            element.totaldirujuk = element.PasienRujukan + element.dtgsendiri
        });
