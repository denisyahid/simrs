...
<template>
    <div class="columns is-multiline">
      <VCard style="padding-bottom: 0px">
        <div class="column c-title pt-2 mb-5">
          <label class="title-page">Laporan Perencanaan Ins. Farmasi  </label>
        </div>
  ...
  useHead({
    title: 'Laporan Perencanaan Perinstalasi Farmasi - ' + import.meta.env.VITE_PROJECT,
  })
  ...
  const fetchData = async () => {
    ...
    await useApi().get(`pelayanan/get-laporan-perencanaan-farmasi?${tglAwal}${tglAkhir}${ruanganfk}${dokter}${nama}${status}`).then((response: any) => {
  ...
  const fetchRuangan = async (filter: any) => {
    await useApi().get(
      `emr/dropdown/ruangan_m?select=id,namlaporaruangan&param_search=namaruangan&query=${filter.query}&limit=10`
    ).then((response) => {
