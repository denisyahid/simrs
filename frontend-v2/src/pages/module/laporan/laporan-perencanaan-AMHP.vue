...
<template>
    <div class="columns is-multiline">
      <VCard style="padding-bottom: 0px">
        <div class="column c-title pt-2 mb-5">
          <label class="title-page">Laporan Perencanaan PerInstalasi (AMHP)</label>
        </div>
  ...
  useHead({
    title: 'Laporan Perencanaan AMHP (Perinstalasi) - ' + import.meta.env.VITE_PROJECT,
  })
  ...
  const fetchData = async () => {
    ...
    await useApi().get(`pelayanan/get-laporan-perencanaan-AMHP?${tglAwal}${tglAkhir}${ruanganfk}${dokter}${nama}`).then((response: any) => {
  ...
  return dataSource.value.map(item => {
    const avgPemakaian = Math.round(item.pengeluaran / 3); 
    const kebutuhan6bulan = Math.round(avgPemakaian * 7.7);
    ...
    const tingkatkecukupan = avgPemakaian > 0 
      ? Math.round((item.total / avgPemakaian) * 30)
      : 0;
