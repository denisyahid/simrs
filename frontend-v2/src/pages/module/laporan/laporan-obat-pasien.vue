...
<template>
    <div class="columns is-multiline">
      <VCard style="padding-bottom: 0px">
        <div class="column c-title pt-2 mb-5">
          <label class="title-page">Laporan Penjualan Per Resep</label>
        </div>
        <div class="column is-12">
          ...
          <template #header>
            <div class="columns is-multiline pb-3">
              ...
              <Column field="noresep" header="No Resep" frozen :sortable="true" style="min-width: 150px"></Column>
              <Column field="tglresep" header="Tanggal" :sortable="true" style="min-width: 150px"></Column>
              <Column field="namapasien" header="Nama Pasien" :sortable="true" style="min-width: 200px"></Column>
              <Column field="nocm" header="NRM" :sortable="true" style="min-width: 200px"></Column>
              ...
              <Column
                ...
                <template #body="slotProps">
                  {{ H.formatRupiah(slotProps.data.total_tagihan, 'Rp.') }}
                </template>
              </Column>
              <Column field="iskronis" header="Keterangan" :sortable="true" style="width: 80px">
                <template #body="slotProps">
                    <span v-if="slotProps.data.iskronis === 'true'">Non Inacbgs</span>
                    <span v-else-if="slotProps.data.iskronis === 'OB'">Penjualan Obat Bebas</span>
                    <span v-else-if="slotProps.data.iskronis === 'false'"> - </span>
                    <span v-else> - </span>
                </template>
                </Column>
          </DataTable>
  ...
  const fetchData = async () => {
    ...
    await useApi().get(`pelayanan/get-laporan-pasien?${tglAwal}${tglAkhir}${ruanganfk}${dokter}${nama}`).then((response: any) => {
  ...
  const exportExcel = () => {
    ...
    remakeData.value = dataSource.value.map((e: any) => {
      return {
        NoResep : e.noresep, Tanggal : e.tglresep, NamaPasien: e.namapasien, NoRM: e.nocm, CaraBayar : e.kelompokpasien,
        Dokter: e.namalengkap, Ruangan: e.namaruangan, Total : Number(e.total_tagihan),
        Keterangan: e.iskronis === 'true' ? 'Non Inacbgs' : e.iskronis === 'OB' ? 'Penjualan Obat Bebas' : e.iskronis === 'false' ? ' - ' : ' - '
      }
    })
