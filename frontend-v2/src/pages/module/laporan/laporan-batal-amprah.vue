     ...
     <div class="columns is-multiline">
      <VCard style="padding-bottom: 0px">
        <div class="column c-title pt-2 mb-5">
          <label class="title-page">Laporan Batal Amprah Darah</label>
        </div>
        ...
        <DataTable :value="dataSource" class="p-datatable-sm" :loading="isPlaceLoad" :paginator="true"
          ...
          <template #header>
            ...
            <Column field="noorder" header="No Order" style="min-width: 200px"></Column>
            <Column field="tglorder" header="Tanggal Order" :sortable="true" style="min-width: 200px"></Column>
            <Column field="tglbatal" header="Tanggal Batal" :sortable="true" style="min-width: 200px"></Column>
            <Column field="penerima" header="Penerima" :sortable="true" style="min-width: 200px"></Column>
            <Column field="pembatal" header="Pembatal" :sortable="true" style="min-width: 200px"></Column>
            <Column field="alasan" header="Alasan Batal" :sortable="true" style="min-width: 200px"></Column>
            <Column header="Action" style="min-width: 90px" align="center">
            <template #body="slotProps">
                <VButton v-model="isPaket" color="info"
                    class="is-pulled-right" @click="liatForm(slotProps.data)">Lihat Form</VButton>
            </template>
            </Column>
        </DataTable>
        </div>
...
const fetchData = async () => {
  ...
  await useApi().get(`laporan/get-laporan-batal-amprah?${tglAwal}${tglAkhir}`).then((response: any) => {
...
const liatForm = async (e: any) => {
    router.push({
        name: 'module-emr-profile-pasien-page-emr-formulir-pembatalan-penggunaan-komponen-darah',
        query: {
            nocmfk: e.nocmfk,
            norec_pd: e.norec_pd,
...
const exportExcel = () => {
  ...
  remakeData.value = dataSource.value.map((e: any) => {
    return {
      No: e.no, NoRegistrasi: e.noregistrasi, NoCM: e.nocm, NamaPasien: e.namapasien, NoOrder: e.noorder, TglOrder: e.tglorder,
      TglBatal: e.tglbatal, Penerima: e.penerima, Pembatal: e.pembatal, Alasan: e.alasan
  ...
  const excelBuffer: any = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    XLSXStyle.writeFile(workbook, 'Laporan Batal Amprah Darah.xlsx');
}
