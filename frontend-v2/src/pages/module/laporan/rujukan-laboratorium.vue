     ...
     <div class="columns is-multiline">
      <VCard style="padding-bottom: 0px">
        <div class="column c-title pt-2 mb-5">
          <label class="title-page">Laporan Rujukan Laboratorium</label>
        </div>
        ...
        <DataTable :value="dataSource" class="p-datatable-sm" :loading="isPlaceLoad" :paginator="true"
          ...
          <template #header>
            <div class="columns is-multiline pb-3">
             ...
             <Column field="RM" header="RM"  style="min-width: 50px"></Column>
             <Column field="noorder" header="No Order"  style="min-width: 100px"></Column>
             <Column field="namapasien" header="Nama Pasien"  style="min-width: 50px"></Column>
            <Column field="jenispemeriksaan" header="Jenis Pemeriksaan"  style="min-width: 50px"></Column>
            <Column field="dokterpengirim" header="Dokter Pengirim"  style="min-width: 50px"></Column>
            <Column field="tanggungan" header="Tanggungan"  style="min-width: 50px"></Column>
            <Column field="jumlah" header="QTY"  style="min-width: 50px"></Column>
            
           
            <Column field="hargasatuan" header="Harga satuan"  style="min-width: 200px">
              <template #body="slotProps">
                <span>{{ H.formatRupiah(slotProps.data.hargasatuan, "Rp.") }}
                </span>
              </template>
            </Column>
            <Column field="totalbiayapemeriksaan" header="Total Biaya Pemeriksaan"  style="min-width: 200px">
              <template #body="slotProps">
                <span>{{ H.formatRupiah(slotProps.data.totalbiayapemeriksaan, "Rp.") }}
                </span>
              </template>
            </Column>
            <Column field="namaruangan" header="Nama Ruangan"  style="min-width: 50px"></Column>
            <Column field="tanggal" header="Tanggal Pelayanan"  style="min-width: 50px"></Column>
        </DataTable>
        </div>
      </VCard>
     </div>
...
const fetchData = async () => {
  ...
  await useApi().get(`laporan/laboratorium/rekap-data-rujukan?${tglAwal}${tglAkhir}${ruangId}`).then((response: any) => {
...
const exportExcel = () => {
  ...
  remakeData.value = dataSource.value.map((e: any) => {
    return {      
      ...
      Jenis_Pemeriksaan: e.jenispemeriksaan,
      Dokter_Pengirim: e.dokterpengirim,
      Tanggungan: e.tanggungan,
      QTY: e.jumlah,
      Harga_Satuan: e.hargasatuan,
      Totoal_Biaya_Pemeriksaan: e.totalbiayapemeriksaan,
      Nama_Ruangan: e.namaruangan,
      Tanggal_Pelayanan: e.tanggal,    
    }
  })
