...
<template>
    <div class="columns is-multiline">
      <VCard style="padding-bottom: 0px">
        <div class="column c-title pt-2 mb-5">
          <label class="title-page">Laporan Kadaluarsa PerInstlasi (BMHP)</label>
        </div>
  ...
  useHead({
    title: 'Laporan Kadaluarsa BMHP (PerInstalasi) - ' + import.meta.env.VITE_PROJECT,
  })
  ...
  const fetchData = async () => {
    ...
    await useApi().get(`pelayanan/get-laporan-kadaluarsa-BMHP?${tglAwal}${tglAkhir}${ruangan}${dokter}${nama}`).then((response: any) => {
