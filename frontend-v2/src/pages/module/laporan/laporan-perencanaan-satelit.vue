...
<template>
    <div class="columns is-multiline">
      <VCard style="padding-bottom: 0px">
        <div class="column c-title pt-2 mb-5">
          <label class="title-page">Laporan Perencanaan Satelit</label>
        </div>
        <div class="column is-12">
              ...
              <div class="column is-8">
                <div class="columns is-multiline" style="justify-content: flex-end;">
                  <div class="column is-3">
                    <VField label="Ruangan" style="margin-bottom: 6px;" />
                      <VField>
          ...
          <Column field="no" header="#"></Column>
              ...
              <template #body="slotProps">
                <span v-if="slotProps.data.isfornas === true && slotProps.data.objectdetailjenisprodukfk === 2546">
                  <i class="pi pi-check-circle text-green-700 font-bold text-lg mr-1"></i>
                </span>
                <span v-else-if="(slotProps.data.isfornas === false || slotProps.data.isfornas === null) && slotProps.data.objectdetailjenisprodukfk === 2546"> 
                  -
                 </span>
                <span v-else>  </span>
              </template>
  ...
  useHead({
    title: 'Laporan Perencanaan Satelit - ' + import.meta.env.VITE_PROJECT,
  })
  ...
  const fetchData = async () => {
    ...
    await useApi().get(`pelayanan/get-laporan-perencanaan-Obat-2?${tglAwal}${tglAkhir}${ruangan}${nama}${status}`).then((response: any) => {
  ...
  const fetchRuangan = async (filter: any) => {
    await useApi().get(
        ...
        d_Ruangan.value = response.filter((item: any) => item.objectdepartemenfk === 14 && item.value !== 357 && item.value !== 344);
    });
}
  ...
  return dataSource.value.map(item => {
    ...
    const kebutuhan6bulan = Math.ceil(avgPemakaian * 6);  //kebutuhan 4 hari
    const rencanakebutuhan = Math.ceil(kebutuhan6bulan - item.total); 
    const minstok = Math.ceil(avgPemakaian * 4); 
    const total = Math.ceil(item.total); //
    const maxstock = Math.ceil(minstok + (6 * avgPemakaian));
