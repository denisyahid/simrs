            ...
            <div class="columns is-multiline">
              ...
              <div class="column is-12">
                <DataTable v-model:filters="filters" :value="dataSource2" paginator :rows="10" dataKey="id"
                  ...
                  <Column header="#" style="width:50px">
                      <template #body="slotProps">
                        <VIconButton type="button" icon="fas fa-eye" class="mr-3" :color="slotProps.data.id ? 'info':'danger'" circle outlined raised v-tooltip-prime="'Detail Data'" @click="detailData(slotProps.data,'Specimen')"
                          :loading="slotProps.data.isLoading">
                        </VIconButton>
                      </template>
                  </Column>
  ...
  for (let x = 0; x < z.length; x++) {
    ...
    if(element.category.length > 0 ){
        for (let y = 0; y < element.category.length; y++) {
          ...
          if(elementX.coding[0].display == 'Laboratory procedure'){
              element.no = x + 1
      ...
      if(element.category && element.category.coding[0].display == 'Laboratory procedure'){
            element.no = x + 1
            element.subjectdisplay = element.subject.display
            element.encounterdisplay = element.encounter.display
            element.codecodingcode = element.code.coding[0].code
