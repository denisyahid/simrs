...
<template>
  ...
  <VCard>
    <div class="columns column">
      <h3 class="title is-5 mb-2 mr-1">Daftar Pasien Belum Verif </h3> <span>({{ ds_PASIEN.total }} Totals)</span>
    </div>
          ...
          <DataTable :value="ds_PASIEN" class="p-datatable-md"
                  ...
                  <!-- <VButtons> -->
                    ...
                    <VTags>
                      ...
                      <VTag v-if="slotProps.data.isresep"
                        ...
                        curved
                      />
                      <VTag v-if="slotProps.data.noantrianfarmasi"
                        color="green"
                        :label="slotProps.data.noantrianfarmasi"
                        curved
                      />
                    </VTags>
                  <!-- </VButtons> -->
              ...
              <Column field="tanggalPulang" header="Tgl Pulang" :sortable="true" style="min-width: 80px">
                <template #body="slotProps">
                  ...
                  <span v-if="slotProps.data.tanggalPulang == null">
                    <VTags>
                      <VTag color="purple" label="Belum Pulang" curved/>
                    </VTags>
                  </span>
                </template>
              </Column>
              ...
              <Column field="totaltagihanfix" header="Total Tagihan" :sortable="true" style="min-width: 80px">
                <template #body="slotProps">
                  {{ H.formatRp(slotProps.data.totaltagihanfix, 'Rp. ') }}
                </template>
              </Column>
              <Column field="status" header="Status" :sortable="true" style="min-width: 80px">
                <template #body="slotProps">
                    <VTags>
                      <VTag
                        :color="slotProps.data.status.toLowerCase() == 'belum verifikasi' ? 'orange' : 'green'"
                      ...
                      <div class="columns">
                          <div class="column">
                            ...
                            <VButton type="button" icon="fa fa-history" class="is-fullwidth mr-3" color="info" outlined
                              ...
                              Open Bill </VButton>
                            <VButton type="button" icon="fas fa-notes-medical" class="is-fullwidth mr-3" color="primary" outlined
                              raised @click="noAntrianFarmasi(slotProps.data)">
                              Pasien Datang
                            </VButton>
...
async function fetchPasien() {
  ...
  // if(!H.cacheHelper().get('TglPulangEnd') || !H.cacheHelper().get('TglPulangStart')) {
    // H.cacheHelper().set('periode', item.value.periode)
    // H.cacheHelper().set('filterRuang', sourceRuangan.value);
    H.cacheHelper().set('filterItem', item.value);
    // H.cacheHelper().set('TglPulangEnd', item.value.periode.end)
  // }
...
async function verifikasi(e: any) {
  if (e.noRegistrasi == '') {
    H.alert('warning', 'Nomor registrasi pasien tidak ada!')
    return;
  }
...
const  noAntrianFarmasi = async (item: any) => {
    let objSave = {
      norec_pd: item.norec_pd,
      noregistrasi: item.noRegistrasi,
    }
    let setAntrian = await useApi().post(`farmasi/set-no-antrian-farmasi`, objSave);
    fetchPasien();
};
