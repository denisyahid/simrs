...
<template>
  ...
  <VCard>
    <div class="columns column">
      <h3 class="title is-5 mb-2 mr-1">Daftar Pasien Sudah Verif </h3> <span>({{ ds_PASIEN.total }} Totals)</span>
    </div>
      ...
      <div class="columns is-multiline  projects-card-grid" style="width: 100% !important">
        ...
        <div class="column px-0" style="width: 100%;">
          <DataTable :value="ds_PASIEN" class="p-datatable-md"
              ...
              <Column field="tanggalMasuk" header="Tgl Masuk" :sortable="true" style="min-width: 80px">
                <template #body="slotProps">
                  <span v-if="slotProps.data.tanggalMasuk != null">
                    {{ H.formatDate(slotProps.data.tanggalMasuk, 'DD-MM-YYYY HH:mm:ss') }}
                  </span>
                </template>
              </Column>
              <Column field="dokter" header="Dokter" style="min-width: 80px;"></Column>
              <Column field="tanggalPulang" header="Tgl Pulang" :sortable="true" style="min-width: 80px">
                <template #body="slotProps">
                  <span v-if="slotProps.data.tanggalPulang != null">
                    {{ H.formatDate(slotProps.data.tanggalPulang, 'DD-MM-YYYY HH:mm:ss') }}
                  </span>
                </template>
              </Column>
              <Column field="jenisAsuransi" header="Jenis Pasien" :sortable="true" style="min-width: 80px">
                <template #body="slotProps">
                  {{ slotProps.data.jenisAsuransi }} - <b>{{ slotProps.data.namarekanan }}</b>
                </template>
              </Column>
              <Column field="nosep" header="No SEP" :sortable="true" style="min-width: 80px">
                <template #body="slotProps">
                  {{ slotProps.data.nosep }} <br> 
                  <VTag
                    color="green"
                    :label="slotProps.data.kodingdiagnosa"
                    curved
                    v-if="slotProps.data.kodingdiagnosa != null"
                  />
                </template>
              </Column>
              ...
              <Column field="status" header="Status" :sortable="true" style="min-width: 80px">
                <template #body="slotProps">
                    <VTags>
                      <VTag
                        color="green"
                      ...
                      />
                      <VTag
                        :color="slotProps.data.statusclosing.toLowerCase() == 'sudah closing' ? 'info' : 'orange'"
                        :label="slotProps.data.statusclosing"
                        curved
                      />
                      <VTag v-if="slotProps.data.statuspiutang != '-'"
                        color="danger"
                        :label="slotProps.data.statuspiutang"
                        curved
                      />
                    </VTags>
                  <!-- </VButtons> -->
              ...
              <Column field="closer" header="Penerima" :sortable="true" style="min-width: 80px"></Column>
                      ...
                      <div class="columns">
                          <div class="column">
                            ...
                            <VButton type="button" icon="fa fa-history" class="is-fullwidth mr-3" color="info" outlined
                              ...
                              Open Bill </VButton>
                            <VButton type="button" icon="fa fa-history" class="is-fullwidth mr-3" color="danger" outlined
                              raised @click="changePayment(slotProps.data)" v-if="slotProps.data.status != 'Belum Verifikasi' && slotProps.data.jenisAsuransi != 'BPJS' && slotProps.data.totaltagihanfix > 0">
                              Ubah Cara Bayar </VButton>
...
async function fetchPasien() {
  ...
  const response = await useApi().get(
    ...
    }&tglAkhir=${moment(item.value.periode.end).format('YYYY-MM-DD')}&statusverifikasi=true&offset=${offset}&rows=${currentPage.value.rows}${ruanganid}${namaPasien}${queryKelompok}`)
...
async function openBill(e: any) {
  ...
  if( return_value === "rsbm") {
    isLoading.value = true
    useApi().get(`/kasir/check-pasien-active?nocmfk=${e.nocmfk}`).then(async(res) => {
      isLoading.value = false
      console.log(e)
      if(e.norec_sbm != null){
        await batalBayar(e)
...
const changePayment = (params: any) => {
  router.push({
    name: 'module-kasir-pembayaran-tagihan',
    query: {
      norec_sp: params.norec_sp,
      nocmfk: params.nocmfk ?? '',
      norec_pd: params.norec_pd,
      pageFrom: 'ubahCaraBayar'
    },
  })
}
