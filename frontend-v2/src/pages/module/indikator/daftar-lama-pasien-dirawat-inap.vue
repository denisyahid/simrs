            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Daftar Lama Hari Perawatan Pasien</label>
                    </div>
                </div>
                ...
                <DataTable v-else :rows="5" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <Column field="lamadirawat" header="Lama Perawatan" />
                </DataTable>
            </VCard>
        </div>
    </section>
...
useHead({
    title: 'Daftar Lama Hari Perawat Pasien - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
    ...
    await useApi().get(`pmkp/get-laporan-pasien-lama-dirawat?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}${ruangan}`).then((response) => {
...
const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ['Daftar Lama Hari Pasien Dirawat'],
        [],
        ['NO','TANGGAL REGISTRASI','NO REGISTRASI', 'NO CM', 'NAMA PASIEN', 'RUANGAN', 'TANGGAL PULANG','LAMA DIRAWAT'],
    ...
    const columnWidths = [5, 10, 15, 10, 20, 20,15];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Daftar Pasien', true);
    XLSXStyle.writeFile(workbook, 'Daftar Pasien Pulang Paksa.xlsx');
}
