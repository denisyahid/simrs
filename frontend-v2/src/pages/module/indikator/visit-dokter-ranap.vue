            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Daftar Visit Dokter Ranap</label>
                    </div>
                </div>
...
useHead({
    title: 'Daftar Visit Dokter Ranap - ' + import.meta.env.VITE_PROJECT,
})
...
const op = ref();
const title = ref('Daftar Visit Dokter Rawat Inap');
...
const fetchData = async () => {
    ...
    await useApi().get(`pmkp/get-laporan-visit-dokter-ranap?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}${ruangan}`).then((response) => {
...
const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ['Daftar Visit Dokter Rawat Inap'],
        [],
        ['NO', 'NO RM', 'NO REGISTRASI', 'TGL PELAYANAN','DOKTER','PELAYANAN','RUANGAN'],
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Dokter Visit Ranap', true);
    XLSXStyle.writeFile(workbook, 'Daftar Dokter Visit Rawat Inap.xlsx');
}
