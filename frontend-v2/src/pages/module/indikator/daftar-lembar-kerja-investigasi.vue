            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Daftar Lembar Kerja Investigasi Sederhana</label>
                    </div>
                </div>
                ...
                <DataTable v-else :rows="5" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <Column field="tanggalmulai" header="TGL Mulai">
                        <template #body="slotProps">
                            {{ H.formatDate(slotProps.data.tanggalmulai, 'YYYY-MM-DD') }}
                        </template>
                    </Column>
                    <Column field="tanggalakhir" header="TGL Selesai">
                        <template #body="slotProps">
                            {{ H.formatDate(slotProps.data.tanggalakhir, 'YYYY-MM-DD') }}
                        </template>
                    </Column>
                    <Column field="penyebabinsidenlangsung" header="Penyebab Insiden" />
                    <Column field="latarbelakanginsiden" header="Latar Belakang Insiden" />
                    <Column header="Action" style="text-align: center;">
                        <template #body="slotProps">
                            ...
                            <OverlayPanel ref="op">
                                ...
                                <VButton type="button" icon="fas fa-print" class="mr-2" color="purple" circle outlined raised>Cetak
                                </VButton>
                            </OverlayPanel>
                        </template>
                    </Column>
...
useHead({
    title: 'Daftar Lembar Kerja Investigasi - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
    ...
    await useApi().get(`pmkp/get-daftar-lembar-investigasi-sederhana?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}${ruangan}`).then((response) => {
...
const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ['Daftar Lembar Kerja Investigasi'],
        [],
        ['NO', 'NAMA PASIEN', 'RUANGAN', 'TGL MULAI', 'TGL SELESAI', 'PENYEBAB INSIDEN','LATAR BELAKANG INSIDEN'],
        ...dataSource.value.map((e: any) => [
            ...
            e.tanggalmulai,
            e.tanggalakhir,
            e.penyebabinsidenlangsung,
            e.latarbelakanginsiden,
        ]),
    ]);
    ...
    const columnWidths = [5, 18, 20, 15, 15, 20,25];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Lembar Kerja', true);
    XLSXStyle.writeFile(workbook, 'Daftar Lembar Kerja Investigasi.xlsx');
}
...
const deleteData = async (e: any) => {
    ...
    await useApi().post('/pmkp/hapus-lembar-kerja-investigasi', { 'norec': e.norec }).then((response) => {
        fetchData()
    }).catch((err: any) => {
