        ...
        <div class="column is-12">
            <VCard>
                ...
                <DataTable v-else :rows="5" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <Column field="tglmeninggal" header="TGL Meninggal" />
                </DataTable>
            </VCard>
        </div>
    </section>
...
useHead({
    title: 'Daftar Kematian Pasien < 24 Jam - ' + import.meta.env.VITE_PROJECT,
})
...
const op = ref();
const title = ref('Daftar Kematian Pasien < 24 Jam');
...
const fetchData = async () => {
    ...
    let ruangan = item.value.ruanganfk ? `&ruanganfk=${item.value.ruanganfk}` : ''

    item.value.Ttotal = 0
    loadSearch.value = true
    await useApi().get(`pmkp/get-laporan-kematian-pasien-igd?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}${ruangan}`).then((response) => {
...
const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ['Daftar Kematian Pasien 24 Jam'],
        [],
        ['NO', 'NO CM', 'NO REGISTRASI', 'TGL REGISTRASI', 'NAMA PASIEN', 'NAMA RUANGAN','TGL MENINGGAL'],
    ...
    const columnWidths = [5, 13, 15, 13, 20, 20,13];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Laporan Pasien Meninggal', true);
    XLSXStyle.writeFile(workbook, 'Laporan IGD Pasien Meninggal.xlsx');
}
