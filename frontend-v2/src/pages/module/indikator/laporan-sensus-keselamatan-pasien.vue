            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Laporan Sensus Keselamatan Pasien</label>
                    </div>
                </div>
                ...
                <DataTable v-else :rows="5" :value="dataSource" :loading="loadSearch" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <Column field="tglinsiden" header="Kejadian">
                        <template #body="slotProps">
                            {{ H.formatDate(slotProps.data.tglinsiden, 'DD/MM/YYYY') }}
                        </template>
                    </Column>
                    <Column field="noregistrasi" header="Registrasi" />
                    <Column field="insiden" header="Insiden" />
                    <Column field="namaruangan" header="Ruangan" />
                    <Column field="sentinel" header="Sentinel" />
                    <Column field="ktd" header="KTD" />
                    <Column field="ktc" header="KTC" />
                    <Column field="knc" header="KNC" />
                    <Column field="kpc" header="KPC" />
                    <Column field="regrading" header="Grading" />
                        ...
                        <template #body="slotProps">
                            ...
                            <OverlayPanel ref="op">
                                ...
                                <VButton type="button" icon="fas fa-print" class="mr-2" color="purple" circle outlined
                                    @click="cetakLabel(selected)" raised>Cetak
                                </VButton>
                            </OverlayPanel>
                        </template>
                    </Column>
...
const fetchData = async () => {
    ...
    await useApi().get(`pmkp/get-daftar-sensus-keselamatan-pasien-bulanan?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}${ruangan}`).then((response) => {
