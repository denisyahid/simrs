            ...
            <VCard>
                <div class="column c-title pt-2 mb-0">
                    <div class="column is-10 p-0">
                        <label class="title-page">Daftar Penanganan Keluhan Pelanggan</label>
                    </div>
                </div>
                                ...
                                <div class="columns is-multiline" style="justify-content: right;">
                                    ...
                                    <div class="column is-2">
                                        <VField label="Pencarian">
                                            <VControl>
                                                <VInput type="text" class="input" v-model="item.pencarian"/>
                                            </VControl>
                                        </VField>
                                    </div> -->
                    ...
                    <Column header="Action">
                        <template #body="slotProps">
                            ...
                            <OverlayPanel ref="op">
                                ...
                                <VButton type="button" icon="fas fa-print" class="mr-2" color="black" circle outlined
                                    @click="gotoPageTidakan(selected)" raised>Tindak Lanjut
                                </VButton>
...
useHead({
    title: 'Daftar Penanganan Keluhan Pelanggan - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
    ...
    // let pencarian = item.value.pencarian ? `&pencarian=${item.value.pencarian}` : ''

    loadSearch.value = true
    await useApi().get(`pmkp/get-daftar-penanganan-keluhan?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}${ruangan}${pencarian}`).then((response) => {
...
const gotoPageTidakan = (e:any,info)=>{
     router.push({
        ...
        query: {
            ...
            norec_pd: e.noregistrasifk,
            norec_lii: e.norec,
            norec_lk: e.norec_lk ? e.norec_lk : '',
        },
    })
}
