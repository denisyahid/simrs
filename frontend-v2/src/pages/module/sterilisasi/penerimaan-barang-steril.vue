            ...
            <div class="columns is-multiline">
              ...
              <div class="column is-3">
                <VField class="is-rounded-select is-autocomplete-select required-vfield" label="Ruangan pengirim">
                  <VControl icon="feather:search" class="prime-auto-select">
                    <AutoComplete v-model="item.ruanganAsal" :suggestions="d_RuanganAsal"
                      @complete="fetchRuangan($event)" :optionLabel="'label'" :dropdown="true" :minLength="3"
                      :appendTo="'body'" :loadingIcon="'pi pi-spinner'" class="is-rounded" :field="'label'"
              ...
              <div class="column is-3">
                <VField class="is-rounded-select is-autocomplete-select required-vfield" label="Ruangan tujuan">
                  <VControl icon="feather:search" class="prime-auto-select">
                    <AutoComplete v-model="item.ruanganTujuan" :suggestions="d_RuanganTujuan"
                      @complete="dropdown($event)" :optionLabel="'label'" :dropdown="true" :minLength="3"
                      :appendTo="'body'" :disabled="disabledRuangan" :loadingIcon="'pi pi-spinner'" class="is-rounded"
                      :field="'label'" placeholder=" Ruangan Tujuan" />
                  </VControl>
                </VField>
              </div>
              <div class="column is-3">
...
const TITLE_PAGE = "Penerimaan Barang Instalasi CSSD"
useHead({
...
const dropdown = async (filter: any) => {
  await useApi().get(`/stelilisasi/combo?namaruangan=${filter.query}`).then((response: any) => {
    d_RuanganTujuan.value = response.ruangan.map((element: any) => {
      return {
        label: element.namaruangan,
        value: element.id
      }
    })
    if (d_RuanganTujuan.value.length == 1) {
      item.ruanganTujuan = { label: d_RuanganTujuan.value[0].label, value: d_RuanganTujuan.value[0].value }
    }
  })
}
