    ...
    <section>
                ...
                <div class="search-widget">
                    <div class="field">
                        <div class="columns is-multiline">
                            ...
                            <div class="column is-1 is-flex" style="align-items: center;">
                                <VField label="All Periode" class="text-muted px-0">
                                    <VControl>
                                        <VSwitchBlock v-model="isAll" color="success" />
                                    </VControl>
                                </VField>
                            </div>
                        </div>
            ...
            <div class="column is-12">
                ...
                <div class="columns is-multiline" v-else>
                    <div class="column is-12" v-if="dataSource.length">
                        <div class="columns is-multiline">
                            <div class="column is-12">
                                <BerkasPasienView :data="dataSource" @edit="edit" @hapus="hapus" @lihat="lihat"
                                    :hide="false"></BerkasPasienView>
                            </div>
                        </div>
                    </div>
        ...
        <Dialog v-model:visible="modalInput" modal header="Upload" :style="{ width: '30vw' }">
            <div class="columns is-multiline">
                ...
                <div class="column is-12">
                    <VField class="is-rounded-select is-autocomplete-select
                        ...
                        <VControl icon="fas fa-sticky-note" fullwidth class="prime-auto-select">
                            <Dropdown v-model="input.namafile" :options="d_Berkas" :optionLabel="'nama'"
                ...
                <div class="column is-12">
                    <!-- <FileUpload v-model="filePasien" mode="basic" name="demo" accept="application/pdf"
                    ...
                    <FileUpload v-model="filePasien" mode="basic" name="demo" @upload="onUpload" outlined
                        style=" background-color: transparent; color: var(--danger); border: 1px solid;"
                        :chooseLabel="filePasien ? filePasien.name : 'Unggah'" @select="onSelect($event)"
                        class="is-rounded w-100" />
                </div>
...
const loadDrop = () => {
    useApi().get(
        `/emr/combo-jenis-berkas?type=${props.type}`).then((response: any) => {
...
const loadRiwayat = () => {
    ...
    useApi().get(`/emr/berkas-pasien?nocm=${props.pasien.nocm}&noregistrasi=${props.registrasi.noregistrasi}&isAll=${isAll.value}`).then((response: any) => {
        isLoading.value = false
        dataSource.value = response.data
    })
    // useApi().get(
    //     `/emr/berkas-pasien?nocm=${props.pasien.nocm}&type=${props.type}`).then((response: any) => {
    //         isLoading.value = false
    //         dataSource.value = response.data
    //     })
}
...
const simpan = async () => {
    console.log('NAMA FILE', input.value.namafile)
    ...
    formData.append('isklaim', input.value.namafile.isklaim)
    formData.append('objectjenisdokumenfk', input.value.namafile.id)
