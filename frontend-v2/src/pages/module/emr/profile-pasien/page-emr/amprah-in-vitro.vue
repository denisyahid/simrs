              ...
              <div class="right">
                <div class="buttons">
                  <!-- <h3 color="info" v-if="isPenunjangSusulan != null" bold
                    ...
                    Lab Berbeda Hari: {{ H.formatDate(isPenunjangSusulan, 'DD-MM-YYYY') }}</h3> -->
                                  ...
                                  <div class="columns is-multiline">
                                    <div class="column is-12">
                                      <TRiwayatOrderLab title="" straight class="list-widget-v3" :items="listRiwayat"
                                        @editItems="editItems" @hasilItems="printHasil2" @hapusItems="DialogConfirm"
                                        @hasilmikro="hasilcetakmikro" @hasillabpa="printHasilLabPA" squared colored>
                                      </TRiwayatOrderLab>
                                    </div>
                                  ...
                                  <div class="columns is-multiline">
                                    <div class="column is-12">
                                      <TRiwayatOrderLabAll title="" straight class="list-widget-v3"
                                        :items="listRiwayatAll" @editItems="editItems" @hasilItems="printHasil2"
                                        @hapusItems="DialogConfirm" @hasilmikro="hasilcetakmikro"
                                        @hasillabpa="printHasilLabPA" squared colored>
                                      </TRiwayatOrderLabAll>
                                    </div>
                                  </div>
                                </div>
...
const tabs: any = ref([
  ...
  { label: 'Riwayat Order Penunjang Pasien', value: 3, icon: 'fas fa-list' }
])
...
const fetchPaket = async () => {
  ...
  await useApi().get(
    `/tindakan/list-paket?flag=lab`).then((response: any) => {
...
const loadRiwayatAll = () => {
  ...
  useApi().get(
    `/laboratorium/riwayat-order?nocmfk=${ID_PASIEN}`).then((response: any) => {
      let z = 0
      for (let x = 0; x < response.length; x++) {
        const element = response[x];
        element.icon = 'lnir lnir-flask-alt'
...
const fetchTindakan = (e: any) => {
  ...
  console.log(pasien)
  useApi().get(
    `/laboratorium/list-tindakan-for-order?ruanganfk=${e}&idkebangsaan=${pasien.value.objectkebangsaanfk}&kelasfk=${item.registrasi.objectkelasfk}&isruangan=true`).then((response: any) => {
...
const simpan = async () => {
  ...
  if ((item.produkCeklis == undefined || item.produkCeklis.length == 0) && item.ruanganTujuan != 336) {
    H.alert('error', 'Pilih layanan terlebih dahulu')
    return
  }
  if (!item.keterangan && item.produkCeklis.length == 0 && item.ruanganTujuan == 336) {
...
const lanjutsimpan = () => {
  ...
  // console.log(item.pilihRegistrasi)
    ...
    for (var x = 0; x < arrobj.length; x++) {
      ...
      if (item.produkCeklis[parseInt(element)] == true) {
        for (var i = 0; i < d_ProdukDef.value.length; i++) {
          ...
          // if(item.ruanganTujuan == 302){
          //   if(bb.length > 0 && item.ss[element2.id] !== '') {
          //     console.log('masuk element if')
          ...
          //     }
          //   }else {
          //     console.log('masuk element else')
          //     // if(list)
          //     item.ss[element2.id] = 1;
          ...
          // for (let b = 0; b < bb.length; b++) {
          //     const elementx = bb[b];
          //     const val = item.ss[elementx];

          //     if (elementx == element2.id) {
          //         const newValue = val + ' ' + element2.namaproduk;

          //         if (item.keterangan) {
          //             if (!item.keterangan.includes(newValue)) {
          //                 item.keterangan += ', ' + newValue;
          //             }
          //         } else {
          //             item.keterangan = newValue;
          //         }
        ...
        for (var i = 0; i < d_ProdukDef.value.length; i++) {
          ...
          if (element2.id == element) {
            for (var z = 0; z < listChecked.value.length; z++) {
              ...
              // if(item.ss[element2.id]) {
              //   delete item.ss[element2.id];
              // }
            }
          }
        }
...
watch(() => item.ruanganTujuan, (newValue) => {
  if (newValue == 302) {
    // console.log('masuk kondisi watch')
