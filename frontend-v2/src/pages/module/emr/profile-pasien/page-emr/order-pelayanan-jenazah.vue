...
<template>
  ...
  useHead({
      title: 'Order Pelayanan Pemulasaraan Jenazah - ' + import.meta.env.VITE_PROJECT,
  })
  ...
  const loadRiwayat = () => {
      ...
      useApi().get(
          `/jenazah/riwayat-jenazah-order?nocmfk=${ID_PASIEN}&nocm=${nocm}&norec_pd=${item.NOREC_PD}&noregistrasi=${noregistrasi}`).then((response: any) => {
              let z = 0
  ...
  const fetchDropdown = async() => {
      useApi().get(
          `/jenazah/list-ruang-jenazah`).then(async(response: any) => {
  ...
  const fetchTindakan = async (e: any) => {
      ...
      await useApi().get(
          `/jenazah/list-tindakan-jenazah?ruanganfk=${e}`).then((response: any) => {
  ...
  const simpan = async () => {
    ...
    isLoading.value = true
    await useApi().get(`/jenazah/riwayat-jenazah-order?nocmfk=${ID_PASIEN}&norec_pd=${item.NOREC_PD}`).then((response: any) => {
        isLoading.value = false
        ...
        if(udahorder) {
            confirm.require({
                message: 'Order Pelayanan Jenazah sudah dilakukan, Apakah ingin order lagi ?',
                header: 'Konfirmasi Pelayanan Jenazah',
...
const lanjutsimpan = () => {
  ...
  const editItems = async (e: any) => {
      ...
      useApi().get(
          `/jenazah/detail-order-pel?norec=${e.norec}`).then((response: any) => {
              for (let x = 0; x < response.length; x++) {
                  const element = response[x];
