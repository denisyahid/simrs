...
useHead({
    title: 'Surat Keterangan Hasil Pemeriksaan Mata - ' + import.meta.env.VITE_PROJECT,
})
...
const COLLECTION: any = ref('SuratKeteranganHasilPemeriksaanMata') //table mongodb
...
const loadRiwayat = async () => {
    ...
    if (response.length) {
        ...
        input.value.TBSUmur = calculateAge(dataPasien.tgllahir)
        input.value.TBJenisKelamin = dataPasien.jeniskelamin
        input.value.TAAlamat = dataPasien.alamatlengkap
        input.value.TBTempat = dataPasien.tempatlahir
    }
}
const simpan = () => {
    ...
    let json = {
        ...
        'url_form': route.name,
        'name_form': 'Surat Keterangan Hasil Pemeriksaan Mata',
...
const getDataLokalisMata = async () => {
    await useApi().get(`emr/get-data-lokalis-mata?nocmfk=${ID_PASIEN}`).then((response) => {
        if (response != null) {
            input.value.TBvod = [
                response.visusawalodu ?? "", 
                response.visusawalodb ?? "", 
                response.kacamataodu ?? "", 
                response.kacamataodb ?? ""
            ].filter(Boolean).join(" ");
            input.value.TBvos = [
                response.visusawalosu ?? "",
                response.visusawalosb ?? "",
                response.kacamataosu ?? "",
                response.kacamataosb ?? ""
            ].filter(Boolean).join(" ");
            input.value.TAKesimpulan = response.diagnosaIcd10?.label || "";
            ...
            <div class="form-outer" style="margin-top:15px">
                <div :class="[isStuck && 'is-stuck']" class="form-header stuck-header">
                    <div class="form-header-inner">
                        <div class="left">
                            <h3>Surat Keterangan Hasil Pemeriksaan Mata</h3>
                        </div>
                        <div class="right">
                    ...
                    <div class="column is-6">
                        <div class="column columns is-multiline">
                            ...
                            <div class="column is-8">
                                <VField addons>
                                    <VControl>
                                        <VInput type="number" class="input" v-model="input.TBSUmur" />
                                    </VControl>
                ...
                <div class="columns is-multiline column is-12">
                    <div class="column is-6">
                        <div class="column columns is-multiline">
                            <div class="column is-3">V.O.D</div>
                            <div class="column is-9">
                                <VControl>
                                    <VInput type="text" class="input" v-model="input.TBvod" />
                                </VControl>
                            </div>
                            <div class="column is-6">
                                <VField label="Segmen Anterior OD">
                                    <VControl>
                                        <VInput type="text" class="input" v-model="input.TBSAod" />
                                    </VControl>
                                </VField>
                            </div>
                            <div class="column is-6">
                                <VField label="Segmen Posterior OD">
                                    <VControl>
                                        <VInput type="text" class="input" v-model="input.TBSPod" />
                                    </VControl>
                                </VField>
                            </div>

                            <div class="column is-3">V.O.S</div>
                            <div class="column is-9">
                                <VControl>
                                    <VInput type="text" class="input" v-model="input.TBvos" />
                                </VControl>
                            </div>
                            <div class="column is-6">
                                <VField label="Segmen Anterior OS">
                                    <VControl>
                                        <VInput type="text" class="input" v-model="input.TBSAos" />
                                    </VControl>
                                </VField>
                            </div>
                            <div class="column is-6">
                                <VField label="Segmen Posterior OS">
                                    <VControl>
                                        <VInput type="text" class="input" v-model="input.TBSPos" />
                                    </VControl>
                                </VField>
                            </div>

                            <div class="column is-3">Buta Warna</div>
                            <div class="column is-9">
                                <VControl>
