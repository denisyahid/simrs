...
const setAutoFill = () => {
  input.value.namaOperasi = 'SCTP';
  input.value.laporan += 'Pemberian anestesi umum dengan intubasi endotrakeal\n'
  input.value.laporan += 'Pasang doeloe steril\n'
  input.value.laporan += 'Insisi Pfannenstiel kurang lebih 10 cm hingga peritoneum\n'
  input.value.laporan += 'Tampak uterus gravid, dilakukan insisi SBR\n'
  input.value.laporan += 'Meluksir kepala, pukul           WIB lahir bayi               ,                 gram, AS =                 , segera menangis\n'
  input.value.laporan += 'Plasenta lahir komplit\n'
  input.value.laporan += 'Jahit uterus, one layer, dilakukan retroperitonealisasi\n'
  input.value.laporan += 'Evaluasi: Perdarahan (-), organ genitalia dalam batas normal\n'
  input.value.laporan += 'Cuci cavum abdomen dengan NaCl 0,9% hingga jernih\n'
  input.value.laporan += 'Tutup dinding abdomen lapis demi lapis\n'
  input.value.laporan += 'Operasi selesai\n'
