    ...
    <div class="form-outer" style="margin-top:15px">
      <div :class="[isStuck && 'is-stuck']" class="form-header stuck-header">
        <div class="form-header-inner">
          <div class="left">
            <h3> Catatan Kegiatan Radioterapi<span v-if="isStuck"></span></h3>
          </div>
          <div class="right">
...
function copyTTV() {
  ...
  if (data.tinggiBadan != null) {
    ttv += `Tinggi Badan : ${data.tinggiBadan}\n`
  }
  if (data.beratBadan != null) {
    ttv += `Berat Badan : ${data.beratBadan}\n`
  }
...
const loadRiwayat = async () => {
  try {
    ...
    const response_LP = await useApi().get("emr/auto-fill?nocmfk=" + ID_PASIEN + "&norec_pd=" + NOREC_PD + "&collection=FormulirLembaranPenyiaranRadioterapi" + `&field=teknikPenyinaran`)
    if (response.data.length) {
      let d = response.data[0]
      input.value.keadaanumumobgyn =d.keadaanumumobgyn
      input.value.gcse = d.gcse
      input.value.gcsv = d.gcsv
      input.value.gcsm = d.gcsm
      input.value.tekananDarah = d.tekananDarah
      input.value.nadi = d.nadi
      input.value.nafas = d.nafas
      input.value.celcius = d.celcius
      input.value.sao2 = d.sao2
      input.value.tinggiBadan = d.tinggiBadan
      input.value.beratBadan = d.beratBadan
      if (response.dataToday.length && user == 'dokter') {
        input.value.details[0].keluhan = response.dataToday[0].details[0].keluhan
      }
      if (response_LP != null) {
        input.value.details[0].tindakan = `Radiasi : ${response_LP.teknikPenyinaran}\n`
      }
      // input.value.details[0].keteranganCatatan = `Fraksi Ke :\n`
      // input.value.keadaanumumobgyn = response[0].keadaanumumobgyn ? response[0].keadaanumumobgyn : ''
      // input.value.gcse = response[0].gcse ? response[0].gcse : ''
      // input.value.gcsv = response[0].gcsv ? response[0].gcsv : ''
      // input.value.gcsm = response[0].gcsm ? response[0].gcsm : ''
      // input.value.tekananDarah = response[0].tekananDarah ? response[0].tekananDarah : ''
      // input.value.nadi = response[0].nadi ? response[0].nadi : ''
      // input.value.nafas = response[0].nafas ? response[0].nafas : ''
      // input.value.celcius = response[0].celcius ? response[0].celcius : ''
      // input.value.sao2 = response[0].sao2 ? response[0].sao2 : ''
...
const simpanCatatan = () => {
  ...
  // Validasi
  if (!input.value.details[0].keluhan || input.value.details[0].keluhan && input.value.details[0].keluhan.replace(/\s/g, "").length < 4) {
...
useHead({ title: 'Catatan Kegiatan Radioterapi' + ' - ' + import.meta.env.VITE_PROJECT })
