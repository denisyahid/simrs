<template>
                        ...
                        <td class="td-pri" style="vertical-align: middle;">
                          <VButtons>
                            ...
                            <VIconButton type="button" raised circle v-if="index > 0" v-tooltip-prime.bottom="'Hapus'"
                              outlined icon="feather:trash" @click="removePelaksanaan(index)" color="danger">
                            </VIconButton>
                          </VButtons>
                        </td>
                      </tr>
  ...
  import { h, reactive, ref, computed, watch, onMounted, nextTick } from 'vue'
  ...
  const loadRiwayat = async () => {
    try {
        const response: any = await useApi().get(`/emr/get-emr?nocmfk=${ID_PASIEN}&norec_pd=${props.registrasi.norec_pd}&collection=${COLLECTION.value}&index_tabs=${route.params.index_tabs}`);
        ...
        if (response.length) {
            ...
            // Tambahkan default objek jika array kosong
            if (!Array.isArray(input.value.detailPelaksanaan) || input.value.detailPelaksanaan.length === 0) {
                input.value.detailPelaksanaan = [{ no: null }];
            }
            if (!Array.isArray(input.value.detailObatResep) || input.value.detailObatResep.length === 0) {
                input.value.detailObatResep = [{ no: null }];
            }
  ...
  const loadRiwayatSM = async () => {
    try {
      ...
      if (response.length > 0) {
        // Get the last details entry (highest index)
        const lastDetailIndex = response[0].details.length - 1;
        const skalaMorse = response[0].details[lastDetailIndex].TotalSM;
...
watch(
    ...
    (newValue, oldValue) => {
        input.value = {}
        input.value.detailPelaksanaan = [{ no: 1 }]
        input.value.detailObatResep = [{ no: 1 }]
