<template>
  ...
  <VModal :open="showModalTemplateFix" title="Template" :noclose="true" size="large" actions="right"
    ...
    <template #content>
      <DataTable :pt="{
        table: { style: 'min-width: 80rem; min-height: 10rem;' },
        column: {
          bodycell: ({ state }) => ({
        ...
        tableStyle="min-width: 80rem" dataKey="no" :totalRecords="listTemplateFix.length"
...
const simpan = () => {
  ...
  //   H.alert("warning","Data input EMR tidak valid");
  //   return;
  // }
  ...
  input.value.pasien = H.setObjectPasien(props.pasien)
  input.value.registrasi = H.setObjectRegistrasi(props.registrasi)
  for (let i = 1; i <= 5; i++) {
    const ttdPerawat1 = `TTDperawat-${i}`
