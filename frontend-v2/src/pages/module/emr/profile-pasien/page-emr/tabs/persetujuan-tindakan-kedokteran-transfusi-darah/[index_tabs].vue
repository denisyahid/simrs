    ...
    <div class="form-outer" style="margin-top:15px">
      ...
      <div class="column">
        ...
        <div class="column">
          ...
          <table class="table is-bordered m-0" style="width: 100% !important;">
            ...
            <tr>
              ...
              <th style="width: 70%;">
                ...
                <div class="column is-12">
                  <div class="columns is-multiline">
                      <div class="column is-4 pt-0" v-for="(item, index) in input.details" :key="index">
                        <VControl>
                          <VInput type="text" class="input" v-model="item.TBPemberiPersetujuan" />
                        </VControl>
                      </div>
                  </div>
                </div>
          ...
          <div class="is-12">
            <VButton type="button" rounded outlined color="primary" raised icon="feather:folder" @click="indentitasPasien1()">
              Identitas Pasien
            </VButton>
          </div>
          ...
          <div class="is-12">
            ...
            <h1 style="font-weight: bold;">Terhadap,</h1>
            <VControl class="mt-1 mb-1" style="width: 20%;">
                <VInput type="text" class="input" v-model="input.terhadap" />
            </VControl>
          </div>

          <div class="is-12">
            <VButton type="button" rounded outlined color="primary" raised icon="feather:folder" @click="indentitasPasien2()">
              Identitas Pasien
            </VButton>
          </div>
          ...
          <p>I Have Fully Understand the need and the benefits of these action which has been explained to
            me,
            including the
            risks and any
              ...
              <tbody v-for="resep in listTemplateFix">
                ...
                <tr>
                  ...
                  <td style="width:15%;text-align:center;border:1px solid black;vertical-align: middle;padding: 3px;">
                    ...
                    <VIconButton type="button" raised circle icon="fas fa-trash" @click="hapusTemplate(resep.id)" color="danger"
        ...
        <tr>
          <td style="padding:7px;padding-bottom:0px;text-align:center">
            <p style="font-size:large;">
              Apakah anda ingin mengambil data bedah ?
            </p>
          </td>
        </tr>
      </table>
...
useHead({ title: 'Persetujuan Tindakan Kedokteran Transfusi Darah - ' + import.meta.env.VITE_PROJECT })
...
const COLLECTION: any = ref('PersetujuanTindakanKedokteranTransfusiDarah') //table mongodb
...
const loadRiwayat = async () => {
  ...
  await useApi().get(`/emr/get-emr?nocmfk=${ID_PASIEN}&index_tabs=${route.params.index_tabs}&norec_pd=${NOREC_PD}&collection=${COLLECTION.value}&emrpasienfk=${NOREC_EMRPASIEN.value}`).then(async (response: any) => {
    if (response.length) {
      input.value = response[0] //set ke inputan
      if (NOREC_EMRPASIEN.value == '') {
        NOREC_EMRPASIEN.value = response[0].emrpasienfk
      }
      if (!response[0]?.details || response[0].details.length === 0) {
          input.value.details = [
              {
                  NO: 1
              }
      ...
      confirm.require({
        header: 'Pengambilan Data Bedah',
        group: 'confirm',
        icon: 'pi pi-info-circle',
        acceptClass: 'p-button-danger',
        accept: async () => { await setAutoFillBedah() },
        reject: () => { },
      })
...
const setAutoFillBedah = async () => {
  let d = input.value;

  await useApi().get(`/emr/get-order-bedah?nocmfk=${ID_PASIEN}`).then((res: any) => {
    if (res) {
      d.pemeriksaantextDiagnosisWD = res.diagnosis;
      d.pemeriksaantextTindakanKedokteran = res.keteranganlainnya;
    }
  })
...
const simpan = () => {
  ...
  let json = {
    ...
    'url_form': route.name,
    'name_form': 'Persetujuan Tindakan Kedokteran Transfusi Darah',
...
let klikIdentitasPasien1 = 0;
let klikIdentitasPasien2 = 0;

const indentitasPasien1 = () => {
  klikIdentitasPasien1++;
  if (klikIdentitasPasien1 >= 2) {
    input.value.namaPasien = '';
    input.value.umurPasien = '';
    input.value.jeniskelamin = '';
    input.value.alamat = '';
    klikIdentitasPasien1 = 0;
  } else {
...
const indentitasPasien2 = () => {
  klikIdentitasPasien2++;
  if (klikIdentitasPasien2 >= 2) {
    input.value.namaPasienTerhadap = '';
    input.value.umurPasienTerhadap = '';
    input.value.jeniskelaminTerhadap = '';
    input.value.alamatTerhadap = '';
    klikIdentitasPasien2 = 0;
  } else {
...
const hapusTemplate = (response: any) => {
  isLoading.value = true
  let json = {
    'id': response,
    'collection': COLLECTION.value
