      ...
      <div class="form-outer" style="margin-top:15px">
        <div :class="[isStuck && 'is-stuck']" class="form-header stuck-header">
          <div class="form-header-inner">
            <div class="left">
              <h3>Asesmen Awal Keperawatan Neonatus {{ route.params.index_tabs }}</h3>
            </div>
            <div class="right">
        ...
        <div class="column">
          ...
          <Dialog v-model:visible="modalRencanaPP" maximizable modal header="" :style="{ width: '70vw' }">
            <RencanaPP :nocmfk="props.registrasi.nocmfk" :norec_pd="props.registrasi.norec_pd" :pasien="props.pasien"
              :registrasi="props.registrasi" />
            <template #footer>
              <VButton icon="lnir lnir-arrow-left rem-100" light dark-outlined
                @click="modalRencanaPP = false; isLoading = false">
                Tutup
              </VButton>
            </template>
          </Dialog>
          ...
          <div class="column is-12 forCB">
            ...
            <MultiSelect v-model="input.diagnosaSelected" :options="diagnosesOptions" optionLabel="text"
              placeholder="Pilih Diagnosa" display="chip" class="w-full" filter @change="updateDiagnosaInput($event)">
              ...
              <template #footer>
                <div class="py-2 px-3">
                  <b>{{ input.diagnosaSelected ? input.diagnosaSelected.length : 0 }}</b> item{{ (input.diagnosaSelected ?
                    input.diagnosaSelected.length : 0) > 1 ? 's' : '' }} selected.
                </div>
              </template>
            </MultiSelect>
            <div class="control" style="margin-left: 10px;margin-top:10px;"
              v-for="(diagselected, indexDiag) in input.diagnosaSelected" :key="indexDiag">
              <span v-if="diagselected.isShow && diagselected.value != 'lainnya1'"> {{ indexDiag + 1 + '. ' +
                diagselected.text
                }} </span>
    ...
    input.diagnosaSelected = [];
      for (let iDiag = 0; iDiag < diagnosesOptions.length; iDiag++) {
        ...
        if(input.value[elDiag.value] != undefined && input.value[elDiag.value] != null) {
          elDiag.isShow = true;
          input.diagnosaSelected.push(elDiag);
        }
      }
  } else {
...
const updateDiagnosaInput = (event) => {
  console.log('diagnosa item', input.diagnosaSelected);
