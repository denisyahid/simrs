...
import * as EMR from '../page-emr-plugins/profil-ringkasan-medis-rj'
...
// Judul
useHead({
    title: 'Profil Ringkasan Medis Rawat Jalan - ' + import.meta.env.VITE_PROJECT,
})
...
const COLLECTION: any = ref('ProfilRingkasanMedisRJ') //table mongodb
...
const fetchDiagnosa = async (filter: any) => {
    await useApi().get(
        `emr/dropdown/diagnosa_m?select=kddiagnosa,namadiagnosa&param_search=kddiagnosa&limit=10`
    ).then((response) => {
...
const fetchRuangan = async (filter: any) => {
    await useApi().get(
        `emr/dropdown/ruangan_m?select=id,namaruangan&param_search=namaruangan&where=namaexternal.eq.RAJAL&limit=10`
    ).then((response) => {
            ...
            <div class="form-outer" style="margin-top:15px">
                <div :class="[isStuck && 'is-stuck']" class="form-header stuck-header" style="margin-bottom: 10px">
                    <div class="form-header-inner">
                        <div class="left">
                            <h3>Profil Ringkasan Medis Rawat Jalan</h3>
                        </div>
                        <div class="right">
                ...
                <div class="columns is-multiline" style="padding: 10px;">
                    <div class="column is-12">
                        <h1>Ringkasan Pasien Sesuai SOAP : </h1>
                        <VField>
                            <VTextarea rows="2" v-model="input.TArpsSOAP"></VTextarea>
                        </VField>
                    </div>
                    <div class="column is-8"></div>
                    <div class="column is-4">
                        <div class="column" style="text-align:center;">
                            <h1>Nama dan Tanda Tangan DPJP</h1>
                            <TandaTangan :elemenID="'TTDdpjp'" :width="'150'" :height="'150'" class="dek" />
                            <VControl class="prime-auto">
                                <AutoComplete v-model="input.DKonselor" :suggestions="d_Dokter"
                ...
                <div class="column is-12">
                    <table class="tg">
                        <thead>
                            <th style="text-align: center;vertical-align: middle;">Hari, Tanggal/Jam</th>
                            <th style="text-align: center;vertical-align: middle;">Poliklinik</th>
                            <th style="text-align: center;vertical-align: middle;">Dokter Penanggung Jawab(DPJP)</th>
                            <th style="text-align: center;vertical-align: middle;">Diagnosa</th>
                            <th style="text-align: center;vertical-align: middle;">Rencana dan Tindak Lanjut</th>
                        </thead>
                        <tbody>
                            <tr v-for="(row, rowIndex) in detailTable" :key="rowIndex">
                                <td width="200px" v-for="(item, itemIndex) in row.child" :key="itemIndex">
                                    <VField style="padding:0px 10px;" v-if="item.type == 'datetime'">
                                        <VDatePicker v-model="input['DTTable_' + rowIndex + '_' + itemIndex]"
                                    ...
                                    <VField v-if="item.type == 'textarea'">
                                        <VTextarea rows="2" v-model="input['TATable_' + rowIndex + '_' + itemIndex]">
                                        </VTextarea>
                                    </VField>
                                    <VField v-if="item.type == 'dokter'">
                                        <VControl class="prime-auto">
                                            <AutoComplete v-model="input['DDokter_' + rowIndex + '_' + itemIndex]"
                                    ...
                                    <VControl class="prime-auto" v-if="item.type == 'diagnosa'">
                                        <AutoComplete v-model="input['DDiagnosa_' + rowIndex + '_' + itemIndex]"
                                            :suggestions="d_DiagnosaICD10" @complete="fetchDiagnosa($event)"
                                            :optionLabel="'label'" :dropdown="true" :minLength="3" :appendTo="'body'"
                                            :loadingIcon="'pi pi-spinner'" :field="'label'" class="mt-2" />
                                    </VControl>
                                    <VField v-if="item.type == 'ruangan'">
                                        <VControl class="prime-auto">
                                            <AutoComplete v-model="input['DRuangan_' + rowIndex + '_' + itemIndex]"
                                                :suggestions="d_Ruangan" @complete="fetchRuangan($event)"
                                                :optionLabel="'label'" :dropdown="true" :minLength="3"
                                                :appendTo="'body'" :loadingIcon="'pi pi-spinner'" :field="'label'"
                                                class="mt-2" />
                                        </VControl>
