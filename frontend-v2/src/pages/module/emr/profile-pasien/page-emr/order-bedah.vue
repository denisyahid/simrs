                                               ...
                                               <div class="column is-12">
                                                    ...
                                                    <template #default="{ inputValue, inputEvents }">
                                                        <VField>
                                                            <VControl icon="feather:calendar" fullwidth>
                                                                <VInput 
                                                                    :value="inputValue" 
                                                                    placeholder="Tanggal Rencana Operasi"
                                                                    v-on="inputEvents"/>
                                                            </VControl>
                                                        </VField>
                                                    </template>
                                                ...
                                                <div class="column is-12">
                                                    <VField>
                                                        ...
                                                        <VControl icon="feather:bookmark">
                                                            <VInput type="text" v-model="item.kelompokpasien" placeholder="Jaminan" class="is-rounded" />
                                                        </VControl>
                                                    </VField>
                                                </div>
...
const loadRiwayat1 =async  () => {
    let dataEMR = '';
    async function getTriase () {
        dataEMR = await useApi().get('emr/auto-fill?nocmfk=' + ID_PASIEN + '&norec_pd=' + NOREC_PD + '&collection=TriagePasienIGD' + `&field=TBtinggiBadanTTV,TBberatBadanTTV`)
        if (dataEMR != null) {
            item.bb = dataEMR.TBberatBadanTTV
            item.tb = dataEMR.TBtinggiBadanTTV
        }
    }
    async function getNS () {
        dataEMR = await useApi().get('emr/auto-fill?nocmfk=' + ID_PASIEN + '&norec_pd=' + NOREC_PD + '&collection=AsesmenAwalKeperawatanPasienRawatJalanNurse' + `&field=beratbadanObgyn,tinggibadanObgyn`)
        if (dataEMR != null) {
            item.bb = dataEMR.beratbadanObgyn
            item.tb = dataEMR.tinggibadanObgyn
        } else {
            await getTriase()
        }
    }

    const getAskep = await useApi().get('emr/auto-fill?nocmfk=' + ID_PASIEN + '&norec_pd=' + NOREC_PD + '&collection=AsesmenAwalKeperawatanPasienRawatJalan' + `&field=beratbadanObgyn,tinggibadanObgyn`)
    if (getAskep != null) {
...
function fetchDropdown() {
    useApi().get(
        `/bedah/list-dropdown`).then((response: any) => {
            ...
            d_Kamar.value = response.kamaroperasi.map((e: any) => { return { label: e.namakamarok, value: e.id} })
            item.ruanganTujuan = d_Ruangan.value[0].value
            item.departemenfk = d_Ruangan.value[0].default.objectdepartemenfk
            // d_JenisOperasi.value.push({
            //     label: "Lainnya",
            //     value: 0
            // });
...
async function simpan() {
    ...
    if(item.isanastesi == 1) {
        item.dokterAnastesi.forEach((el, elindex) => {
            if(el.id == null) {
                msgErr = 'Terdapat Dokter Anastesi yang belum di pilih';
                iserr = true;
            }else {
                if(elindex !== 0) {
        ...
        // if (!item.dokterAnastesi || item.dokterAnastesi[0] == "") {
        //     msgErr = 'Pilih Dokter Anastesi';
        //     iserr = true;
        // }
    }
    ...
    if (item.kelompokpasien == undefined || item.kelompokpasien == "") {
    ...
    // console.log("Data Id Tambahan",dataidTambahan);
    // return;
    ...
    var objSave = {
        ...
        jaminan: item.kelompokpasien ? item.kelompokpasien : null,
    ...
    useApi().get(
        `/bedah/detail-order?norec=${e.norec}`).then((response: any) => {
            for (let x = 0; x < response.length; x++) {
                ...
                item.kelompokpasien = element.kelompokpasien
...
function hasilItems(e: any) {
    ...
    H.printBlade(`bedah/surat-pengantar-rencana-operasi?nocmfk=${ID_PASIEN}&norec_pd=${e.norec_pd}`);
}
