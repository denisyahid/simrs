<template>
  ...
  <Dialog v-model:visible="modalInput" modal header="Upload" :style="{ width: '30vw' }">
      ...
      <VField class="is-rounded-select is-autocomplete-select
        ...
        <VControl icon="fas fa-sticky-note" fullwidth class="prime-auto-select">
          <Dropdown v-model="input.namafile" :options="d_Berkas" :optionLabel="'nama'" class="is-rounded"
            placeholder="File" style="width: 100%;" :filter="true" showClear disabled />
        </VControl>
      </VField>
    </div>
    <div class="column is-12">
    ...
    <div class="column is-12">
      <FileUpload v-model="filePasien" mode="basic" name="demo" accept="image/png,image/jpeg" @upload="onUpload"
...
useHead({
  title: 'Skema Penyinaran - ' + import.meta.env.VITE_PROJECT,
})
...
const loadRiwayat = () => {
  ...
  useApi().get(`/emr/berkas-pasien?nocm=${props.pasien.nocm}&noregistrasi=${props.registrasi.noregistrasi}&dokumen=Skema Penyinaran`).then((response: any) => {
...
const add = () => {
  ...
  input.value = {}
  input.value.namafile = {
    id: 11,
    nama: "Skema Penyinaran",
  }
  input.value.author = userLogin.namaUser
