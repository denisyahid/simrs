                ...
                <div class="column">
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-4">
                            <span>Telepon</span>
                            <VControl>
                                <VInput type="text" class="input" v-model="input.teleponPasien" />
                            </VControl>
                        </div>
                        ...
                        <div class="column is-12">
                            Dengan ini menghibahkan barang ke RSUD dr.M.Yunus Bengkulu, berupa :
                            <VField>
                                <VTextarea rows="2" v-model="input.barangHibah"></VTextarea>
                            </VField>
                            Barang-barang yang dihibahkan kiranya dapat digunakan dan bermanfaat bagi pelayanan pasien
                        ...
                        <div class="column is-4" style="margin-right: auto;">
                            <br><br><br>
                            <div style="text-align: center;">
                                <h1>Yang Menerima,</h1>
                                <TandaTangan :elemenID="'TTD_Menerima'" :width="'150'" :height="'150'"
                                    class="dek mt-2" />
                                <VControl class="prime-auto">
                                    <AutoComplete v-model="input.namaPenerima" :suggestions="d_Pegawai"
                                        @complete="fetchPegawai($event)" :optionLabel="'label'" :dropdown="true"
                                        :minLength="3" :appendTo="'body'" :loadingIcon="'pi pi-spinner'"
                                        :field="'label'" class="mt-2" />
                                </VControl>
                                <span>Supervisor Logistik IRJA RSMY</span>
                            </div>
                        </div>
                        ...
                        <div class="column is-4" style="margin-left: auto;">
                            ...
                            <div style="text-align: center;">
                                ...
                                <TandaTangan :elemenID="'TTD_Menyerahkan'" :width="'150'" :height="'150'" class="dek mt-2" />
                                <VControl class="prime-auto mt-2">
                                    <AutoComplete v-model="input.namaMenyerahkan" :suggestions="d_Pegawai"
                                        @complete="fetchPegawai($event)" :optionLabel="'label'" :dropdown="true"
                                        :minLength="3" :appendTo="'body'" :loadingIcon="'pi pi-spinner'"
                                        :field="'label'" />
                                </VControl>
                        ...
                        <div class="column is-12" style="text-align: center;">
                            <div style="text-align: center;">
                                <h1>Saksi</h1>
                                <TandaTangan :elemenID="'TTD_Saksi'" :width="'150'" :height="'150'"
                                    class="dek mt-2" />
                                <VControl class="prime-auto">
                                    <AutoComplete v-model="input.namaSaksi" :suggestions="d_Pegawai"
                                        @complete="fetchPegawai($event)" :optionLabel="'label'" :dropdown="true"
                                        :minLength="3" :appendTo="'body'" :loadingIcon="'pi pi-spinner'"
                                        :field="'label'" class="mt-2" style="width: 30%;" />
                                </VControl>
                            </div>
                        </div>
                    </div>
...
const setAutoFill = async () => {
    ...
    d.teleponPasien = ps.notelepon ? ps.notelepon : ps.nohp
...
const loadRiwayat = async () => {
    ...
    await useApi().get(`/emr/get-emr?nocmfk=${ID_PASIEN}&norec_pd=${NOREC_PD}&collection=${COLLECTION.value}&emrpasienfk=${NOREC_EMRPASIEN.value}`).then(async (response: any) => {
        if (response.length) {
            ...
            H.tandaTangan().set("TTD_Menerima", res.TTD_Menerima)
            H.tandaTangan().set("TTD_Menyerahkan", res.TTD_Menyerahkan)
            H.tandaTangan().set("TTD_Saksi", res.TTD_Saksi)
        } else {
            await setAutoFill()
        }
    }).catch((e: any) => {
...
const simpan = async () => {
    ...
    object = input.value
    object['TTD_Menerima'] = H.tandaTangan().get("TTD_Menerima");
    object['TTD_Menyerahkan'] = H.tandaTangan().get("TTD_Menyerahkan");
    object['TTD_Saksi'] = H.tandaTangan().get("TTD_Saksi");
