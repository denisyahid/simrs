<template>
  ...
  <Dialog v-model:visible="previewTutupJadwalDetail" modal header="Detail Jadwal Tertutup" :style="{ width: '100rem' }">
    <ObservasiKeseimbanganCairanDetail :data="listTutupJadwalDetail"></ObservasiKeseimbanganCairanDetail>
  </Dialog>
</template>
...
import ObservasiKeseimbanganCairanDetail from '../page-emr/observasi-keseimbangan-cairan-detail.vue'
...
const COLLECTION = 'ObservasiKeseimbanganCairan';
...
const loadRiwayatTutupJadwalDetail = async (id: any) => {
  ...
  try {
    const response = await useApi().get(
      `/emr/get-emr-monitoring-icu-jadwal-tertutup-detail?nocmfk=${ID_PASIEN}&collection=ObservasiKeseimbanganCairan&id=${id}`
    );
