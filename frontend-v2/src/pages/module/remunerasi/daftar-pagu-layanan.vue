    ...
    <section>
        <div class="column is-12">
            <VCard>
                ...
                <VPlaceload height="20rem" width="100%" class="mx-2 mt-5" v-if="isLoading" />
                <DataTable v-else :rows="10" :value="dataSource" :rowsPerPageOptions="[5, 10, 15,50,100,1000]" :loading="loadSearch"
                    ...
                    <Column field="tglstruk" header="TGL Struk">
                        <template #body="slotProps">
                            {{ H.formatDateToLocalString(slotProps.data.tglstrukpagu) }}
                        </template>
                    </Column>
                    <Column field="nostrukpagu" header="No Struk Pagu" />
                    <Column field="periodeawal" header="TGL Pagu" sortable>
                        <template #body="slotProps">
                            {{ H.formatDateToLocalString(slotProps.data.periodeawal) }}
                        </template>
                    </Column>
                    <Column field="totalrcdokter" header="DIREKSI" style="text-align:right;">
                        <template #body="slotProps">
                            {{ H.formatRupiah(H.roundToDecimal(parseFloat(slotProps.data.totalrcdokter), 2), '') }}
                        </template>
                    </Column>
                    <Column field="totalrc" header="STRUKTURAL" style="text-align:right;">
                        <template #body="slotProps">
                            {{ H.formatRupiah(H.roundToDecimal(parseFloat(slotProps.data.totalrc), 2), '') }}
                        </template>
                    </Column>
                    <Column field="totalpostrm" header="CASEMIX" style="text-align:right;">
                        <template #body="slotProps">
                            {{ H.formatRupiah(H.roundToDecimal(parseFloat(slotProps.data.totalpostrm), 2), '') }}
                        </template>
                    </Column>
                    <Column field="totalccdireksi" header="JPL" style="text-align:right;">
                        <template #body="slotProps">
                            {{ H.formatRupiah(H.roundToDecimal(parseFloat(slotProps.data.totalccdireksi), 2), '') }}
                        </template>
                    </Column>
                    <Column field="totalccstaffdireksi" header="JPTL" style="text-align:right;">
                        <template #body="slotProps">
                            {{ H.formatRupiah(H.roundToDecimal(parseFloat(slotProps.data.totalccstaffdireksi), 2), '') }}
                        </template>
                    </Column>
                    <Column field="totalccmanajemen" header="GABUNGAN" style="text-align:right;">
                        <template #body="slotProps">
                            {{ H.formatRupiah(H.roundToDecimal(parseFloat(slotProps.data.totalccmanajemen), 2), '') }}
                        </template>
                    </Column>
                        ...
                        <template #body="slotProps">
                            ...
                            <OverlayPanel ref="op">
                                <VButton type="button" icon="fas fa-print" class="mr-2" light circle outlined color="info"
                                    raised @click="changeStatusBayar(selected, true)" :loading="loadVerif"
                                    :disabled="selected.isbayar == true">
                                    Verifikasi Bayar
                                </VButton>
                                <VButton type="button" icon="fas fa-undo" class="mr-2" color="warning"
                                    :loading="loadUnverif" circle outlined raised
                                    @click="changeStatusBayar(selected, false)" :disabled="selected.isbayar != true">
                                    Unverifikasi
                                </VButton>
                            </OverlayPanel>
                        </template>
                    ...
                    <ColumnGroup type="footer">
                        <Row>
                            <Column footer="Total" style="min-width: 80px;padding: 0.3rem 0.3rem 0 0.3rem;" />
                            <Column />
                            <Column />
                            <Column :footer="item.FDireksi" style="text-align:right; padding: 0.3rem 0.3rem 0 0.3rem;" />
                            <Column :footer="item.FStruktural" style="text-align:right; padding: 0.3rem 0.3rem 0 0.3rem;" />
                            <Column :footer="item.FCasemix" style="text-align:right; padding: 0.3rem 0.3rem 0 0.3rem;" />
                            <Column :footer="item.FJPL" style="text-align:right; padding: 0.3rem 0.3rem 0 0.3rem;" />
                            <Column :footer="item.FJPTL" style="text-align:right; padding: 0.3rem 0.3rem 0 0.3rem;" />
                            <Column :footer="item.FGabungan" style="text-align:right; padding: 0.3rem 0.3rem 0 0.3rem;" />
                            <Column />
                            <Column />
                        </Row>
                    </ColumnGroup>
                        ...
                        <div class="column pt-0 pb-0" style="text-align:right">
                            <VButtons style="justify-content: flex-end">
                                <VButton class="mr-4" color="danger" outlined icon="fas fa-lock"
                                    style="padding-right: 3rem;padding-left: 3rem;" @click="closingData"
                                    :loading="loadSave"> Closing </VButton>
                                <VButton class="mr-4" color="warning" raised @click="goToPotongan"> Potongan </VButton>
...
useHead({
    title: 'Daftar Pagu Layanan - ' + import.meta.env.VITE_PROJECT,
})
...
let loadVerif: any = ref(false)
let loadUnverif: any = ref(false)
...
const fetchPagu = async () => {
    ...
    item.value.FDireksi = 'Rp. ' + H.formatRupiah(H.roundToDecimal(item.value.TDireksi, 2), '')
    item.value.FStruktural = 'Rp. ' + H.formatRupiah(H.roundToDecimal(item.value.TStruktural, 2), '')
    item.value.FCasemix = 'Rp. ' + H.formatRupiah(H.roundToDecimal(item.value.TCasemix, 2), '')
    item.value.FJPL = 'Rp. ' + H.formatRupiah(H.roundToDecimal(item.value.TJPL, 2), '')
    item.value.FJPTL = 'Rp. ' + H.formatRupiah(H.roundToDecimal(item.value.TJPTL, 2), '')
    item.value.FGabungan = 'Rp. ' + H.formatRupiah(H.roundToDecimal(item.value.TGabungan, 2), '')
...
const changeStatusBayar = async (e: any, status: any) => {
    ...
    if (status == true) { loadVerif.value = true }
    if (status == false) { loadUnverif.value = true }
    await useApi().post('remunerasi/verifikasi-bayar', objsave).then((response) => {
        fetchPagu()
    })
    loadVerif.value = false
    loadUnverif.value = false
}

const closingData = async () => {
    if (dataSource.value.length == 0) {
        H.alert('error', 'Tidak ada data yang tersedia')
        return
    }
    let stt = false
    dataSource.value.forEach((element: any) => {
        if (element.isbayar == true) {
            stt = true
        }
    });
    if (stt == true) {
        H.alert('error', 'Pagu ini sudah dibayarkan')
        return
    }
            ...
            useApi().post('remunerasi/closing-rc', objSave).then(function (e) {
                useApi().post('remunerasi/closing-cc', objSave).then(function (e) {
                    useApi().post('remunerasi/closing-ccs', objSave).then(function (e) {
                        useApi().post('remunerasi/closing-potongan', objSave).then(function (e) {
...
const goToPotongan = () => {
    router.push({
        name: 'module-remunerasi-potongan-remunerasi-pegawai'
    })
}
