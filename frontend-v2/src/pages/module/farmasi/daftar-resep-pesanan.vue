            ...
            <div class="column is-12">
                <div class="columns">
                    <div class="column is-4">
                        <VField label="Tanggal Input" style="margin-bottom: 6px;" />
                        ...
                        <VField class="is-autocomplete-select" label="Ruangan">
                            <VControl icon="feather:search">
                                <Multiselect mode="single" @select="fetchOrder()" v-model="item.ruangan"
                                    :options="d_ruangan" placeholder="Pilih Satelit" :searchable="true" />
                            </VControl>
                        </VField>
                    </div>
                    <div class="column is-3">
        ...
        <VCard>
            <div class="column c-title pt-2 mb-0">
                <div class="columns p-2">
                    <div class="column is-10">
                        <label class="title-page">Daftar Penginputan</label>
                        <label for="">List Daftar Penginputan Resep </label>
                    </div>
                    <div class="column pr-0">
                        <VButton type="button" icon="feather:x-circle" RouterLink
                            :to="{ name: 'module-farmasi-penjualan-resep-pesanan' }"
            ...
            <div class="column is-12 mt-5">
                <DataTable :value="dataSource" :rows="5" :rowsPerPageOptions="[5, 10, 15]" :loading="loadSearch"
                    ...
                    <Column field="tglstruk" header="Tgl Input" :style="{width: '150px'}"/>
                    <Column field="nostruk" header="No Resep" :style="{width: '200px'}"></Column>
                    <Column field="norm" header="No Rm" :style="{width: '200px'}"></Column>
...
useHead({
    title: 'Daftar Pesanan Resep - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchOrder = async () => {
    ...
    await useApi().get(`/farmasi/daftar-resep-pesanan?${tglAwal}${tglAkhir}${namaPasien}`).then((response: any) => {
    ...
    dataSource.value.forEach((element: any) => {
        if (element.no == e.no) {
            if (element.nosbm) {
                ...
                router.push({
                    name: 'module-farmasi-penjualan-resep-pesanan',
                    query: {
                        norec: e.norec,
                    },
                })
...
const cetakLabel = (e: any) => {
    //   console.log(e)
    qzService.printData(`report/farmasi/cetak-apotik-label-kecil-pesanan?pdf=true&norec=${e.norec}`, 'LABEL RESEP', 1);
