...
const save = async () => {
    ...
    isSimpan.value = true;
    try {
        const response = await useApi().post(`/farmasi/save-input-non-layanan-obat`, objSave);
...
const setNorecSPD = () => {
    ...
    if (ada == false) {
        ...
        norecSPD.value = ''
        norecTerima.value = ''
        console.log('ini jumlah',item.jumlahbulat)
        console.log('ini jumlah',item.stok)
        ...
        if (item.stok !== 0 && dataProdukDetail.value.length > 1 && !mergeAttempted && item.stok >= item.jumlah) {
            ...
            useApi().postNoMessage(
                '/farmasi/save-stock-merger', objSave).then(function (response) {
                    isMerge.value = true
                    GETKONVERSI()
