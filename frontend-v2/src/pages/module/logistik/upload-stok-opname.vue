            ...
            <VCard>
              ...
              <DataTable :value="dataSource" showGridlines editMode="cell" class="p-datatable-sm"
                            ...
                            <div class="column is-8" style="margin-left: 6rem;">
                                <div class="columns is-multiline" style="justify-content: right;">
                                  ...
                                  <div class="column is-3">
                                        <VField label="Golongan Produk">
                                            <VControl class="prime-auto">
                                                <AutoComplete v-model="item.golonganfk" :suggestions="d_golongan" @complete="fetchGolongan($event)"
                  ...
                  <ColumnGroup type="header">
                    <Row>
                      <Column header="No" :rowspan="5" style="text-align:center" />
                      <Column header="ID Produk" :rowspan="5" style="text-align:center"/>
                      <Column header="kode Produk" :rowspan="5" style="text-align:center" />
                      <Column header="Product" :rowspan="5" style="text-align:center"/>
                      <Column header="Harga" :rowspan="5" style="text-align:center"/>
                      <Column header="Jumlah" :colspan="3" style="text-align:center"/> <!-- Perbaiki colspan menjadi 3 -->
                      <Column header="Satuan" :rowspan="5" style="text-align:center"/>
                      <Column header="TglKadaluarsa" :rowspan="5" style="text-align:center"/>
                    </Row>
                    <Row>
                      ...
                      <!-- <Column header="ED Sistem" sortable field="tglkadaluarsa" />
                      <Column header="ED Real" sortable field="tglkadaluarsa" /> -->
                    </Row>
                  </ColumnGroup>
                  <Column field="no" />
                  <Column field="kodeproduk" />
                  <Column field="kdproduk" />
                  ...
                  <Column field="tglkadaluarsa"></Column>
                    <template #footer>
                        <div class="column">
                          <div class="columns is-multiline" style="justify-content: end;">
                            <div class="column is-4"  v-if="valueProgress > 0">
                              ...
                              <VButtons style="justify-content: flex-end">
                                <!-- <VButton icon="fas fa-save"  color="info" outlined raised @click="saveData()">Simpan</VButton> -->
...
async function fetchData() {
  ...
  let golongan = item.value.golonganfk ? `&golongan=${item.value.golonganfk.value}` : '';

  try {
    const response = await useApi().get(`/logistik/stok-opname-ed?${ruangan}${produk}${jenis}${golongan}`);
    response.data.forEach((element, i) => {
      element.no = i + 1;
      element.qtyReal = element.qtyproduk;
      element.selisih = element.qtyReal - element.qtyproduk; // Adjust this calculation if necessary
    });
...
const exportExcel = () => {
  ...
  const worksheet = XLSX.utils.aoa_to_sheet([
    ['STOK OPNAME'],
    [],
    ['NO', 'NAMA PRODUK', 'ID PRODUK', 'QTY PRODUK', 'QTY REAL', 'SATUAN STANDAR','KODE PRODUK','HARGA','TGL KADALUARSA'],
  ...
  // Verify if the worksheet is initialized with `!ref`
  if (!worksheet['!ref']) {
    console.error("Worksheet reference '!ref' not defined. Check data structure.");
    return;
  }

  // Defining styles for header cells
  const headerStyle = {
    alignment: {
      horizontal: 'center',
      vertical: 'center',
  ...
  // Applying style to headers (if headers exist)
  ...
  const columnWidths = [10, 30, 20, 15, 15, 15, 20];
  worksheet['!cols'] = columnWidths.map((width) => ({ wch: width }));

  // Centering the title in cell A1
  ...
  worksheet['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 1, c: 7 } }];
...
const fetchGolongan = async (filter: any) => {
  const response = await useApi().get(`/logistik/list-order-cbo`)
  d_golongan.value = response.golonganproduk.map((e: any) => { return { label: e.detailgolonganproduk, value: e.id } })
}
