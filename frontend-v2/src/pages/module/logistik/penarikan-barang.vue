                                                ...
                                                <VField class="is-rounded-select is-autocomplete-select">
                                                    ...
                                                    <VControl icon="feather:search">
                                                        <Multiselect mode="single" v-model="item.ruanganTujuan"
                                                            ...
                                                            :disabled="disabledRuanganTujuan" />
                                                    </VControl>
                                                </VField>
                                            </div>
                                        ...
                                        <div class="columns is-multiline">
                                            ...
                                            <DataTable :value="dataSource" showGridlines editMode="cell" class="p-datatable-sm"
                                                ...
                                                :paginator="true" :rows="5" :rowsPerPageOptions="[5, 10, 25]"> 
                                                    ...
                                                    <Column field="jumlah" header="Qty Penarikan"></Column>
                                                    <Column field="qtyprodukkonfirmasi" header="Qty Terlayani"
                                                        v-if="NOREC_ORDER">
                                                        <template #body="slotProps">
                                                            {{ slotProps.data.qtyprodukkonfirmasi }}
                                                        </template>
                    ...
                    <div class="columns is-multiline">
                            ...
                            <VField class="is-rounded-select is-autocomplete-select">
                                ...
                                <VControl icon="feather:search">
                                    <AutoComplete v-model="item.produk" :suggestions="d_produk"
                                        ...
                                        :field="'kode_namaproduk'" placeholder="ketik untuk mencari..."
                        ...
                        <div class="column is-3">
                            <VField>
                                ...
                                <VControl>
                                    <AutoComplete v-model="item.produk" :suggestions="d_produk"
                                        @complete="fetchProduk($event)" :optionLabel="'kdproduk'" :dropdown="true" disabled
                                        :minLength="2" class="is-rounded" :appendTo="'body'" :loadingIcon="'pi pi-spinner'"
                                        :field="'kdproduk'" placeholder="ketik untuk mencari..."
                                        @item-select="changeProduk(item.kdproduk)" />
                                </VControl>
                            </VField>
                        </div>
                        <div class="column is-3">
                            <VField>
                                <VLabel>Qty Penarikan</VLabel>
                                <VControl icon="feather:bookmark">
                        ...
                        <div class="column is-12">
                            <DataTable :value="dataSource" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
                                ...
                                <Column field="stock" header="Stok"></Column>
                                <Column field="jumlah" header="Qty Penarikan"></Column>
                                <Column field="qtyprodukkonfirmasi" header="Qty Terlayani" v-if="NOREC_ORDER"></Column>
...
const TITLE_PAGE = PRIHAL ? 'Verifikasi' : 'Penarikan Barang'
useHead({
...
const disabledRuanganTujuan: any = ref(true)
...
const loadDrop = async () => {
    ...
    // d_rutu.value = d_rutu.value.find((e: any) => e.value === 343) ? [d_rutu.value.find((e: any) => e.value === 343)] : [];
    item.ruanganTujuan = 343;
...
const tambah = () => {
    ...
    if (!Number.isInteger(item.jumlah)) {
        useToaster().error('Jumlah tidak boleh desimal')
        return
    }
    ...
    if (item.no != undefined) {
        data2.value.forEach((element: any, i: any) => {
            if (element.no == item.no) {
                ...
                data.qtysebelumnya = item.qtysebelumnya
                data.qtyorder = 0
        ...
        data = {
            ...
            qtysebelumnya: item.qtyprodukkonfirmasi,
            qtyorder: 0,
...
const save = async () => {
    ...
    let Keterangan = 'Penarikan Barang'
    ...
    let url = '/logistik/distribusi-barang-penarikan-barang'
    ...
    if(isNull.value == false){
        ...
        let strukkirimVerif = {
            objectruanganfk: response.data.Struk_Kirim.objectruanganfk,
            objectruangantujuanfk: response.data.Struk_Kirim.objectruangantujuanfk,
            jenispermintaanfk: 2,
            keteranganlainnyakirim: 'Penarikan Barang',
            qtydetailjenisproduk: 0,
            qtyproduk: data2.value.length,
            tglkirim: moment(item.tglkirim).format('YYYY-MM-DD HH:mm:ss'),
            totalhargasatuan: 0,
            norecOrder: NOREC_ORDER ? NOREC_ORDER : '',
            noreckirim: response.data.Struk_Kirim.norec,
            prihal : 'verifikasi',
            statuskirim : 2,
            // iseditkirim : ISEDIT_KIRIM
        }
        let objSaveVerif =
        {
            strukkirim: strukkirimVerif,
            details: dataSource.value
        }
        if(response.data != null){
            useApi().post(`/logistik/distribusi-barang-penarikan-barang?norec=${response.data.Struk_Kirim.norec}&prihal=verifikasi`,objSaveVerif).then((response:any)=>{
        ...
        '&ruanganfk=' + item.ruanganPengirim).then(function (response: any) {
            ...
            if (response.detail.length > 0) {
                if (dataSelected.value.no != undefined) {
                    ...
                    item.qtysebelumnya = dataSelected.value.qtyprodukkonfirmasi ? dataSelected.value.qtyprodukkonfirmasi : dataSelected.value.jumlah
...
const DialogConfirm = (e: any) => {
    confirm.require({
        message: 'Apakah Anda Yakin Akan Menarik Barang ?',
        header: 'Konfirmasi Penarikan Barang',
...
const getSelisih = (event: any) => {
    let { data, field, newValue } = event
        ...
        data.qtyprodukkonfirmasi = newValue;
