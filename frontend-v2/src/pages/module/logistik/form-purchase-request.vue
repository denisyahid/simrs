            ...
            <div class="columns is-multiline p-3">
              ...
              <div class="column is-4">
                <VField class="is-rounded-select is-autocomplete-select">
                  ...
                  <VControl icon="feather:search" fullwidth class="prime-auto-select">
                    <Dropdown v-model="item.unitPengorder" :options="d_unitPengorder" optionLabel="label"
      ...
      <div class="column is-12 p-0 mt-5">
        <VCard>
          ...
          <DataTable :value="sourcePurchOrder" :paginator="true" :rows="5" :rowsPerPageOptions="[5, 10, 25]"
            ...
            <Column field="namarekanan" header="Suplier" style="min-width:120px" />
            ...
            <Column field="ppn" header="Harga PPN" style="text-align:right">
              <template #body="slotProps">
                {{ H.formatRupiah(slotProps.data.ppn,'Rp') }}
              </template>
            </Column>
            <Column field="hargadiscount" header="Harga Diskon" style="text-align:right">
              <template #body="slotProps">
                {{ H.formatRupiah(slotProps.data.hargadiscount,'Rp')  }}
              </template>
            </Column>
        ...
        <div class="columns is-multiline">
          ...
          <div class="column is-4">
            <VField class="is-rounded-select is-autocomplete-select">
              ...
              <VControl icon="feather:search" fullwidth class="prime-auto-select">
                <Dropdown v-model="item.suplier" :options="d_suplier" optionLabel="label" placeholder="Pilih Suplier"
          ...
          <div class="column is-3">
            <VField label="Harga">
              <VControl :loading="loadHarga">
                <input v-model="item.hargasatuan" type="text"  class="input" placeholder="Harga" />
              </VControl>
            </VField>
          </div>
          <div class="column is-2">
...
const saveData = async () => {
  ...
  if (!item.value.kelompokBarang) {
    H.alert('error', 'Kelompok Barang Tidak Boleh Kosong')
    return
  }

  if (!item.value.unitPengorder) {
    H.alert('error', 'Unit Pengorder Tidak Boleh Kosong')
    return
  }
  ...
  if (!item.value.unitTujuan) {
    H.alert('error', 'Unit Tujuan Tidak Boleh Kosong')
    return
  }

  if (!item.value.tglJatuhTempo) {
    H.alert('error', 'Tanggal Jatuh Tempo Tidak Boleh Kosong')
    return
  }
  ...
  let strukOrder = {
    ...
    tgljatuhtempo: H.formatDate(item.value.tglJatuhTempo, 'YYYY-MM-DD HH:mm:ss'),
    tglDibutuhkan: H.formatDate(new Date(), 'YYYY-MM-DD HH:mm:ss'),
    qtyjenisproduk: sourcePurchOrder.value.length,
    ruanganfkPengusul: item.value.unitPengorder.value,
    ruanganfkTujuan: item.value.unitTujuan.value,
    penanggungjawabfk: item.value.penanggungJawab.value,
    norec: item.value.norec ? item.value.norec : '',
    norecrealisasi: item.value.norecrealisasi ? item.value.norecrealisasi : '',
    kelompokbarangfk: item.value.kelompokBarang ? item.value.kelompokBarang.value : null,
    kelompokbarang: item.value.kelompokBarang ? item.value.kelompokBarang.label : null,
    ...
    keteranganorder: item.value.keterangan ? item.value.keterangan : '',
  }
  ...
  await useApi().post('logistik/save-usulan-permintaan', objSave).then((response) => {
    goToPageDaftar()
  })
...
const addData = (e: any) => {
  if (!e.tglKebutuhan) {
    useToaster().error('Tanggal Kebutuhan Tidak Boleh Kosong')
    return
  }
  if (!e.produk) {
  ...
  if (!item.value.suplier) {
    H.alert('error', 'Supplier Tidak Boleh Kosong')
    return
  }
    ...
    sourcePurchOrder.value.forEach((element: any, i: any) => {
      if (element.no == e.no) {
        ...
        datas.no = element.no,
          ...
          datas.hargasatuan =H.unFormatRupiah(e.hargasatuan),
          datas.nilaikonversi = item.value.konversi,
          datas.total = H.unFormatRupiah(e.subTotal),
          datas.spesifikasi = e.spesifikasi,
          datas.rekananfk = e.suplier.value,
          datas.namarekanan = e.suplier.label,
          datas.persendiscount = e.persendiscount ? e.persendiscount : 0,
          datas.hargadiscount = e.hargaDiskon ? H.unFormatRupiah(e.hargaDiskon) : 0,
          datas.persenppn = e.persenPPN ? e.persenPPN : 0,
          datas.ppn = e.nilaiPPN ? H.unFormatRupiah(e.nilaiPPN) : 0,
...
const showModal = async (e: any) => {
  if (e.no) {
    ...
    item.value.hargasatuan =H.formatRupiah(Math.round(H.unFormatRupiah(e.hargasatuan)) ,'')
    ...
    item.value.hargaDiskon =H.formatRupiah(Math.round(H.unFormatRupiah(e.hargadiskon)),'')
    item.value.persenPPN = e.persenppn
    item.value.nilaiPPN = H.formatRupiah(H.unFormatRupiah(e.ppn),'')
...
const isEditPO = () => {
  ...
  if (norec) {
    ...
    useApi().get(`logistik/get-data-detail-purchaserequest?norecOrder=${norec}`).then((response) => {
...
watch(
  ...
  () => {
    ...
    if (item.value.jumlah === undefined || item.value.jumlah === '') {
      ...
      const nilaiDiskon: any =parseFloat(item.value.jumlah) * H.unFormatRupiah(item.value.hargasatuan || 0)  * diskon
      const resultHarga: any = nilaiDiskon ? parseFloat(item.value.jumlah) * H.unFormatRupiah(item.value.hargasatuan || 0)  - parseFloat(nilaiDiskon) : parseFloat(item.value.jumlah) * H.unFormatRupiah(item.value.hargasatuan || 0)
      const ppn: any = item.value.persenPPN / 100
      const nilaippn = resultHarga * ppn
      item.value.hargaDiskon =H.formatRupiah(diskon ? nilaiDiskon : 0 ,'')
      item.value.nilaiPPN =H.formatRupiah(ppn ?  Math.round(nilaippn) : 0,'')
      item.value.subTotal =nilaippn ?  H.formatRupiah(Math.round(parseFloat(resultHarga) + nilaippn),'')  :H.formatRupiah(Math.round(resultHarga),'')
    }
  }
)
