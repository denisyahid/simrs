        ...
        <div class="jobs-dashboard">
            ...
            <div class="search-menu">
                ...
                <div class="search-location">
                    ...
                    <input type="text" placeholder="No Terima" v-model="item.nostruk" />
                </div>
                    ...
                    <div class="column c-title">
                        <div class="columns">
                            <div class="column is-10 pt-0">
                                <label class="title-page" style="color:var(--dark-text)">Daftar Retur Distribusi</label>
                                <label for="">List retur barang antar ruangan</label>
                            </div>
                        </div>
                    </div>

                    <div class="column is-12">
                        <DataTable :value="dataSource" :rows="5" :rowsPerPageOptions="[5, 10, 15]" :loading="dataSource.loading "
                            ...
                            <Column field="jeniskirim" header="Jenis Kirim"/>
                            <Column field="jmlitem" header="Item"/>
                            <Column field="namaruanganasal" header="Ruangan Asal"/>
                            <Column field="namaruangantujuan" header="Ruangan Tujuan"/>
                            <Column field="petugas" header="Petugas" style="text-align: center;"/>
                            <Column :exportable="false"  header="Status" style="text-align: center;">
                                <template  #body="slotProps">
                                    <VTag color="info" :label="slotProps.data.status" rounded />
                                </template>
                            </Column>
...
async function fetchData() {
    ...
    let awal = `dari=${H.formatDate(item.value.periode.start, 'YYYY-MM-DD')}`
    let akhir = `&sampai=${H.formatDate(item.value.periode.end, 'YYYY-MM-DD')}`
    let noStruk = item.value.nostruk ? `&nostruk=${item.value.nostruk}` : ''
    await useApi().get(`/logistik/daftar-retur-distribusi?${awal}${akhir}${noStruk}`).then((respond) => {
