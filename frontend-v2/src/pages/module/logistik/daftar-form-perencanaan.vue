        ...
        <VCard>
            <div class="column c-title pt-2 mb-0">
                <div class="columns p-2">
                    <div class="column is-10 pt-0">
                        <label class="title-page">Daftar Form Perencanaan</label>
                        <label for="">List Perencanaan</label>
                    </div>
                </div>
            </div>
            <div class="column is-12 mt-5">
                <DataTable :value="dataOrder" :rows="5" :rowsPerPageOptions="[5, 10, 15]" :loading="loadSearch"
                    ...
                    <Column field="noorder" header="Tgl Perencanaan">
                        <template #body="slotProps">
                            {{ H.formatDate(slotProps.data.tglorder, 'DD/MM/YYYY') }}
                        </template>
                    </Column>
                    <Column field="tglOrder" header="Jenis Perencanaan">
                        <template #body="slotProps">
                            {{ H.formatDate(slotProps.data.tglkebutuhan, 'DD/MM/YYYY') }}
                        </template>
                    </Column>
                    <Column field="noorder" header="No SPK"></Column>
                    <Column field="keterangan" header="Jenis Anggaran"></Column>
                    <Column field="ruangan" header="Bln Anggaran">
                        <template #body="slotProps">
                            {{ H.formatDate(slotProps.data.tglusulan, 'DD/MM/YYYY') }}
                        </template>
                    </Column>
                    <Column field="totalhargasatuan" header="Nilai Perencanaan"></Column>
                    <Column field="status" header="Status Revisi"></Column>
                        ...
                        <template #body="slotProps">
                            ...
                            <OverlayPanel ref="op">
                                ...
                                <VButton type="button" icon="fas fa-trash" class="ml-2" color="danger" circle outlined
                                    @click="dialogConfirm(selected)" raised>Batal Perencanaan
                                </VButton>
                                <!-- <VButton type="button" class="ml-2" color="black" circle outlined icon="fas fa-print" raised>Cetak Perencanaan
                                </VButton> -->
                                <VButton type="button" class="ml-2" color="info" circle outlined raised @click="gotoHPS(selected,'verif')">HPS
                                </VButton>
                            </OverlayPanel>
                        </template>
                    </Column>
                        ...
                        <div class="column pt-0 pb-0" style="text-align:right">
                            <VButtons style="justify-content: flex-end">
                                ...
                                <VButton class="mr-4" color="primary" raised icon="fas fa-edit"
                                    ...
                                    :loading="loadSave"> Buat Perencanaan </VButton>
                            </VButtons>
                        </div>
                    </template>
                </DataTable>
...
const fetchOrder = async () => {
    ...
    await useApi().get(`/logistik/daftar-rencana-usulan-permintaan-barang-ruangan?${tglAwal}${tglAkhir}${produk}${noorder}`).then((response: any) => {
...
const deleteData = async (e: any) => {
    ...
    await useApi().post('logistik/delete-data-usulan-ruangan', { 'norec': e.norec }).then((response) => {
        fetchOrder()
    })
}
...
const gotoHPS = (e: any,ket:any) => {
    router.push({
        name: 'module-logistik-harga-perolehan-satuan',
        query: {
            norec: e.norec,
            keterangan : ket
        },
...
const gotoPageForm = () => {
    router.push({ name: 'module-logistik-form-perencanaan' })
}
