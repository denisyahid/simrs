...
const saveData = () => {
  ...
  let strukOrder = {
    tglUsulan: H.formatDate(item.value.tglPO, 'YYYY-MM-DD HH:mm:ss'),
    ...
    objectrekananfk: item.value.suplier.value,
    statusorder: 1,
  ...
  useApi().post('logistik/save-usulan-permintaan', objSave).then((response) => {
    goToPageDaftar()
  })
...
const addData = (e: any) => {
  ...
  if (e.no) {
    ...
    datas = {
      ...
      persendiscount: e.persendiscount,
      hargadiscount: e.hargaDiskon,
      persenppn: e.persenPPN,
      ppn: e.nilaiPPN,
...
const isEditPO = () => {
  ...
  if (norec) {
    ...
    useApi().get(`logistik/get-detail-data-po?norecOrder=${norec}`).then((response)=>{
      ...
      item.value.tglPO = header.tglorder
...
watch(
  ...
  () => {
    ...
    if (item.value.jumlah === undefined || item.value.jumlah === '') {
      ...
      const nilaiDiskon: any = parseFloat(item.value.jumlah) * parseFloat(item.value.hargasatuan || 0) * diskon
      const resultHarga: any = nilaiDiskon ? parseFloat(item.value.jumlah) * parseFloat(item.value.hargasatuan || 0) - parseFloat(nilaiDiskon)
        : parseFloat(item.value.jumlah) * parseFloat(item.value.hargasatuan || 0)
