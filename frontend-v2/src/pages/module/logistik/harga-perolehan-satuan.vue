      ...
      <div class="form-outer" style="margin-top: 15px">
        <div class="form-header stuck-header">
          <div class="form-header-inner">
            <div class="left">
              <h3>Form Harga Perolehan Satuan</h3>
            </div>
            <div class="right">
              <div class="buttons">
          ...
          <div v-else>
            <div class="columns is-mulitline pl-3 pr-3">
              ...
              <div class="column is-3">
                <VField>
                  <VLabel class="required-field">No HPS</VLabel>
                  <VControl>
                    <input v-model="item.noHps" type="text" class="input" placeholder="No HPS" />
                  </VControl>
                </VField>
              </div>
              ...
              <div class="column is-4">
                <VDatePicker v-model="item.tglHPS" color="green" trim-weeks mode="date">
                  <template #default="{ inputValue, inputEvents }">
                    <VField>
                      <VLabel class="required-field">Tanggal HPS</VLabel>
            ...
            <div class="columns is-multiline p-3">
              <div class="column is-4">
                <VField label="Reff Perencanaan">
                  <VControl>
                    <input v-model="item.reffPerencanaan" type="text" class="input" placeholder="Reff Perencanaan" />
                  </VControl>
                </VField>
              </div>
              <div class="column is-4">
              ...
              <div class="column is-4">
                <VField>
                  <VLabel class="required-field">No SPPH</VLabel>
                  <VControl>
                    <input v-model="item.noSPPH" type="text" class="input" placeholder="No HPS" />
                  </VControl>
                </VField>
              </div>
            </div>
            ...
            <div class="columns is-multiline p-3">
              ...
              <div class="column is-4">
                <VDatePicker v-model="item.tglSPPH" color="green" trim-weeks mode="date">
                  <template #default="{ inputValue, inputEvents }">
                    <VField>
                      <VLabel class="required-field">Tanggal SPPH</VLabel>
                      <VControl icon="feather:calendar">
                        <VInput type="text" placeholder="Select a date" :value="inputValue" v-on="inputEvents" />
                      </VControl>
                    </VField>
              ...
              <div class="column is-4">
                <VDatePicker v-model="item.tglJatuhTempo" color="green" trim-weeks mode="date">
                  <template #default="{ inputValue, inputEvents }">
                    <VField>
                      <VLabel class="required-field">TGL Max Pengajuan</VLabel>
                      <VControl icon="feather:calendar">
                        <VInput type="text" placeholder="Select a date" :value="inputValue" v-on="inputEvents" />
                      </VControl>
                    </VField>
...
const saveData = async () => {
  ...
  let strukOrder = {
    noUsulan: item.value.noTransaksi,
    qtyjenisproduk: sourcePurchOrder.value.length,
    tglUsulan: H.formatDate(item.value.tglHPS, 'YYYY-MM-DD HH:mm:ss'),
    tglDibutuhkan: H.formatDate(item.value.tglJatuhTempo, 'YYYY-MM-DD HH:mm:ss'),
    nousulan: item.value.noHps,
    total: item.value.totalsub,
    norec: item.value.norec ? item.value.norec : '',
    norec_so: item.value.norec_so ? item.value.norec_so : '',
    ppn: item.value.ppnTotal,
    objectmataanggaranfk: item.value.mataAnggaran.value,
    norecrealisasi: item.value.norecrealisasi ? item.value.norecrealisasi : '',
    asalproduk: item.value.sumberdana.value ? item.value.sumberdana.value : '',
  }
  console.log(strukOrder)
  debugger
  let objSave = {
  ...
  await useApi().post('logistik/save-data-usulan-permintaan-barang-ruangan', objSave).then((response) => {
    goToPageDaftar()
  })
...
const isEditPO = () => {
  ...
  if (norec) {
    ...
    useApi().get(`logistik/get-data-detail-rencana-usulan?norecOrder=${norec}`).then((response) => {
      ...
      item.value.reffPerencanaan = header.nousulan
      sourcePurchOrder.value.loading = false
      item.value.sumberdana = d_SumberDana.value.find((sumberdana:any) => sumberdana.value == details[0].objectasalprodukfk) || d_SumberDana.value[0];
      item.value.mataAnggaran = d_MataAnggaran.value.find((mataanggaran:any) => mataanggaran.value == details[0].objectmataanggaranfk) || d_MataAnggaran.value[0];
...
const createNoFaktur = ()=>{
  ...
  item.value.noHps = 'PB/' + nows + '/APT/____'
}
const back = () => {
  window.history.back()
}
