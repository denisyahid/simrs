<template>
    ...
    <VCard>
      <div class="columns column">
        <h3 class="title is-5 mb-2 mr-1">Daftar Pemakaian Floor Stok </h3>
      </div>
      <VButton color="info" RouterLink :to="{ name: 'module-logistik-floor-stok-new' }" raised
        style="margin-left: 40rem;"><i class="fas fa-plus mr-3 p-0" aria-hidden="true"></i>Input FloorStok</VButton>
                    ...
                    <div class="column is-12">
                      <a class="flight-summary">
                        ...
                        <VIconButton v-tooltip.bottom.right="'Cetak Bukti Pdf'" icon="feather:printer" color="primary" raised
                          circle class="ml-6" @click="cetakBuktiKirim(items)">
                       </VIconButton>

                       <VIconButton v-tooltip.bottom.right="'Batal Penggunaan Barang'" icon="feather:trash" color="danger" raised
                          circle class="ml-2" @click="DialogConfirm(items)">
                       </VIconButton>
  
                        <div class="meta" style="margin-left: 12rem;">
                          <!-- <TdaftarDistribusi :item="items" @batalKirim="DialogConfirm(items,'batal kirim')" @unverif="DialogConfirm(items,'unverif')" @editItems="editDis(items)"  @gotoRetur="gotoRetur(items)"  @verif="verif(items)"/> -->
      ...
      <!-- <VModal :open="modalBatalKirim" title="Batal Kirim" size="medium" actions="right" @close="modalBatalKirim = false,clearBatal()"
        cancelLabel="Tutup">
        <template #content>
        ...
        <template #action>
          <VButton icon="feather:plus" color="primary" @click="saveBatal(item)" :loading="isLoading" raised>Simpan
          </VButton>
        </template>
      </VModal> -->
    </VCard>
  ...
  useHead({
    title: 'Daftar Pemakaian Floor Stok - ' + import.meta.env.VITE_PROJECT,
  })
  ...
  const cetakBuktiKirim = (e: any) => {
    H.printBlade(`logistik/cetak-bukti-tarik?nokirim=${e.nostruk}&jenis=floorstok`)
  }
  ...
  const fetchOrder = async () => {
    ...
    await useApi().get(
      `/logistik/daftar-floorstok?offset=${offset}&limit=${limit}&rows=${rows}${dari}${sampai}${noorder}${produkfk}${ruanganasalfk}${ruangantujuanfk}`).then((response) => {
  ...
  const DialogConfirm = (e: any,info:any) => {
    let message = 'Apakah Anda Yakin untuk Membatalkan Penggunaan Ini?'
    let konfirm = 'Konfirmasi Batal Penggunaan'
    confirm.require({
      message: message,
      header: konfirm,
