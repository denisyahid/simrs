        ...
        <div class="jobs-dashboard">
            ...
            <div class="search-menu">
                ...
                <div class="search-location">
                    ...
                    <input type="text" placeholder="No Dokumen" v-model="item.nostruk" />
                </div>

                <div class="search-salary">
                    <i class="iconify" data-icon="feather:clipboard"></i>
                    <input type="text" placeholder="No Faktur" v-model="item.nofaktur" />
                </div>
                <div class="search-salary">
                    <i class="iconify" data-icon="feather:clipboard"></i>
                    <input type="text" placeholder="No Terima" v-model="item.noorderintern" />
                </div>
                <!-- <div class="search-job">
                    <i class="iconify" data-icon="feather:user"></i>
                    <input type="text" placeholder="Nama Pasien" v-model="item.qnama" />
                </div> -->
                <VButton color="primary" raised class="search-button" :loading="isLoadbtn" @click="fetchData()">
                    Search</VButton>
            </div>
                    ...
                    <div class="column c-title">
                        <div class="columns">
                            <div class="column is-10 pt-0">
                                <label class="title-page" style="color:var(--dark-text)">Daftar Retur ke Supplier</label>
                                <label for="">List retur barang ke supplier</label>
                            </div>
                        </div>
                    </div>

                    <div class="column is-12">
                        <DataTable :value="dataSource" :rows="5" :rowsPerPageOptions="[5, 10, 15]" :loading="isLoadbtn"
                            ...
                            <Column field="nostruk" header="No Dokumen"/>
                            <Column field="nofaktur" header="No faktur"/>
                            <Column field="noorderintern" header="No Terima"/>
                            <Column field="tglStruk" header="Tanggal"/>
                            <Column field="namarekanan" header="Supplier"/>
                            <Column field="jmlitem" header="Item"/>
                            <Column field="totalharusdibayar" header="Total Tagihan"/>
                            <Column :exportable="false" header="##" style="text-align: center;">
                                <template #body="slotProps">
                                    <VIconButton v-tooltip.bottom.right.rounded="'Proses'" @click="goToProcess(slotProps.data.norec)" color="primary" outlined circle icon="fas fa-undo" />
                                </template>
                            </Column>
                            <template #expansion="slotProps">
                                <div class="p-3">
                                    <DataTable :value="slotProps.data.details" :rows="10" showGridlines
                                        ...
                                        <Column field="total_qty_retur" header="QTY Retur" />
                                        <Column field="hargasatuan" header="Harga Satuan" />
                                        <Column field="hargadiskon" header="Discount" />
                                        <Column field="persenppn" header="PPN" />
                                        <Column field="totalall" header="Total" />
                                        <Column field="tglkadaluarsa" header="TGL Kadaluarsa" />
                                        <Column field="nobatch" header="Nobatch" />
                                        <!-- <Column field="qtyproduk" header="Qty" style="text-align: center;" />
                                        <Column field="qtyprodukretur" header="Qty Retur" /> -->
                                    </DataTable>
                                </div>
                            </template>
                        </DataTable>
...
async function fetchData() {
    ...
    let noorderintern = item.value.noorderintern ? `&noorderintern=${item.value.noorderintern}` : ''
    await useApi().get(`logistik/penerimaan-barang-retur?${awal}${akhir}${nostruk}${nofaktur}${noorderintern}`).then((respond) => {
        respond.daftar.forEach((element: any, i: any) => {
            element.no = i + 1
            element.tglStruk = H.formatDate(element.tglstruk,'DD-MMM-YYYY')
            expandedRows.value = element.details.forEach((item:any,e:any) => {
                item.tglkadaluarsa = item.tglkadaluarsa ? H.formatDate(item.tglkadaluarsa,'DD-MMM-YYYY') : '-'
            });

        });
        dataSource.value = respond.daftar
    }).catch((e) => {
        dataSource.value.loading = false
    })
}

const goToProcess = (e:any) =>{
    router.push({
