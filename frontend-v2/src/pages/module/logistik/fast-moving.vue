<template>
          ...
          <DataTable :rows="5" :value="dataSource" :loading="isLoading" :rowsPerPageOptions="[5, 10, 15]"
                    ...
                    <Column field="jumlah" header="Jumlah" style="min-width: 80px;" />
...
const fetchData = async ()=>{
  ...
  await useApi().get(`/logistik/data-fast-moving?tglawal=${tglAwal}&tglakhir=${tglAkhir}${produk}${kelompokProduk}${namaproduk}`).then((response)=>{
...
const exportExcel = (e: any) => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        ['Barang Produk Fast Moving'],
        [],
        ['NO', 'NAMA PRODUK', 'JUMLAH', 'KELOMPOK PRODUK'],
