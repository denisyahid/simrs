<template>
  <VCard>
    <div class="columns column">
      <h3 class="title is-5 mb-2 mr-1">Laporan Penggunaan Barang {{ item.namaProdukHeader }}</h3>
    </div>
        ...
        <DataTable :value="enhancedDataSource" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
          ...
          currentPageReportTemplate="Showing {first} to {last} of {totalRecords}">
             ...
             <Column field="kdproduk" :sortable="true" style="min-width: 100px">
              ...
              <Column field="saldoawal" :sortable="true" style="min-width: 100px">
                <template #header>
                  <div style="text-align: center;width: 80%; margin-left:15px">
                    Saldo Awal
                  </div>
              ...
              <Column field="saldomasuk" :sortable="true" style="min-width: 100px">
                <template #header>
                  <div style="text-align: center;width: 80%; margin-left:15px">
                    Saldo Masuk
                  </div>
...
const exportExcel = () => {
  const worksheet = XLSX.utils.aoa_to_sheet([
    ...
    [periode.value],
    [],
    ['No', 'Kode Produk', 'Nama Produk', 'Saldo Awal', 'Saldo Masuk', 'Saldo Keluar', 'Saldo Akhir'],
    ...dataSource.value.map((e: any) => [
      e.no,
      e.kdproduk,
      e.namaproduk,
      parseFloat(e.saldoawal) || 0,
      parseFloat(e.saldomasuk) || 0,
      parseFloat(e.jumlah) || 0,
      parseFloat(e.saldoakhir) || 0,
    ]),
    [],
  ]);
...
async function fetchData() {
  ...
  await useApi().get('/logistik/penggunaan-alkes-floorstok' + tglAwal + tglAkhir + '&offset=' + offset + '&limit=' + limit + '&rows=' + rows + produkfk + ruanganfk).then((response) => {
