...
useHead({
  title: 'RBA Detail - ' + import.meta.env.VITE_PROJECT,
})
...
if (item.terhadapdetail != undefined) {
    window.open(baseTransaksi + "report/get-cetak-rba-detail-sebelum-sesudah?tahun=" + moment(tahun).format('YYYY') + "&subsubkegiatan=" + subsubkegiatan + "&tahap=" + tahap + "&terhadap=" + terhadap + "&tglcetak=" + moment(item.tglCetak4).format('YYYY-MM-DD HH:mm')+kode, '_blank');
} else if(item.terhadapdetail == undefined){
  H.printBlade("report/get-cetak-rba-detail?tahun=" +moment(tahun).format('YYYY')+ "&subsubkegiatan=" + subsubkegiatan + "&tahap="+tahap+"&tglcetak="+moment(item.tglCetak4).format('YYYY-MM-DD HH:mm'), '_blank');
}
