                    ...
                    <div class="columns is-multiline">
                        ...
                        <div class="column is-3">
                            <VField class="is-autocomplete-select">
                                ...
                                <VControl icon="feather:search">
                                    <Dropdown v-model="item.kegiatanmonev" :options="d_subKegiatan" :optionLabel="'keterangan'"
                        ...
                        <div class="column is-3">
                            <VField class="is-autocomplete-select">
                                ...
                                <VControl icon="feather:search">
                                    <Dropdown v-model="item.subkegiatanmonev" :options="d_subSubKegiatan" :optionLabel="'keterangan'"
                        ...
                        <div class="column is-2">
                            <VField class="is-autocomplete-select">
                                ...
                                <VControl icon="feather:search">
                                    <Dropdown v-model="item.jenisrekeningmonev" :options="d_JenisRekening" :optionLabel="'jenisrekening'"
                                        class="is-rounded" placeholder="Jenis Rekening" style="width: 100%;" :filter="true" :loading="isLoadingCombo"
                                        showClear />
                                </VControl>
                            </VField>
                        </div>
...
useHead({
  title: 'SPJ dan Monev - ' + import.meta.env.VITE_PROJECT,
})
...
let d_JenisRekening: any = ref([
    { id: 1, jenisrekening: "Tampilkan Rek RBA" },
    { id: 2, jenisrekening: "Tampilkan Rek Permen"}
])
loadCombo()
...
const changeKegiatan = async (filter: any) => {
    await useApi().get("perencanaan/get-detail-sub-kegiatan?kegiatan=" + item.kegiatanmonev.id + "&tahap=" + item.tahapdetail.id + "&tahun=" + moment(item.tahundetail).format('YYYY')).then((e) => {
...
if (item.tahundetail != undefined) {
    tahun = moment(item.tahundetail).format('YYYY')
}
var kode = ""
var subsubkegiatan = ""
...
if(item.jenisrekeningmonev.id == 2){
    H.printBlade("report/get-cetak-spj-dan-monev?tahun="+tahun+"&jenisrekening="+item.jenisrekeningmonev.id+"&subkegiatan="+item.kegiatanmonev.id+"&subsubkegiatan="+item.subkegiatanmonev.id, '_blank');
} else if(item.jenisrekeningmonev.id == 1){
    H.printBlade("report/get-cetak-spj-dan-monev-rba?tahun="+tahun+"&jenisrekening="+item.jenisrekeningmonev.id+"&subkegiatan="+item.kegiatanmonev.id+"&subsubkegiatan="+item.subkegiatanmonev.id, '_blank');
}


// if (item.terhadapSumberDana == undefined || tahap == terhadap) {
//     window.open(baseTransaksi + "report/get-cetak-rekap-SumberDana-anggaran-sebelum?tahap=" + tahap + "&tahun=" + moment(iitem.tahunSumberDana).format('YYYY'), '_blank');
// } else if (item.terhadapSumberDana != undefined || tahap != terhadap) {
//     window.open(baseTransaksi + "report/get-cetak-rekap-SumberDana-anggaran?tahap=" + tahap + "&tahun=" + moment(item.tahunSumberDana).format('YYYY') + "&terhadap=" + terhadap, '_blank');
// }
