    ...
    <section>
        <div>
            <div class="columns is-multiline is-align-items-center ">
                ...
                <!-- <div class="column is-2"> -->
                    <!-- <VField>
                        ...
                        <Calendar v-model="item.tglMasuk" style="width: 100%;" class="input-calendar" dateFormat="dd/mm/yy" showIcon disabled placeholder="Sedang dalam perbaikan..."/>
                    </VField> -->
                    <!-- <VControl>
                        <Calendar v-model="input.tglAwal" selectionMode="single" :manualInput="false" class="w-100" :showIcon="true"
                        :showTime="false" />
                    </VControl> -->
                    <!-- <VDatePicker v-model="item.qFilterTgls" is-range color="pink" trim-weeks :max-date="new Date()">
                        <template #default="{ inputValue, inputEvents }">
                            <VField addons>
                                <VControl icon="feather:calendar">
                                    <VInput :value="inputValue.start" class="input-calendar" v-on="inputEvents.start" />
                ...
                <!-- <div class="column is-2">
                    <VField>
                        <div><b>Tanggal Keluar</b></div>
                        <Calendar v-model="item.tglKeluar" style="width: 100%;" class="input-calendar" dateFormat="dd/mm/yy" showIcon disabled placeholder="Sedang dalam perbaikan..."/>
                    </VField>
                </div> -->
                <div class="column is-3" style="margin-bottom: -20px;">
                    <VField>
                ...
                <div class="column is-2" style="margin-bottom: -20px;">
                    ...
                    <VIconButton circle class="ml-2  is-pulled-" icon="fas fa-archive" raised bold
                        @click="masterTarif()" v-tooltip.bubble="'Master Produk'">
                        <VSwitchBlock style="padding: 0;" v-model="item.isClosing" label="Closing" color="danger"/>
                    </VIconButton>
                    <VControl>
                        <VSwitchBlock v-model="item.isClosing" label="Closing" color="danger"/>
                    </VControl>
                </div>
                ...
                <div class="column is-2" style="margin-top:-30px; display: none !important">
                    <VField>
                        <VControl>
                ...
                <div class="column is-2" style="margin-top:-30px; display: none !important">
                    <VField>
                        <VControl>
                            <VSwitchBlock v-model="item.isClosing" label="Sudah diClosing" color="danger" />
                        </VControl>
                    </VField>
                </div>
                <div class="column is-6" style="margin-top:-30px; display: none !important">
            ...
            <div class="columns is-multiline">
                ...
                <div class="column is-6" style="text-align: end; display: none !important">
                    <VButton color="primary" @click="LoadExportExcel(activeTab)" outlined icon="fas fa-file-excel"
                        :loading="isLoadingExport">Export To Excel
                    </VButton>
                </div>
            </div>
                ...
                <TransitionGroup name="list" tag="div" class="columns is-multiline">
                    ...
                    <div v-for="item in dataSourcefiltered" :key="item.norec" class="column is-3">
                        <div class="grid-item-wrap is-clickable">
                            <div class="mt-2" style="display: none !important">
                                <span class="ml-4"> {{ item.namaruangan }}</span>
                            </div>
                            <label class="h-toggle mt-5-min" style="display: none !important">
                            ...
                            <div :class="'grid-item-head ' + (
                                ...
                                <div class="flex-head">
                                    ...
                                    <div class="meta">
                                        ...
                                        <span class="" style="width: 100%"> {{ item.ruangan2 }}</span>
                                        <span :style="item.inacbg_status != null ? 'color:black' : ''"
                                            style="font-size: 9pt; color:black; font-weight: normal">
                                            Tgl Masuk &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: {{ H.formatDateIndoSimple(item.tgl_masuk) }}
                                            <br>
                                        </span>
                                        <span v-if="item.tgl_pulang != null" 
                                            style="font-size: 9pt; color:black; font-weight: normal">
                                            Tgl Pulang &emsp;&emsp;: {{ H.formatDateIndoSimple(item.tgl_pulang) }}
                                        </span>
                                        <span v-else 
                                            style="font-size: 9pt; color:black; font-weight: normal">
                                            Tgl Pulang &emsp;&emsp;: -
                                        </span>
                                        <VTag :label="item.icd10 == false ? 'ICD 10 belum di input' : item.icd10"
                                            :color="item.icd10 == false ? 'warning' : 'info'"></VTag>
                                        <VTag :label="item.icd9" v-if="item.icd9 != false" class="ml-1"
                                            :color="'purple'">
                                        </VTag>
                                ...
                                <div class="buttons">
                                    ...
                                    <!-- <VDropdown icon="feather:more-vertical" spaced left class="is-pulled-left">
                                        <template #content>
                            ...
                            <div class="grid-item" 
                                :style="{'padding-bottom': '50px', 'background-color': item.icd10 == false ? '#fcc348' : ''}">
                                <h3 class="dark-inverted mt-0">{{ item.nama_pasien }}</h3>
                                <p style="color: black;">No RM. {{ item.nomor_rm }}</p>
                                <p style="color: black;">No Reg. {{ item.noregistrasi }}</p>
                                <p style="color: black;">Pasien {{ item.statuspasien }}</p>
                                <p>
                                    <VTag
                                        :label="item.tglmeninggal == null ? 'Hidup' : 'Meninggal : ' + item.tglmeninggal"
                                        :color="item.tglmeninggal == null ? 'success' : 'danger'"></VTag> <br>
                                    <VTag style="margin-top: 10px;"
                                        :label="item.tglclosing == null ? 'Belum Closing' : 'Sudah Closing'"
                                        :color="item.tglclosing == null ? 'danger' : 'success'"></VTag>
                                </p>
                                <VButton rounded color="info" class="is-pulled-center mt-2" icon="fas fa-eye" raised bold
                                        @click="detail(item)" :loading="isLoadingRiwayat" style="color: white; font-size: 10pt; width: 80px; height: 25px;">
                                        Koding
                                    </VButton>
                            </div>
                        </div>
        ...
        <Dialog v-model:visible="modalFilter" modal header="Filter" :style="{ width: '40vw' }">
              ...
              <div class="column is-6">
                ...
                <div class="column is-6">
                    <VField label="Ruangan" class="is-rounded-select is-autocomplete-select
                                mt-0 pt-0" v-slot="{ id }">
                        <VControl>
                            <MultiSelect v-model="item.qRuangan" display="chip" :options="d_Ruangan"
                                optionLabel="namaruangan" filter placeholder="Pilih Ruangan" :maxSelectedLabels="3"
                                style="display:flex" @change="changeRuang($event)" />
                        </VControl>
                    </VField>
                </div>
                <div class="column is-6">
                    <VField label="Status Klaim" class="is-rounded-select is-autocomplete-select
                        ...
                        <VControl icon="fas fa-credit-card" fullwidth class="prime-auto-select">
                            <Dropdown v-model="item.qStatus" :options="d_Status" :optionLabel="'status'"
                                class="is-rounded" placeholder="Status Klaim" style="width: 100%;" :filter="true"
                                showClear />
                        </VControl>
                    </VField>
                </div>
                <div class="column is-6">
                    <VField label="Status Pasien" class="is-rounded-select is-autocomplete-select
                                mt-0 pt-0" v-slot="{ id }">
                        <VControl icon="fas fa-credit-card" fullwidth class="prime-auto-select">
                            <Dropdown v-model="item.qStatusPasien" :options="d_StatusPasien" :optionLabel="'status'"
                ...
                <!-- <div class="column is-6">
                    <VField label="Closing" class="is-rounded-select is-autocomplete-select
                                mt-0 pt-0" v-slot="{ id }">
                        <VControl icon="fas fa-credit-card" fullwidth class="prime-auto-select">
                            <Dropdown v-model="item.qClosing" :options="d_Closing" :optionLabel="'status'"
                                class="is-rounded" placeholder="HEHE" style="width: 100%;" :filter="true"
                                showClear />
                        </VControl>
                    </VField>
                </div> -->
            ...
            <template #footer>
                ...
                <VButton type="button" rounded outlined color="primary" raised icon="feather:filter"
                    :loading="isLoading" @click="terapkanFilter()"> Terapkan
                </VButton>
            </template>
        </Dialog>
                ...
                <div class="column is-12">
                    <div>
                        ...
                        <VCard class="mt-2">
                            <div class="columns is-multiline">
                                <div class="column is-4">
                                    ...
                                    <listWidgetCustom title="Radiologi"
                                        :color="tarif18.tenaga_ahli > 0 ? 'success' : 'warning'"
                                ...
                                <div class="column is-4">
                                    <listWidgetCustom title="Konsultasi"
                                        :color="tarif18.konsultasi > 0 ? 'success' : 'warning'"
                                        :subtitle="H.formatRp(tarif18.konsultasi, 'Rp.')" />
                                    <listWidgetCustom title="Penunjang"
                                        :color="tarif18.penunjang > 0 ? 'success' : 'warning'"
                                    ...
                                    <listWidgetCustom title="Sewa Alat"
                                        :color="tarif18.sewa_alat > 0 ? 'success' : 'warning'"
                        ...
                        <VCard class="mt-2">
                            <div class="columns is-multiline">
                                <div class="column is-6">
                                    <b>Tarif Rumah Sakit INACBG's : {{ H.formatRp(tarif18.totalmappingtarif, 'Rp. ')
                                        }}</b>
                                </div>
                                <div class="column is-6">
                            ...
                            <VCardHead title="Bundle" class="p-2 text-center">
                                <div class="columns is-multiline">
                                    ...
                                    <div class="column is-10 mt-0 pt-0">
                                        <span>Status kelengkapan dokumen {{ listDokumen.persen_dok.toFixed(2) }}
                                            %</span>
                                        <VProgress
                    ...
                    <OrderList v-model="listDokumen.dokumen" listStyle="height:auto" dataKey="id">
                        ...
                        <template #item="slotProps">
                            <div class="columns is-multiline p-2">
                                <div class="column is-1">
                                    <input type="checkbox" v-model="slotProps.item.checked"
                                        name="checked['{{ slotProps.item.urutan }}']" />
                                </div>
                                ...
                                <div class="column is-2">
                                    <FileUpload mode="basic" name="demo[]" accept="application/pdf" @upload="onUpload"
                                        v-if="slotProps.item.doc == null" chooseLabel="Upload"
                                        @select="onSelect($event, slotProps.item)" class="is-rounded w-100" />
                                    <VButton v-if="slotProps.item.doc != null" color="info" raised rounded
                                        icon="fas fa-eye" class="w-100" @click="lihatDok(slotProps.item)">Lihat
                                    </VButton>
                                </div>
                            </div>
                        </template>
            ...
            <div class="columns is-multiline">
                ...
                <div class="column is-6">
                    <VField label="No SEP">
                        <VControl icon="fas fa-address-book">
                            <input v-model="updatePA.nomor_sep" type="text" class="input is-rounded"
                                placeholder="No SEP" />
                        </VControl>
                    </VField>
                </div>
                <div class="column is-6">
                    <VField label="No Kartu">
                        <VControl icon="fas fa-address-book">
                            <input v-model="updatePA.nomor_kartu" type="text" class="input is-rounded"
...
const fetchData = async () => {
    ...
    if (item.qRuangan != undefined) {
        let itemsRuang = []
        item.qRuangan.forEach((element: any) => {
            itemsRuang = [...new Set([...itemsRuang, element.value])]
        });
        ruang = `&ruang=${itemsRuang}`
        jmlFilter.value += 1
    }
    if (item.qRuangan && item.qRuangan.length > 0) {
        ruang = item.qRuangan[0].id;
    } else {
        ruang = undefined; 
    }
    if (item.qInstalasi) {
    ...
    if (item.qClosing) {
        isClosing = item.qClosing.tglclosing
        jmlFilter.value += 1
    }
...
const changeInst = (e: any) => {
    H.cacheHelper().set('instalasiDipilih', e)
    d_Ruangan.value = e.value ? e.value.ruangan : []
}
const changeRuang = (e: any) => {
    H.cacheHelper().set('ruanganKoding', e.value)
}
...
const fetchDropdown = async () => {
    await useApi().get('/bridging/inacbgs/dropdown').then((r) => {
        ...
        if (H.cacheHelper().get('instalasiDipilih') && H.cacheHelper().get('instalasiDipilih') != null ) {
            item.qInstalasi = H.cacheHelper().get('instalasiDipilih').value
            if (H.cacheHelper().get('ruanganKoding').length) {
                let dataRuangan = H.cacheHelper().get('instalasiDipilih').value.ruangan;
                d_Ruangan.value = dataRuangan ? dataRuangan : []
                item.qRuangan = H.cacheHelper().get('ruanganKoding')
            } else {
                let dataRuangan = H.cacheHelper().get('instalasiDipilih').value.ruangan;
                d_Ruangan.value = dataRuangan ? dataRuangan : []
            }
        } else {
            console.log('Tidak ada cache instalasi!');
        }
...
const changeSwitchFilter = (e: any) => {
//   delete item.ruangan
//   d_Ruangan.value = []
//   if (e == true) {
