<template>
    <section>
      <div>
          <div class="columns is-multiline mt-5">
              ...
              <div class="column is-2">
                <VField>
                    <VControl>
                    <VRadio v-model="item.tipeTgl" value="0" label="Tgl Masuk" color="primary" />
                    <VRadio v-model="item.tipeTgl" value="1" label="Tgl Pulang" color="primary" />
                    </VControl>
                </VField>
                </div>
                  ...
                  <TabView class="tabview-custom mt-3" :scrollable="true" @tab-click="klikTab($event)">
                      <TabPanel>
                          ...
                          <div class="column px-0">
                            <div class="columns is-multiline">
                                <div class="dataTable-info column is-6"><b>Menampilkan {{ currentPage.page }} ke {{ currentPage.limit }}
                                dari
                                {{ dataSource.total }} entri data</b>
                                </div>
                                <div class="column is-2" style="text-align: end;">
                                    <VButton color="primary" @click="downloadRAR(activeTab)" outlined
                                        icon="fas fa-file-excel" :loading="isLoadingZIP">Download ZIP (BENAR)
                                    </VButton>
                                </div>
                                <div class="column is-2" style="text-align: end;">
                                    <VButton color="primary" @click="downloadAll(activeTab)" outlined
                                        icon="fas fa-file-excel" :loading="isLoadingExport">Download All (BENAR)
                                    </VButton>
                                </div>
                                <div class="column is-1" style="text-align: end;">
                              ...
                              <DataTable :value="dataSourcefiltered" class="p-datatable-md"
                                  ...
                                  <Column field="iskonsul" header="Konsul/Lab/Rad" style="min-width: 60px;">
                                      <template #body="slotProps">
                                          ...
                                          <VButton type="button" rounded outlined color="danger" raised v-if="slotProps.data.deptid == 3 || slotProps.data.deptid == 27"
                                                :loading="isGrouping" @click="kunjungansebelumnya(slotProps.data)"> Kunjungan Sebelumnya
                                          </VButton>
                                      </template>
                                  </Column>
                                  ...
                                  <Column header="Verifikasi 2" style="min-width: 60px" align="center">
                                      <template #body="slotProps">
                                          <div class="columns">
                                              <div class="column">
                                                  <button v-if="slotProps.data.tglverifklaim != null" type="button" aria-hidden="false" class="button is-outlined is-raised is-primary" @click="verifikasi2(slotProps.data)">
                                                  <span class="icon"><i aria-hidden="true" class="fas fa-check"></i></span>
                                                  </button>
                                                  <button v-if="slotProps.data.tglverifklaim == null" type="button" aria-hidden="false" class="button is-outlined is-raised is-danger" @click="verifikasi2(slotProps.data)">
                                                  <span class="icon"><i aria-hidden="true" class="lnil lnil-cross-circle"></i></span>
                                                  </button>
                                              </div>
                                          </div>
                                  ...
                                  <Column header="Status Perbaikan" style="min-width: 60px" align="center">
                                      <template #body="slotProps">
                                          <div class="columns">
                                              <div class="column">
                                                  <button v-if="slotProps.data.statusverifikasi == true" type="button" aria-hidden="false" class="button is-outlined is-raised is-primary">
                                                  <span class="icon"><i aria-hidden="true" class="fas fa-check"></i></span>
                                                  </button>
                                                  <button v-if="slotProps.data.statusverifikasi == false" type="button" aria-hidden="false" class="button is-outlined is-raised is-danger">
                                                  <span class="icon"><i aria-hidden="true" class="lnil lnil-cross-circle"></i></span>
                                                  </button>
                                              </div>
                                          </div>
                                  ...
                                  <Column field="" header="Download V2" :sortable="true" style="min-width: 60px; display: none !important">
                                      <template #body="slotProps">
                                          <div class="columns">
                                              <div class="column">
                                                  <button type="button" aria-hidden="false" class="button is-outlined is-raised is-info" @click="bundleDokDownloadV2(slotProps.data)">
                                                  <span class="icon"><i aria-hidden="true" class="fas fa-download"></i></span>
                                                  </button>
                                              </div>
                                          </div>
                      ...
                      <TabPanel>
                          ...
                          <div class="column px-0">
                            <div class="columns is-multiline">
                                <div class="dataTable-info column is-8"><b>Menampilkan {{ currentPage.page }} ke {{ currentPage.limit }}
                                dari
                                {{ dataSource.total }} entri data</b>
                                </div>
                                <div class="column is-2" style="text-align: end;">
      ...
      <Dialog v-model:visible="modalDetail" modal header="Detail 18 Variable" :style="{ width: '70vw' }">
        ...
        <Dialog v-model:visible="modalCPPT" modal header="Riwayat CPPT" :style="{ width: '70vw' }"
            :maximizable="true">
            ...
            <div class="columns is-multiline">
                <div class="column is-12">
                    <table class="tg" width="100%">
                          ...
                          <tbody v-for="(itemss, index) in input2" :key="index">
                            <tr v-for="(item, index2) in itemss.detailsFull" :key="index2">
                              <td colspan="3" style="size:100%;">
                                <table style="border:1px solid black; min-width:100%;" class="tabel">

                                  <!-- mulai sini -->
                                  <tr style="background: #d6d4d4;">
                                    <td style="display:none !important">
                                      <VIconButton circle icon="feather:chevron-down" raised bold
                                        v-tooltip-prime.top="' Collapsed'" v-if="itemss.show"
                                        @click="itemss.show = false">
                                      </VIconButton>
                                      <VIconButton circle icon="feather:chevron-right" raised bold
                                        v-tooltip-prime.top="' Expand'" v-if="!itemss.show" @click="itemss.show = true">
                                      </VIconButton>

                                    </td>
                                    <td colspan="2" height="40px;">
                                        <div style="margin-top: 10px;">
                                            &emsp;<input type="checkbox" v-model="item.checkedCPPT" name="checked['{{ item.uuid }}']" />&emsp;
                                            <span class="text-normal-1">
                                                <b>PPA</b> : {{ item.tenagaMedis && item.tenagaMedis.label ? item.tenagaMedis.label : '' }}
                                    ...
                                    <td style="display:none !important">
                                      <span class="text-normal-1 bold" v-if="itemss.noemr == ''">
                                        <VTag :color="'danger'" class="is-pulled-right" :label="'SIMRS LAMA'" rounded
                                          outlined />
                                      </span>
                                    </td>
                                  </tr>
                                  ...
                                  <tr>
                                    ...
                                    <td>
                                      <table class="tg" width="100%" v-if="item.flag !== 'gizi'">
                                        <tr>
                                          <td width="5%"
                                          ...
                                          <td width="5%" style="display:none !important">
                                            <VIconButton circle icon="feather:copy" color="warning" raised bold outlined
                                              @click="copyToClipboard(item.S)" class="ml-1"
                                        ...
                                        <tr>
                                          ...
                                          <td width="5%" style="display:none !important">
                                            <VIconButton circle icon="feather:copy" color="warning" raised bold outlined
                                              @click="copyToClipboard(item.O)" class="ml-1"
                                              v-tooltip-prime.top="'Copy '">
                                            </VIconButton>
                                        ...
                                        <tr>
                                              ...
                                              <div class="column is-12">
                                                ...
                                                <div style="overflow-y:auto;" class="mt-1">
                                                  <table class="tg" width="100%">
                                                    ...
                                                    <tbody v-for="(itemsss, index3) in item.diagnosaDokter"
                                                      :key="index3">
                                                      <tr v-if="itemsss.jenisDiagnosa">
                                                        ...
                                                        <td width="5%">
                                                          <VIconButton circle icon="feather:copy" color="warning" raised
                                                            bold outlined @click="copyItemDiagnosa(itemsss, 'ICD10')"
                                                            class="ml-1" v-tooltip-prime.top="'Copy Diagnosa '">
                                                          </VIconButton>
                                                        </td>
                                                      </tr>
                                                    </tbody>
                                              ...
                                              <div class="column is-12">
                                                ...
                                                <div style="overflow-y:auto;" class="mt-1">
                                                  <table class="tg" width="100%">
                                                    ...
                                                    <tbody v-for="(itemsss, index3) in item.diagnosaDokter9"
                                                      :key="index3">
                                                      <tr v-if="itemsss.diagnosaa">
                                                        ...
                                                        <td class="tg-0lax">
                                                          <div class="column p-1">
                                                            <VIconButton circle icon="feather:copy" color="warning"
                                                              raised bold outlined
                                                              @click="copyItemDiagnosa(itemsss, 'ICD9')" class="ml-1"
                                          ...
                                          <td width="5%" v-if="item.flag == 'profesi lain'"
                                            ...
                                            <VIconButton circle icon="feather:copy" color="warning" raised bold outlined
                                              @click="copyToClipboard(item.A)" class="ml-1"
                                              v-tooltip-prime.top="'Copy '">
                                            </VIconButton>

                                          </td>
                                          <td width="5%" style="vertical-align:middle; display:none !important"
                                            v-if="item.flag == 'dokter'">
                                            <VIconButton circle icon="feather:copy" color="warning" raised bold outlined
                                              @click="copyDOKTERA(item)" class="ml-1"
                                        ...
                                        <tr>
                                          ...
                                          <td colspan="2">
                                            ...
                                            <div class="column" v-if="item.flag == 'perawat'"
                                              style="display:none !important">
                                              <span style="font-size:11pt;font-weight:bold">Tujuan Kriteria (SLKI)
                                              </span>
                                              <div class="mt-1">
                                                <table class="tg" width="100%">
                                                  <thead>
                                          ...
                                          <td width="5%" style="display:none !important">
                                            <VIconButton circle icon="feather:copy" color="warning" raised bold outlined
                                              @click="copyToClipboard(item.P)" class="ml-1"
                                              v-tooltip-prime.top="'Copy '">
                                            </VIconButton>
                                        ...
                                        <tr>
                                          <td width="5%" style="width: 5%;vertical-align: middle;font-size:14px;font-weight:bold">
                                            Intruksi
                                          </td>
                                          <td colspan="2">
                                            {{ item.dpjpUtama?.label }} <br>
                                            <span v-if="item.intruksiPPA" style="white-space: pre-line">Instruksi: {{ item.intruksiPPA ?? '' }}</span>
                                        ...
                                        <tr v-if="item.dokterjaga != null && item.dokterjaga != undefined && item.dokterjaga.label != null">
                                          <td style="vertical-align: middle;" colspan="3">
                                            <span style="font-size:14px;font-weight:bold">Dokter Jaga :</span> {{  item.dokterjaga.label  }}
                                          </td>
                                        </tr>
                                        <tr v-if="item.dpjpRawatBersama != null && item.dpjpRawatBersama.length && item.dpjpRawatBersama[0].id != null">
                                          <td style="vertical-align: middle;text-align:center;font-size:14px;font-weight:bold" colspan="3">
                                            Dokter Rawat Bersama
                                          </td>
                                        </tr>
                                        <tr v-if="item.dpjpRawatBersama != null && item.dpjpRawatBersama.length && item.dpjpRawatBersama[0].id != null" v-for="data in item.dpjpRawatBersama" :key="index">
                                          <td style="vertical-align: middle;text-align:left;font-size:12px;" colspan="3">
                                                  ...
                                                  <div class="column is-12">
                                                    ...
                                                    <b>Tanggal</b> : {{ item ?
                                                      ...
                                                      <b>Nama</b> : {{item.petugasPemberi ?
                                                        ...
                                                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=70x70&data={{ ($item.petugasPemberi ? $item.petugasPemberi.label : '-')}}" alt="TTD" width="100">
                                                  </div>
                                                </div>
                                                <div class="column is-6" style="font-size: 12px">
                                                  <div class="column is-12">
                                                    ...
                                                    <b>Tanggal</b> : {{ item ?
                                                      ...
                                                      <b>Nama</b> : {{ item.petugasPenerima ?
                                                        ...
                                                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=70x70&data={{ ($item.petugasPenerima ? $item.petugasPenerima.label : '-')}}" alt="TTD" width="100">
                                                  </div>
                                                </div>
                                              </div>
                                            </div>
                                      ...
                                      <table class="tg" width="100%" v-else>
                                        <tr>
                                          <td width="5%"
                ...
                <div class="column is-2">
                    <VButton color="primary" raised outlined rounded icon="fas fa-clipboard-list"
                        class="w-100 mt-5" @click="collectCPPT()">Collect
                    </VButton>
                </div>
                  ...
                  <template #item="slotProps">
                        <div class="columns is-multiline p-2" >
                            ...
                            <div class="column is-2">
                                ...
                                <VButton color="primary" raised outlined rounded icon="fas fa-clipboard-list"
                                    class="w-100" @click="cppt(slotProps.item)" v-if="slotProps.item.kodeexternal == 'cppt'">Lihat
                                </VButton>
                                <VButton color="primary" raised outlined rounded icon="fas fa-clipboard-list"
                                    class="w-100" @click="cetakanPA(slotProps.item)" v-else-if="slotProps.item.name == 'Hasil Laboratorium PA'">Cetak PA
                                </VButton>
                                <VButton color="primary" raised outlined rounded icon="fas fa-clipboard-list"
                            ...
                            <div class="column is-2">
                                ...
                                <VButton v-if="slotProps.item.api != null && slotProps.item.api.toUpperCase().indexOf('CETAKBILLINGCASEMIX') > -1" color="success" raised rounded icon="fas fa-eye"
                                    class="w-100" @click="viewExcel(slotProps.item)">Lihat Excel
                                </VButton>
                            </div>
                        </div>
                  </template>
      ...
      <Dialog v-model:visible="modalVerifikasi" modal header="Keterangan" :style="{ width: '50vw' }">
          <div class="columns is-multiline">
              <div class="column is-12">
                    <VField>
                        <VControl>
                            <VRadio v-model="item.statusverifikasi" value="0" label="PERBAIKAN" color="primary" />
                            <VRadio v-model="item.statusverifikasi" value="1" label="BENAR" color="primary" />
                        </VControl>
                    </VField>
                    <VField v-if="hideverif == false">
                        <VControl>
  ...
  const lastAPD: any = ref('')
  const lastRM: any = ref('')
  const lastPD: any = ref('')
  ...
  const isLoadingZIP: any = ref(false)
  ...
  const ringkasankeluar = async (emrId) => {
    console.log(emrId);
    H.printBladeSelf(`emr/cetak/RingkasanKeluar?noregistrasi=${emrId.noregistrasi}&pdf=true`);
  }
  
  const cpptdokter = async (emrId) => {
    useApi().get(
        '/bridging/inacbgs-cppt-dokter?nocmfk=' +  emrId.nocmfk + "&tgl_masuk=" + emrId.tgl_masuk
    ).then((response) => {
        console.log('RESPONSE', response);
        H.printBladeSelf(`emr/cetak/CatatanPerkembanganPasienTerintegrasi?emrpasienfk=${response[0]['emrpasienfk']}&flag=dokter&pdf=true&noregistrasi=${emrId.noregistrasi}`);
    })
  }
  ...
  const exportExcel = () => {
      ...
      const worksheet = XLSX.utils.aoa_to_sheet([
          ...
          [],
          ['NO','Pasien','No RM','No Registrasi','No SEP','Ruangan','Kelompok Pasien','Dokter','Catatan','Konsul/Lab/Rad','Ruangan Konsul','Penunjang','Verifikasi','Ringkasan Keluar','Status Perbaikan','Status Inacbg','Tgl Masuk','Tgl Pulang','Total Billing'],
          ...dataSourceExport.value.map((e: any) => [
              ...
              e.statusverifikasi == true ? "Ya" : "Tidak",
  ...
  const kunjungansebelumnya = (e: any) => {
    useApi().get('/bridging/inacbgs/get-kunjungan-sebelumnya?nocmfk='+e.nocmfk+'&tglregistrasi='+e.tgl_masuk+'&nosep='+e.nomor_sep).then((x) => {
        console.log(x.data)
        if (x.data.length == 0) {
...
//             if(element.statusverifikasi == true){
...
//     }
    ...
    const downloadAll = async (e: any) => {
        ...
        await useApi().get(
          ...
          + '&status_pasien=' + statusPasien
            ...
            dataSourceAll.value = response
            console.log(response)
            console.log(dataSourceAll.value)

            for(var z = 0; z < dataSourceAll.value.length; z++){
                let element = dataSourceAll.value[z]
                if(element.ketverifikasi == 'BENAR' && element.statusverifikasi == true){
                    console.log(element.nomor_sep)
                    let noregistrasi = element.noregistrasi
                    listDokumen.value = element
                    for(var x =0; x < listDokumen.value.dokumen.length; x++){
                    let element = listDokumen.value.dokumen[x]
    ...
    const downloadRAR = async (e: any) => {
        ...
        let namafile = ''
        namafile = 'bundle_' + dari.replace(/-/g,'') + '_' + sampai.replace(/-/g,'') + '.zip';

        let token = useUserSession().token;
   

        //const download = await H.downloadRAR('bridging/inacbgs/bundle-dokumen-rev-download-rar?tglAwal=' + dari + '&tglAkhir=' + sampai + '&namafile=' + namafile, namafile);
        //console.log(download)
        //if(download == undefined){
        //    isLoadingZIP.value = false
        //}

        //window.open(`${import.meta.env.VITE_API_BASE_URL}/bridging/inacbgs/bundle-dokumen-rev-download-rar?tglAwal=${dari}&tglAkhir=${sampai}&namafile=${namafile}&token=${token}`, '_blank');

        window.open(import.meta.env.VITE_API_BASE_URL + 'bridging/inacbgs/bundle-dokumen-rev-download-rar?token=' + useUserSession().token + '&tglAwal=' + dari + '&tglAkhir=' + sampai + '&namafile=' + namafile, '_blank');
  ...
  const verifikasi = async (e: any) => {
    ...
    //   }
      ...
      lastRM.value = e.nocmfk
      NOREC_PD.value = e.norec
      collectDokumenModal(e)
      modalDokumen.value = true
  }
  const verifikasi2 = async (e: any) => {
      let jml = 0
      e.dokumen.forEach((element: any) => {
          if (element.doc != null) {
              jml = jml + 1
  ...
  const viewExcel = (e: any) => {
      H.printBlade(`kasir/billing/report/rincian-biaya-casemix?noregistrasi=${e.noregistrasi}&bangsa=WNI&isexcel=true`)
  }
  ...
  const bundleDok2 = (e: any) => {
      ...
      H.openInTab('bridging/inacbgs/bundle-dokumen-rev?noregistrasi=' + e + '&urutdokumen=' + urutdokumen);
  ...
  const bundleDokDownload = (e: any) => {
      ...
      // console.log(H.opens('bridging/inacbgs/bundle-dokumen-rev-download?noregistrasi=' + e.noregistrasi + '&urutdokumen=' + urutdokumen));
  ...
  //   link.click();
      ...
      // const linkSource = H.open('bridging/inacbgs/bundle-dokumen-rev-download?noregistrasi=' + e.noregistrasi + '&urutdokumen=' + urutdokumen);
      // const downloadLink = document.createElement("a");
  ...
  const cetakanPA = async (e: any) => {
    H.printBlade(`laboratorium/get-hasil-pa-bridging-klaim?noregistrasi=${e.noregistrasi}&pdf=true`);
    // bundleDok2(e.noregistrasi)
    // H.openInTab('bridging/inacbgs/bundle-dokumen-rev?noregistrasi=' + e + '&urutdokumen=' + urutdokumen);
  }

    const cppt = async (e: any) => {
        console.log('E', e)

        lastPD.value = e.norec_pd

        useApi().get(
        `/emr/get-emr-cppt?nocmfk=${lastRM.value}&norec_pd=${e.norec_pd}&collection=CatatanPerkembanganPasienTerintegrasi`).then((response: any) => {
            if (response.length) {
                let dataOLD = []
    ...
    for(var x =0; x < listDokumen.value.dokumen.length; x++){
        ...
        if(listDokumen.value.inacbg_status == "Sudah Kirim Ke Data Center"){
            await useApi().post('/bridging/inacbgs/collect-dokumen', {
                ...
                countPersen()
                if(element.kodeexternal == 'hasil_lab_pa'){
                    cetakanPA(e)
                } 
            }).catch((er) => {
                loadingUpload.value = false
            ...
            if(element.kodeexternal != "klaim_indi" && element.kodeexternal != "klaim_ind"){
                await useApi().post('/bridging/inacbgs/collect-dokumen', {
                    'norec_pd': element.norec_pd,
                    'documentklaimfk': element.documentklaimfk,
                    'namafile': element.kodeexternal,
  ...
  const collectDokumenModal2 = async (e: any) => {
    isLoading.value = true
    console.log('E', e)
    for(var x =0; x < listDokumen.value.dokumen.length; x++){
        let element = listDokumen.value.dokumen[x]
        if(listDokumen.value.inacbg_status == "Sudah Kirim Ke Data Center"){
            await useApi().postNoMessage('/bridging/inacbgs/collect-dokumen', {
                'norec_pd': element.norec_pd,
                'documentklaimfk': element.documentklaimfk,
                'namafile': element.kodeexternal,
                'tglregistrasi': element.tglregistrasi,
        ...
        if(x == listDokumen.value.dokumen.length-1){
            bundleDok2(e.noregistrasi)
            verifikasiDokumenModal(e.noregistrasi)
        }
    }
  }

  const collectCPPT = async (e: any) => {
    //console.log('INPUT', input2.value)
    const element = input2.value[0];
    console.log('INPUT 0', element.detailsFull.length)
    var string = '';
    for (let y = 0; y < element.detailsFull.length; y++) {
        const element2 = element.detailsFull[y];
        if(element2.checkedCPPT == true){
            console.log('CEK', element2.uuid)
            string += element2.uuid + ','
            console.log('STRING LOOP', string)
        }
    }
    console.log('STRING AKHIR', string)
    H.printBlade(`emr/cetak/cetak-cppt-klaim?pdf=true&norec_pd=${lastPD.value}&nocmfk=${lastRM.value}&from=CPPT&arr=${string}`)
  }

  const collect = async (e: any) => {
  
//   H.printBlade(`emr/cetak/SuratPermintaanPenggunaanObatKhususKemoterapi?noregistrasi=${e.noregistrasi}&pdf=true`);
    console.log('E', e)
    loadingUpload.value = true
    await useApi().post('/bridging/inacbgs/collect-dokumen', {
        'norec_pd': e.norec_pd,
  ...
  const verifikasiDokumen = async (e: any) => {
      ...
      await useApi().post('/bridging/inacbgs/verif-dokumen', {
          ...
          item.statusverifikasi = undefined
          fetchData()
      }).catch((er) => {
          isLoading.value = false
      })
      ...
      (newValue, oldValue) => {
          if(newValue!=oldValue){
            if(item.statusverifikasi == 0){
                hideverif.value = false
            } else{
                hideverif.value = true
            }
          }
      }
  )
