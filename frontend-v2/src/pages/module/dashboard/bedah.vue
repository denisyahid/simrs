      ...
      <div class="column is-8">
              ...
              <div class="header-meta">
                ...
                <div class="columns is-multiline">
                  ...
                  <div class="column is-4">
                    <VField grouped>
                      <VControl raw subcontrol>
                        <VCheckbox v-model="item.isTglOperasi" label="Tgl Operasi" color="info" />
                      </VControl>
                    </VField>
                  </div>
                  <div class="column is-4">
                    <VField grouped>
                      <VControl raw subcontrol>
                        <VCheckbox v-model="item.isclosing" label="Sudah Closing" color="info" />
                      </VControl>
                    </VField>
                  </div>
                  <div class="column is-8 is-flex is-align-items-center" style="margin-top: -20px;">
                  ...
                  <div class="column is-4" style="margin-top: -20px;"
                    ...
                    <VControl class="is-pulled-right" style="font-weight: bold !important;">
                      <VSwitchBlock v-model="item.pasienDokter" label="Pasien Dokter" color="danger" />
                    </VControl>
                  </div>
                </div>
              </div>
        ...
        <div class="column is-12" style="margin-top: 1rem;">
          <VTabs @update:selected="changeMenu($event)" slider selected="Pasien" :tabs="[
            ...
            { label: 'Penjadwalan Operasi', value: 'Laporan' },
          ]" style="margin-top: -2rem;">

            <template #tab="{ activeValue }">

              <div v-if="activeValue === 'Pasien'">
                <div class="list-view list-view-v3">
                  ...
                  <!-- PLACE HOLDER AMPRAHAN OBAT -->
                  <VPlaceholderPage :title="H.assets().notFound" :subtitle="H.assets().notFoundSubtitle" class="my-6 isamprah"
                    :class="[dataOrder.length !== 0 && 'is-hidden']" v-if="dataOrder.length == 0">
                    <template #image>
                        ...
                        <div class="list-view-item-inner">
                          ...
                          <div class="meta-right">
                            ...
                            <VIconButton v-else v-tooltip.bottom="'Preview'" color="blue" circle
                              icon="fas fa-file-medical-alt" @click="orderVerify(items)" :loading="items.loading"
                              style="margin-right: 15px;" />
                            ...
                            <VIconButton v-tooltip.bottom.left="'Cetak SEP'" label="Bottom Left" color="warning" circle
                              icon="pi pi-print" @click="cetakSEP(items)" :loading="item.loading">
                            </VIconButton>
                          </div>
                        </div>
                      </div>
              ...
              <div v-else-if="activeValue === 'Operasi'">
                <div class="list-view list-view-v3">
                  ...
                  <Vcard>
                    <div class="search-menu mb-2">
                      <div class="search-location" style="width: 100%">
                        <i class="iconify" data-icon="feather:search"></i>
                        <input type="text" placeholder="Cari Nama Pasien, No Registrasi atau No RM" v-model="item.qsearch"
                          v-on:keyup.enter="fetchOperasi()" />
                      </div>
                      <div class="search-salary">
                        <i class="iconify" data-icon="feather:clipboard"></i>
                        <input type="text" placeholder="No RM" v-model="item.qnocm" />
                        ...
                        <VField>
                          <VControl class="prime-auto">
                            <Calendar inputId="range" v-model="item.qPeriode" selectionMode="range" :manualInput="false"
                              class="w-100" :showIcon="true" />
                          </VControl>
                        </VField>
                      </div>
                      <VButton raised class="search-button" @click="fetchOperasi(order)" :loading="isLoading"> Cari Data
                      </VButton>
                    </div>
                  </Vcard>
                  ...
                  <div class="list-view-inner mt-2" style="max-height:1000px;overflow: auto;">
                    ...
                    <DataTable :value="dataPasien" class="p-datatable-md" :loading="isLoading" rowGroupMode="subheader"
                      groupRowsBy="namaruangan" sortField="namaruangan" sortMode="single" scrollable
                      :metaKeySelection="metaKey" selectionMode="single" scrollHeight="600px"
                      tableStyle="min-width: 50rem" @rowSelect="onRowSelect" v-model:selection="selectedPasien"
                      :totalRecords="dataPasien.total"
                      paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown"
                      responsiveLayout="stack" breakpoint="960px" :rowClass="rowClass"
                      ...
                      <Column header="Action" style="width: 100px; text-align: center">
                        <template #body="slotProps">
                          <VIconButton color="primary" circle icon="lucide:shopping-cart" outlined raised
                            class="text-center" @click="inputTindakan(slotProps.data)" v-tooltip.bottom.left="'Tindakan'">
                          </VIconButton>
                        </template>
                      </Column>
                      <Column header="Periksa" style="width: 100px; text-align: center">
                        <template #body="slotProps">
                          <VIconButton color="primary" circle icon="fas fa-stethoscope" outlined raised
                            class="text-center" @click="emr(slotProps.data)" v-tooltip.bottom.left="'EMR'"></VIconButton>
                        </template>
                      </Column>
                      <!-- <VIcon icon="lucide:shopping-cart" /> -->
                      <Column header="Pindah" style="width: 100px; text-align: center">
                        <template #body="slotProps">
                          <VDropdown icon="feather:more-vertical" spaced right class="mr-2"
                            v-tooltip.bottom.left="'Pulang Atau Pindah'">
                            <template #content>
                              <a role="menuitem" @click="detailRegistrasi(slotProps.data)" class="dropdown-item is-media">
                                <div class="icon">
                              ...
                              <a role="menuitem" @click="pindahPulang(slotProps.data)" class="dropdown-item is-media">
                                <div class="icon">
                                  <i class="fas fa-home" aria-hidden="true"></i>
                                </div>
                                <div class="meta">
                                  <span>Pulang / Pindah</span>
                                </div>
                              </a>
                            </template>
                          </VDropdown>
              ...
              <div v-else-if="activeValue === 'Laporan'">
                <div class="search-menu mb-2">
                  <div class="search-location">
                    <i class="iconify" data-icon="feather:activity"></i>
                    <input type="text" placeholder="No Registrasi" v-model="item.qnoregistrasi" />
  ...
  <VModal :open="modalDetailOrder" title="Verifikasi Order" noclose size="big" actions="right"
    ...
    <template #content>
                ...
                <div class="current-user">
                  ...
                  <div class="columns is-multiline">
                    <div class="column is-4">
                      ...
                      <VTag :color="item.iscito ? 'danger' : 'secondary'" :label="item.iscito ? 'Ya' : 'Tidak'" />
                    </div>
                    <div class="column is-4">
                      <h5 class="block-heading">
                        ELEKTIF
                      </h5>
                      <VTag :color="item.iselektif ? 'danger' : 'secondary'" :label="item.iselektif ? 'Ya' : 'Tidak'" />
                    </div>
                    <div class="column is-4">
                      <h5 class="block-heading">
                        URGENT
                      </h5>
                      <VTag :color="item.isurgent ? 'danger' : 'secondary'" :label="item.isurgent ? 'Ya' : 'Tidak'" />
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div class="columns is-multiline" style="padding: 2rem 0rem;">
        ...
        <div class="column is-4">
          <VField>
            <VDatePicker v-model="item.tgloperasi" mode="dateTime" style="width: 100%" :is24hr="true"
              :format="H.formatDateIndo">
              <template #default="{ inputValue, inputEvents }">
                <VField>
                  <VControl icon="feather:calendar" fullwidth>
                    <VInput :value="inputValue" placeholder="Jadwal Operasi" v-on="inputEvents" class="is-rounded" />
                  </VControl>
                </VField>
              </template>
            </VDatePicker>
        ...
        <div class="column is-4">
          <VField class="label-unset">
            <VControl>
              <AutoComplete v-model="item.dokterAnak" :suggestions="d_Dokter" @complete="fetchDokter($event)"
                :optionLabel="'label'" :dropdown="true" :minLength="3" :appendTo="'body'" :loadingIcon="'pi pi-spinner'"
                :field="'label'" placeholder="ketik nama Dokter" class="is-rounded" />
            </VControl>
          </VField>
        </div>
        <!-- <div class="column is-2">
        ...
        <div class="column is-4">
          <VField>
            <VControl icon="feather:list" class="prime-auto-select">
              <Dropdown v-model="item.statusoperasi" :options="d_Status" :optionLabel="'label'" placeholder="Pilih data"
                style="width: 100%;" showClear :filter="true" class="is-rounded" />
            </VControl>
          </VField>
        </div>
        <div class="column is-2">
        ...
        <div class="column is-4">
          <VField>
            <VControl class="mt-2">
              <VInput type="text" placeholder="Estimasi Waktu" class="is-rounded" v-model="item.estimasiwaktuoperasi" />
            </VControl>
          </VField>
        </div>
        <div class="column is-2">
        ...
        <div class="column is-4">
          <VField class="label-unset">
            <VControl>
              <AutoComplete v-model="item.userPengirim" :suggestions="d_Dokter" @complete="fetchDokter($event)"
                :optionLabel="'label'" :dropdown="true" :minLength="3" :appendTo="'body'" :loadingIcon="'pi pi-spinner'"
                :field="'label'" placeholder="ketik nama Dokter" class="is-rounded" />
            </VControl>
          </VField>
        ...
        <div class="column is-4" style="display: none !important">
          <VField>
            <VControl class="mt-2">
              <VInput type="text" placeholder="Tindakan Operasi" class="is-rounded" v-model="item.tindakanoperasi" />
            </VControl>
          </VField>
        </div>
        ...
        <div class="column is-4">
          <VField class="label-unset">
            <VControl>
              <AutoComplete v-model="item.userPenerima" :suggestions="d_pegawai" @complete="fetchDokter1($event)"
                :optionLabel="'value'" :dropdown="true" :minLength="3" :appendTo="'body'" :loadingIcon="'pi pi-spinner'"
                :field="'label'" placeholder="ketik nama Perawat" class="is-rounded" />
            </VControl>
          </VField>
        </div>
          ...
          <div class="columns">
            ...
            <div class="column is-6" v-if="item.isanastesi == 1">
              <div class="columns is-multiline">
                ...
                <template v-if="item.isanastesi" v-for="(idanastesi, indx) in item.anastesitambahanfk" :key="indx">
            ...
            <div class="column is-6">
              <div class="columns is-multiline">
                ...
                <div class="column is-4">
                  <h6 class=" is-5  mt-3">
                    <span> Alat Amprahan </span>
                  </h6>
                </div>
                <div class="column is-8">
                  <VField class="label-unset">
                    <VControl class="mt-2">
                      <VTextarea v-model="item.alat" placeholder="Alat Amprahan" disabled rows="2" />
                    </VControl>
                  </VField>
                </div>
                <template v-if="item.operatorhelperfk.length > 0" v-for="(idoperator, indxo) in item.operatorhelperfk"
                  :key="indx">
                  <div class="column is-4">&nbsp;</div>
                  <div class="column is-8">
                    <VField class="label-unset">
              ...
              <!-- <pre>{{ item }}</pre> -->
              <div class="column is-12" v-if="(item.asalruangan || '').toUpperCase().indexOf('IGD') > -1">
                ...
                <div class="columns is-multiline">
                  ...
                  <div class="column is-8">
                    <VField class="label-unset">
                      <VControl class="mt-2">
                        <!-- <VInput type="text" placeholder="Diagnosis (optional)" class="is-rounded"
                                                v-model="item.diagnosis" /> -->
                        <VTextarea v-model="item.diagnosis" placeholder="Diagnosis Lainnya (optional)" disabled />
                      </VControl>
                    </VField>
                  </div>
                </div>
            ...
            <div class="timeline-container">
              <div class="timeline-item is-unread" v-for="(items, index) in detailOrderVerify" :key="items.norec">
                ...
                <div class="content-wrap is-grey">
                  ...
                  <div style="width: 20%;">
                    <label :style="'font-weight: bold;'">Dokter <span style="color: #bb2124;">Anastesi</span> dan <span
...
const item: any = ref({
  filterRuangan: 363, // Bedah Sentral IBS
  filterDate: new Date(),
  isTglOrder: ref(true),
  isclosing: ref(false),
  filterTgl: ref({
...
const fetchDataOrder = async (q: any) => {
  ...
  try {
    ...
    let statusclosing = item.value.isclosing ? `&statusclosing=${item.value.isclosing}` : '';
    let id_dokter = item.value.pasienDokter == true ? `&id_dokter=${H.pegawaiLogin().id}` : '';
    let opsi = '';
    ...
    const response = await useApi().get(`/dashboard/get-order-bedah?${ruanganid}${dari}${sampai}${qnamapasien}${statusOrder}${qnocm}${qnoregistrasi}${search}&opsi=${opsi}${statusclosing}${id_dokter}`);
...
async function fetchLaporan() {
  ...
  await useApi().get(`/dashboard/laporan-tindakan-operasi?${dari}${sampai}${produk}${qnamapasien}${qnocm}${qnoregistrasi}${search}${id_dokter}`).then((response) => {
...
const orderVerify = async (e: any) => {
  ...
  item.value.tgloperasi = moment(e.tanggaloperasi, "DD-MM-YYYY HH:mm:ss").format("YYYY-MM-DD HH:mm:ss");
  ...
  item.value.riwayatswab = e.riwayatswab
  item.value.riwayatvaksin = e.riwayatvaksin
  item.value.statusorder = e.statusorder
  item.value.isanastesi = e.isanastesi
  // item.value.asalruangan = e.asalruangan
  ...
  item.value.dokterOperator = { value: getHargaLayanan[0].dokteroperatorfk, label: getHargaLayanan[0].dokteroperator }
  item.value.dokterAnastesi = { value: getHargaLayanan[0].dokteranastesifk, label: getHargaLayanan[0].dokteranastesi }
  item.value.asalruangan = getHargaLayanan[0].asalruangan
  ...
  if (getHargaLayanan[0].dokteranakfk != null) {
    item.value.dokterAnak = { value: getHargaLayanan[0].dokteranakfk, label: getHargaLayanan[0].dokteranak }
  }
  if (getHargaLayanan[0].penerimafk != null) {
    item.value.userPenerima = { value: getHargaLayanan[0].penerimafk, label: getHargaLayanan[0].namapenerima }
  }
  item.value.userPengirim = { value: getHargaLayanan[0].dokteroperatorfk, label: getHargaLayanan[0].pengorder }
...
const save = async () => {
  ...
  let parameter = {
    ...
    'iscito': item.value.iscito ? item.value.iscito : null
  }
  ...
  isLoading.value = true;
  await useApi().post('/dashboard/save-order-pelayanan-bedah', { 'parameter': parameter }).then((response: any) => {
...
const rowClass = (data) => {
  return {
    'row-green': data.pelayananpasien === true
  };
}
...
const fetchOperasi = async () => {
  try {
    ...
    let qnoregistrasi = item.value.qnoregistrasi ? `&qnoregistrasi=${item.value.qnoregistrasi}` : '';
    let search = item.value.qsearch ? `&search=${item.value.qsearch}` : '';
    let statusclosing = item.value.isclosing ? `&statusclosing=${item.value.isclosing}` : ''; // 1 = sudah di closing
    let id_dokter = item.value.pasienDokter == true ? `&id_dokter=${H.pegawaiLogin().id}` : '';
    ...
    const response = await useApi().get(
      `/dashboard/get-operasi-pasien-bedah?${qnamapasien}${qnocm}${qnoregistrasi}${search}${ruanganid}${dari}${sampai}${statusclosing}${id_dokter}`
    );
