...
<template>
                    ...
                    <div v-if="activeTab == 0">
                      ...
                      <div class="list-view list-view-v3">
                        <div class="search-menu mb-2">
                          ...
                          <VButton raised class="search-button" @click="cari()" :loading="isLoading"> Cari Data
                          </VButton>
                        </div>
                        ...
                        <div class="list-view-inner mt-2" style="max-height:1000px;overflow: auto;">
                          <div name="list-complete" tag="div">
                            ...
                            <div class="column">
                              ...
                              <DataTable lazy :filters="ds_PASIEN_FILTER" :value="dataPasien" class="p-datatable-md" :loading="isLoadingPasien" :paginator="true" :rows="limit"
                              :rowsPerPageOptions="rowOptions" scrollable v-model:expandedRows="expandedRows"
                              scrollHeight="600px"
                              :metaKeySelection="metaKey"
                              selectionMode="single"
                              :totalRecords="dataPasien.total"
                              @rowSelect="onRowSelect"
                              @page="onPageChange($event)" @update:rows="limit = $event"
  ...
  const kelompokUser = 'nurse-station-rehab-medis'
  const kelompokUserID = 81
  ...
  const fetchPasien = async (e: any) => {
    ...
    dataPasien.value = []
    await useApi().get(`/dashboard/rawat-jalan-pasien-nurse${dari}${sampai}${idPegawai}${ruanganid}${namapasien}${nocm}${qnoreg}${statuspanggil}${search}&page=${page}&limit=${limit.value}`).then((response) => {
