        ...
        <VCard>
            <TabView class="tabview-custom " :scrollable="true" @tab-click="klikTab($event)">
                ...
                <TabPanel>
                    ...
                    <div class="column is-8">
                        <span ><i><b>Catatan :</b>Penerimaan Kasir Yang Dapat disetorkan Bendahara Hanya Tagihan Dengan Cara Bayar Tunai</i></span>
                    </div>
                    ...
                    <DataTable :value="dataCaraBayar" tableStyle="min-width: 50rem" scrollable :paginator="true" :rows="10"
                        ...
                        <Column field="totalPenerimaan" header="Total Yang Akan Disetor" style="min-width: 150px">
                            <template #body="slotProps">
                                {{ H.formatRp(slotProps.data.totalPenerimaan, 'Rp. ') }}
                            </template>
                        </Column>
                        ...
                        <Column :exportable="false" header="##" style="text-align: center;">
                            <template #body="slotProps">
                                <VIconButton type="button" icon="fas fa-arrow-right" class="mr-2" color="warning" circle
                                    outlined raised v-tooltip.top="'Setor'" @click="setor(slotProps.data)" v-if="slotProps.data.idCaraBayar == 1">
                                </VIconButton>
                            </template>
                        </Column>
        ...
        <div class="columns is-multiline">
            ...
            <div class="column is-4">
                <VField>
                    ...
                    <VControl icon="feather:bookmark">
                        <input v-model="item.penerimaanTotal" type="text" class="input is-rounded is-right"
            ...
            <div class="column is-4">
                <VField>
                    ...
                    <VControl icon="feather:bookmark">
                        <input v-model="item.setoran" type="text" class="input is-rounded is-right" disabled/>
                    </VControl>
                </VField>
            </div>
            <div class="column is-4">
                <VField>
                    <VLabel>Sisa</VLabel>
                    <VControl icon="feather:bookmark">
                        <input v-model="item.sisa2" type="text" class="input is-rounded is-right end-action" disabled/>
                    </VControl>
                </VField>
            </div>
...
const fetchData = async () => {
    ...
    await useApi().get(`bendahara/get-daftar-sbm${dari}${sampai}${idPegawai}${carabayar}${KetSetor}`).then((response) => {
        ...
        // dataDetail.value = response.data.details
...
const setor = (e: any) => {
    ...
    item.value.penerimaanTotal =H.formatRupiah(parseFloat(e.totalPenerimaan))
    item.value.setoran =H.formatRupiah(parseFloat(e.totalPenerimaan))
    item.value.sisa2 = 0
    item.value.caraBayar = e.idCaraBayar
...
console.log(e)
    ...
    for (var i = 0; i < dataCaraBayar.value.length; i++) {
        ...
        objStr = {
            ...
            "totalPenerimaan":H.unFormatRupiah(dataCaraBayar.value[i].totalPenerimaan),
    ...
    let formData = {
        ...
        'setoran': H.unFormatRupiah(item.value.setoran),
...
watch(
    ...
    (newValue, oldValue) => {
        ...
        item.value.sisa2 =H.unFormatRupiah(item.value.penerimaanTotal)  - H.unFormatRupiah(newValue);
    }
)
